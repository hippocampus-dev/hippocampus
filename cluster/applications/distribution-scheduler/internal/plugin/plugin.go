...
func New(_ context.Context, _ runtime.Object, _ framework.Handle) (framework.Plugin, error) {
	return &DistributionScheduler{}, nil
}
...
func (d *DistributionScheduler) PreFilter(ctx context.Context, state framework.CycleState, pod *v1.Pod, nodeInfos []framework.NodeInfo) (*framework.PreFilterResult, *framework.Status) {
...
func (d *DistributionScheduler) Score(ctx context.Context, state framework.CycleState, pod *v1.Pod, nodeInfo framework.NodeInfo) (int64, *framework.Status) {
	data, err := readStateData(state)
	if err != nil {
		return 0, framework.NewStatus(framework.Success)
	}

	return framework.MaxNodeScore / int64(data.siblingsByNode[nodeInfo.Node().Name]+1), framework.NewStatus(framework.Success)
}
