...
import embedding_gateway.cancellation
...
def get_processed_tokens_total() -> opentelemetry.metrics.Counter:
    ...
    if global_processed_tokens_total is None:
        global_processed_tokens_total = (
            embedding_gateway.telemetry.meter.create_counter(
...
async def embeddings(
    ...
    processed_tokens_total: opentelemetry.metrics.Counter = fastapi.Depends(
        get_processed_tokens_total
    ),
) -> typing.Any:
    raw_body = body.root
    ...
    try:
        async with embedding_gateway.cancellation.cancel_on_disconnect(request):
            try:
                ...
                with embedding_gateway.telemetry.tracer.start_as_current_span(
