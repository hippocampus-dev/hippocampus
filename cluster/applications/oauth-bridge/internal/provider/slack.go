...
func (s *Slack) AuthorizationURL(clientID string, scope string, state string, redirectURI string, _ string) string {
...
func (s *Slack) ExchangeCode(ctx context.Context, clientID string, clientSecret string, code string, redirectURI string, _ string) (map[string]interface{}, error) {
	values := url.Values{
		"client_id":     {clientID},
