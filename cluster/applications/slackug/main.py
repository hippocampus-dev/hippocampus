    ...
    with slackug.telemetry.tracer.start_as_current_span(
        ...
        match s.brain_type:
            case slackug.settings.BrainType.S3:
                ...
                await slackug.slack.manager.register(
                    bolt, slackug.brain.S3Brain(s3_client, s.s3_bucket)
                )
            case _:
...
async def handle_event(
    ...
    groups = (
        slackug.slack.manager.UserGroups.model_validate_json(restored_groups)
        if restored_groups
        else slackug.slack.manager.UserGroups.model_validate({})
    )
    ...
    if group is not None and (
        group.channel_id is None or group.channel_id == event["channel"]
    ):
...
async def handle_message(
    ...
    with slackug.telemetry.tracer.start_as_current_span(
        ...
        if (event.get("subtype") not in ["bot_message"]) and (
            user := event.get("user", event.get("username"))
        ) is not None:
