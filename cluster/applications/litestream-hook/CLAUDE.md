...
2. **Injection Logic**:
   ...
   - **Sidecar Container**: `litestream-hook-replicate` runs continuous replication
     - With `--enable-sidecar-containers=true` (Kubernetes 1.28+): Added to InitContainers with `RestartPolicy: Always` (native sidecar)
     - With `--enable-sidecar-containers=false` (default): Added to Containers as a regular sidecar
...
4. **CLI Flags**:
   - `--enable-sidecar-containers`: Enable native sidecar containers (requires Kubernetes 1.28+). When enabled, the replicate container starts before the main container and terminates after it, ensuring proper database replication lifecycle.
