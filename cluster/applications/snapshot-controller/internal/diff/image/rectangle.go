...
func (r *RectangleDiff) Calculate(baseline image.Image, target image.Image) *DiffResult {
	...
	diffRatio := r.calculateDiffRatio(baseline, target, rectangles)
...
func (r *RectangleDiff) calculateDiffRatio(baseline image.Image, target image.Image, rectangles []Rectangle) float64 {
	totalDiffArea := 0
	for _, rect := range rectangles {
