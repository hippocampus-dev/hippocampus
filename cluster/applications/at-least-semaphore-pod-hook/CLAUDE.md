...
3. When a pod is created, it attempts to pop from the queue; stale inflight entries (heartbeat score older than threshold) are reclaimed back to queue
4. If successful, the pod gets a sidecar container that periodically heartbeats to the inflight Sorted Set
5. On SIGTERM, the sidecar moves the semaphore from inflight Sorted Set back to queue
