...
func Run(a *Args) error {
	...
	webhookServer.Register("/mutate", &webhook.Admission{Handler: &handler{
		...
		staleThresholdSeconds:   a.StaleThresholdSeconds,
	}})
...
func (h *handler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case metav1.GroupVersionKind{Group: "", Version: "v1", Kind: "Pod"}:
		...
		inflightNamespacedKey := fmt.Sprintf("heartbeat/%s", namespacedKey)

		// Reclaim stale entries from inflight sorted set
		staleThreshold := float64(time.Now().Unix() - int64(h.staleThresholdSeconds))
		staleMembers, err := h.queueRedisClient.ZRangeByScore(ctx, inflightNamespacedKey, &redis.ZRangeBy{
			Min: "-inf",
			Max: strconv.FormatFloat(staleThreshold, 'f', -1, 64),
		}).Result()
		if err != nil {
			...
			handlerLogger.Error(err, "unable to query stale entries")
			return admission.Errored(http.StatusInternalServerError, err)
		}
		...
		if len(staleMembers) > 0 {
			...
			for _, member := range staleMembers {
				pipeline.ZRem(ctx, inflightNamespacedKey, member)
				pipeline.RPush(ctx, queueNamespacedKey, member)
			}
			if _, err := pipeline.Exec(ctx); err != nil {
				...
				handlerLogger.Error(err, "unable to reclaim stale entries")
				return admission.Errored(http.StatusInternalServerError, err)
			}
		}

		queue := h.queueRedisClient.LLen(ctx, queueNamespacedKey).Val()
		inflight := h.queueRedisClient.ZCard(ctx, inflightNamespacedKey).Val()
		...
		if diff > 0 {
			pipeline := h.queueRedisClient.Pipeline()
			for i := 0; i < diff; i++ {
				if _, err := pipeline.RPush(ctx, queueNamespacedKey, uuid.New().String()).Result(); err != nil {
					if err := mutex.Unlock(ctx); err != nil {
						handlerLogger.Error(err, "unable to unlock")
						return admission.Errored(http.StatusInternalServerError, err)
					}
		...
		if _, err := h.queueRedisClient.ZAdd(ctx, inflightNamespacedKey, redis.Z{
