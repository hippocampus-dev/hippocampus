...
async def override_server_error_middleware(request: fastapi.Request, call_next):
    ...
    except lancer.exceptions.RetryableError as e:
        lancer.telemetry.logger.error(e)
        return fastapi.responses.PlainTextResponse(
            http.HTTPStatus.SERVICE_UNAVAILABLE.phrase,
            status_code=http.HTTPStatus.SERVICE_UNAVAILABLE,
        )
    except Exception as e:
        lancer.telemetry.logger.error(e, exc_info=True)
        return fastapi.responses.PlainTextResponse(
...
global_datastore: lancer.datastore.DataStore | None = None


async def get_datastore() -> lancer.datastore.DataStore:
    global global_datastore

    if not global_datastore:
        from lancer.datastore.lancedb import LanceDBDataStore

        global_datastore = LanceDBDataStore(
            s.default_chunk_size,
            s.embedding_batch_size,
            s.embedding_model,
            tiktoken.get_encoding("cl100k_base"),
            s.entity_similarity_threshold,
            s.lancedb_path,
        )
...
@app.post("/upsert", operation_id="upsert_knowledge")
async def upsert(
    body: lancer.model.UpsertRequest,
    datastore: lancer.datastore.DataStore = fastapi.Depends(get_datastore),
) -> lancer.model.UpsertResponse:
    return await datastore.upsert(body)


@app.post("/query", operation_id="query_knowledge")
async def query(
    body: lancer.model.QueryRequest,
    datastore: lancer.datastore.DataStore = fastapi.Depends(get_datastore),
) -> lancer.model.QueryResponse:
    return await datastore.query(body)


@app.delete("/delete", operation_id="delete_knowledge")
async def delete(
    body: lancer.model.DeleteRequest,
    datastore: lancer.datastore.DataStore = fastapi.Depends(get_datastore),
) -> lancer.model.DeleteResponse:
    success = await datastore.delete(body.filter)
    return lancer.model.DeleteResponse(success=success)
