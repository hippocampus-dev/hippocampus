...
func main() {
	...
	flag.BoolVar(&enableUserNamespace, "enable-user-namespace", envOrDefaultValue("ENABLE_USER_NAMESPACE", false), "Enable user namespace for runner pods (requires Kubernetes 1.33+ and containerd 2.0+)")
	opts := zap.Options{}
	...
	if enableUserNamespace {
		entrypointLogger.Info("user namespace isolation enabled for runner pods")
	}
	...
	if err := (&controllers.RunnerReconciler{
		...
		EnableUserNamespace:     enableUserNamespace,
