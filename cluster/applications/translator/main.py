    ...
    with translator.telemetry.tracer.start_as_current_span(
        ...
        match s.brain_type:
            case translator.settings.BrainType.S3:
                ...
                await translator.slack.customize.register(
...
def create_channel_scoped_api_client_factory(
    ...
    async def factory(channel: str) -> slack_sdk.web.async_client.AsyncWebClient | None:
        ...
        if conversations_join_cache.get(channel) is None:
            response = await retryable_client.conversations_join(channel=channel)
            conversations_join_cache[channel] = response is not None
...
_EMOJI_REGEXP = re.compile(r"^:[^:\s]+:$")


async def handle_event_with_retry(
    upsert: typing.Callable[
        ..., typing.Awaitable[slack_sdk.web.async_slack_response.AsyncSlackResponse]
    ],
    ...
    async def retry(attempts: int, next_delay: int, max_delay: int | None = None):
        ...
        try:
            ...
            i += 1
            return await retry(attempts, next_delay, max_delay=max_delay)
...
async def handle_event(
    ...
    if _EMOJI_REGEXP.fullmatch(text):
        return
    ...
    if brain is not None:
        ...
        translations = (
            translator.slack.customize.Translations.model_validate_json(
    ...
    try:
            ...
            list(
                {
                    ...
                    for result in response_format.results
        ...
        for i, result in enumerate(response_format.results):
            ...
            if (
                translation.collapse == translator.slack.customize.Toggle.On
                or len(transformed_message) > 3000
            ):
...
async def handle_message(
    ...
    with translator.telemetry.tracer.start_as_current_span(
        ...
        retryable_client = translator.slack.RetryableAsyncWebClient(client)
        upsert = (
            retryable_client.chat_postMessage
        )  # Special Tier (1 per second per channel)
        ...
        await handle_event_with_retry(upsert, event, context)
...
async def handle_member_joined_channel_events(
    ...
    if event.get("user", event.get("username")) == s.slack_bot_member_id:
        await say("Hi, I'm Translator. You can configure me by `/translation`.")
