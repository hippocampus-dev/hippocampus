...
class RedisCircuitBreaker:
    ...
    def __init__(
        ...
        self.key_latencies = f"circuit_breaker:{name}:latencies"
        self.key_slow_calls = f"circuit_breaker:{name}:slow_calls"
    ...
    async def call(self) -> collections.abc.AsyncGenerator[None, None]:
        ...
        try:
            ...
            try:
                await self._on_success(time.time() - started_at)
            except Exception:
                pass
        finally:
            if state is State.HALF_OPEN:
    ...
    async def _on_success(self, latency_seconds: float) -> None:
        now = time.time()
        window_min = now - self.evaluation_window_seconds

        pipe = self.client.pipeline(transaction=False)
        pipe.zrangebyscore(self.key_latencies, window_min, "+inf")
        pipe.zremrangebyscore(self.key_slow_calls, "-inf", window_min)
        pipe.zcard(self.key_slow_calls)
        ...
        pipe = self.client.pipeline(transaction=False)
        pipe.zadd(self.key_latencies, {member: now})
        pipe.zremrangebyscore(self.key_latencies, "-inf", window_min)
        pipe.zremrangebyrank(self.key_latencies, 0, -_MAX_LATENCY_SAMPLES - 1)
        pipe.expire(self.key_latencies, self.evaluation_window_seconds)
        if is_slow:
            pipe.zadd(self.key_slow_calls, {uuid.uuid4().hex: now})
            pipe.expire(self.key_slow_calls, self.evaluation_window_seconds)
            slow_count += 1
        await pipe.execute()

        total_count = len(latencies) + 1
        if slow_count / total_count >= self.latency_rate_threshold:
