...
- The Dockerfile uses a multi-stage build with Python 3.14 and includes all necessary system dependencies
...
5. **api/sse_buffer.py** - Redis Streams-backed SSE buffering:
   - `SSEBuffer.open(stream_id)` - Creates stream metadata in Redis
   - `SSEBuffer.produce(stream_id)` - Context manager for producers that handles completion/error lifecycle
   - `SSEBuffer.append(stream_id, data)` - Appends an event to the stream
   - `SSEBuffer.events(stream_id, after_id)` - Async generator for SSE consumers
...
6. **Stream Buffering**: Uses Redis Streams to decouple background task producers from SSE consumers. `SSEBuffer.open()` creates metadata before the background task starts to prevent race conditions. All state-mutating Redis operations use non-transactional pipelines to batch commands and reduce round-trips. On Redis failure during completion marking, `produce()` falls back to `_mark_error(retryable=True)`. Consumers receive error status as SSE events (`retryable_error` or `error`) rather than exceptions, allowing graceful client-side handling.
...
- `MODEL` - Override default model (gpt-5.4)
- `SSE_BUFFER_TTL_SECONDS` - TTL for Redis SSE stream buffers (default: 300)
