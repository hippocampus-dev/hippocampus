...
6. **Stream Buffering**: Uses Redis Streams to decouple background task producers from SSE consumers. `SSEBuffer.open()` creates metadata before the background task starts to prevent race conditions. All state-mutating Redis operations use non-transactional pipelines to batch commands and reduce round-trips.
