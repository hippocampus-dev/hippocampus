...
def _track_background_task(coro: collections.abc.Coroutine) -> None:
...
global_sse_buffer: api.sse_buffer.SSEBuffer | None = None


def get_sse_buffer() -> api.sse_buffer.SSEBuffer:
    global global_sse_buffer
    if global_sse_buffer is None:
        global_sse_buffer = api.sse_buffer.SSEBuffer(
            redis.asyncio.Redis(
                host=s.redis_host,
                port=s.redis_port,
            ...
            ttl_seconds=s.sse_buffer_ttl_seconds,
        )
    return global_sse_buffer


global_circuit_breaker_requests_total: opentelemetry.metrics.Counter | None = None


def get_circuit_breaker_requests_total() -> opentelemetry.metrics.Counter:
    global global_circuit_breaker_requests_total
    if global_circuit_breaker_requests_total is None:
        global_circuit_breaker_requests_total = api.telemetry.meter.create_counter(
            "circuit_breaker_requests_total",
            description="Total number of requests through the circuit breaker",
        )
    return global_circuit_breaker_requests_total


global_circuit_breaker_open: opentelemetry.metrics.UpDownCounter | None = None


def get_circuit_breaker_open() -> opentelemetry.metrics.UpDownCounter:
    global global_circuit_breaker_open
    if global_circuit_breaker_open is None:
        global_circuit_breaker_open = api.telemetry.meter.create_up_down_counter(
            "circuit_breaker_open",
            description="Whether the circuit breaker is currently open",
        )
    return global_circuit_breaker_open


global_flex_circuit_breaker: api.circuit_breaker.RedisCircuitBreaker | None = None


def get_flex_circuit_breaker() -> api.circuit_breaker.RedisCircuitBreaker:
    global global_flex_circuit_breaker
    if global_flex_circuit_breaker is None:
        global_flex_circuit_breaker = api.circuit_breaker.RedisCircuitBreaker(
            redis.asyncio.Redis(
                host=s.redis_host,
                port=s.redis_port,
                retry=redis.asyncio.retry.Retry(redis.backoff.ExponentialBackoff(), 3),
            ...
            "flex",
            failure_threshold=s.flex_circuit_breaker_failure_threshold,
            reset_timeout_seconds=s.flex_circuit_breaker_reset_timeout_seconds,
            evaluation_window_seconds=s.flex_circuit_breaker_evaluation_window_seconds,
        )
    return global_flex_circuit_breaker
...
TypedContent = TextTypedContent | ImageUrlTypedContent | AudioTypedContent
...
class Message(pydantic.BaseModel):
    role: str
    content: str | collections.abc.Sequence[TypedContent] | None = None
    tool_calls: collections.abc.Sequence[ToolCall] | None = None
...
class V1ChatCompletions(pydantic.BaseModel):
    ...
    reasoning_effort: openai.types.chat.ChatCompletionReasoningEffort = pydantic.Field(
        "medium",
    ...
    verbosity: typing.Literal["low", "medium", "high"] = pydantic.Field(
        "medium",
    ...
    tools: (
        collections.abc.Sequence[openai.types.chat.ChatCompletionToolParam] | None
    ) = pydantic.Field(
        None,
...
class ResponsesInputMessage(pydantic.BaseModel):
    type: typing.Literal["message"] = "message"
    role: typing.Literal["user", "system", "developer"]
    content: str | collections.abc.Sequence[ResponsesInputContent]


class ResponsesFunctionCallOutput(pydantic.BaseModel):
    type: typing.Literal["function_call_output"]
    call_id: str
    output: str


class ResponsesOutputTextContent(pydantic.BaseModel):
    type: typing.Literal["output_text"]
    text: str
    annotations: collections.abc.Sequence[typing.Any] | None = None


ResponsesOutputContent = ResponsesOutputTextContent


class ResponsesOutputMessage(pydantic.BaseModel):
    type: typing.Literal["message"] = "message"
    role: typing.Literal["assistant"]
    content: collections.abc.Sequence[ResponsesOutputContent]
    id: str | None = None
    status: typing.Literal["in_progress", "completed", "incomplete"] | None = None


class ResponsesReasoningSummary(pydantic.BaseModel):
    type: typing.Literal["summary_text"]
    text: str


class ResponsesReasoningContent(pydantic.BaseModel):
    type: typing.Literal["reasoning_text"]
    text: str


class ResponsesReasoningItem(pydantic.BaseModel):
    type: typing.Literal["reasoning"]
    summary: collections.abc.Sequence[ResponsesReasoningSummary] | None = None
    content: collections.abc.Sequence[ResponsesReasoningContent] | None = None
    encrypted_content: str | None = None
    id: str | None = None
    status: typing.Literal["in_progress", "completed", "incomplete"] | None = None


class ResponsesFunctionCall(pydantic.BaseModel):
    type: typing.Literal["function_call"]
    name: str
    arguments: str
    call_id: str
    id: str | None = None
...
class V1Responses(pydantic.BaseModel):
    input: collections.abc.Sequence[ResponsesInputItem] = pydantic.Field(
        ...,
        description="Text, image, or file inputs to the model, used to generate a response.",
    )
    model: cortex.llm.openai.model.CompletionModel | None = pydantic.Field(
        None,
        description="Model ID used to generate the response, like gpt-5.4 or o3.",
    )
    ...
    reasoning: openai.types.shared_params.Reasoning | None = pydantic.Field(
        None,
        description="Configuration for reasoning models. Set reasoning.effort to control how much effort the model spends on reasoning (low, medium, high). Set reasoning.summary to control the summary generation (auto, concise, detailed).",
    )
    text: openai.types.responses.ResponseTextConfigParam | None = pydantic.Field(
        None,
        description="Configuration for text output. Set text.verbosity to control the verbosity of the response (low, medium, high). Set text.format for structured output.",
    )
    tools: collections.abc.Sequence[openai.types.responses.ToolParam] | None = (
        pydantic.Field(
            None,
...
async def completions(
    ...
    buffer = get_sse_buffer()
    last_event_id = request.headers.get("last-event-id")

    if last_event_id is not None and body.stream:
        try:
            stream_id, secret, entry_id = api.sse_buffer.decode_event_id(last_event_id)
        except ValueError:
            return fastapi.responses.JSONResponse(
        ...
        status = await buffer.get_status(stream_id)
        if status is None:
            return fastapi.responses.JSONResponse(
                {"error": {"message": "Gone", "type": "invalid_request_error"}},
                status_code=410,
            )

        rate_limiter = await get_rate_limiter()
        rate_limiter_key = x_auth_request_user
        rate_limit_info = await rate_limiter.remaining(
            rate_limiter_key, s.rate_limit_per_interval
        )

        return sse_starlette.sse.EventSourceResponse(
            buffer.events(stream_id, secret, entry_id),
    ...
    try:
        if cortex_mode:
            ...
            if body.stream:
                ...
                secret = await buffer.open(stream_id)

                async def _consume_cortex_stream() -> None:
                    try:
                        async with buffer.produce(stream_id, secret):
                            ...
                            queue = {
                                asyncio.create_task(task.__anext__()): task
                                for task in tasks
                            }
                            ...
                            while queue:
                                ...
                                for future in done:
                                    ...
                                    try:
                                        await buffer.append(
                                            stream_id, secret, await future
                                        )
                                        queue[asyncio.create_task(task.__anext__())] = (
                                            task
                                        )
                                    except StopAsyncIteration:
                                        continue

                            await buffer.append(stream_id, secret, "[DONE]")
                            await count_token(context, rate_limiter, rate_limiter_key)
                    except Exception as e:
                        api.telemetry.logger.error(e, exc_info=True)

                _track_background_task(_consume_cortex_stream())
                return sse_starlette.sse.EventSourceResponse(
                    buffer.events(stream_id, secret),
                    headers=rate_limit_headers(rate_limit_info),
                )
            else:
                response = None
            ...
            async def _create_chat_completion(
                ...
                return await cortex.llm.openai.AsyncOpenAI(
                    ...
                    service_tier=service_tier if service_tier is not None else openai._types.NOT_GIVEN,
                    stream=body.stream,
                )

            if body.service_tier == "flex":
                try:
                    async with get_flex_circuit_breaker().call():
                        try:
                            response = await _create_chat_completion("flex")
                        except (
                                openai.InternalServerError,
                                openai.APIConnectionError,
                                openai.RateLimitError,
                        ) as e:
                            raise api.circuit_breaker.CircuitBreakerFailure() from e
                    get_circuit_breaker_requests_total().add(
                        1, {"name": "flex", "result": "success"}
                    )
                except api.circuit_breaker.CircuitBreakerOpen:
                    get_circuit_breaker_requests_total().add(
                        1, {"name": "flex", "result": "rejected"}
                    )
                    get_circuit_breaker_open().add(1, {"name": "flex"})
                    response = await _create_chat_completion(
                        s.flex_circuit_breaker_fallback_tier
                    )
                except api.circuit_breaker.CircuitBreakerFailure as e:
                    get_circuit_breaker_requests_total().add(
                        1, {"name": "flex", "result": "failure"}
                    )
                    raise e.__cause__ from e.__cause__
            else:
                response = await _create_chat_completion(body.service_tier)
            ...
            if body.stream:
                        ...
                        async with buffer.produce(stream_id, secret):
                            try:
                                async for r in response:
                                    ...
                                    await buffer.append(stream_id, secret, r.json())
                            except (
                                    httpx.RemoteProtocolError,
                                    httpx.ReadTimeout,
                                    openai.APIConnectionError,
                ...
                _track_background_task(_consume_chat_stream())
                return sse_starlette.sse.EventSourceResponse(
                    buffer.events(stream_id, secret),
                    headers=rate_limit_headers(rate_limit_info),
                )
...
async def realtime(
    ...
    model: str = fastapi.Query(default="gpt-realtime-mini-2025-12-15"),
    x_auth_request_user: str = fastapi.Header(default="anonymous"),
):
    context = APIContext(
        context_id=uuid.uuid4().hex,
...
class V1ImagesGenerations(pydantic.BaseModel):
    ...
    background: typing.Literal["transparent", "opaque", "auto"] | None = pydantic.Field(
        None,
        description="Allows to set transparency for the background of the generated image(s). This parameter is only supported for gpt-image-1 and gpt-image-1.5. Must be one of transparent, opaque or auto (default value). When auto is used, the model will automatically determine the best background for the image. If transparent, the output format needs to support transparency, so it should be set to either png (default value) or webp.",
    )
    model: typing.Literal["dall-e-2", "dall-e-3", "gpt-image-1", "gpt-image-1.5"] = (
        pydantic.Field(
            "dall-e-2",
    ...
    moderation: typing.Literal["low", "auto"] | None = pydantic.Field(
        None,
        description="Control the content-moderation level for images generated by gpt-image-1 and gpt-image-1.5. Must be either low for less restrictive filtering or auto (default value).",
    )
    n: int = pydantic.Field(
        1,
    ...
    output_compression: int | None = pydantic.Field(
        ...
        ge=0,
        le=100,
        description="The compression level (0-100%) for the generated images. This parameter is only supported for gpt-image-1 and gpt-image-1.5 with the webp or jpeg output formats, and defaults to 100.",
    )
    output_format: typing.Literal["png", "jpeg", "webp"] | None = pydantic.Field(
        None,
        description="The format in which the generated images are returned. This parameter is only supported for gpt-image-1 and gpt-image-1.5. Must be one of png, jpeg, or webp.",
    )
    quality: typing.Literal["standard", "hd", "high", "medium", "low", "auto"] = (
        pydantic.Field(
            "auto",
            description="The quality of the image that will be generated. auto (default value) will automatically select the best quality for the given model. hd and standard are supported for dall-e-3. high, medium and low are supported for gpt-image-1 and gpt-image-1.5. standard is the only option for dall-e-2.",
        )
    )
    response_format: typing.Literal["url", "b64_json"] | None = pydantic.Field(
        None,
        description="The format in which generated images with dall-e-2 and dall-e-3 are returned. Must be one of url or b64_json. URLs are only valid for 60 minutes after the image has been generated. This parameter isn't supported for gpt-image-1 and gpt-image-1.5 which will always return base64-encoded images.",
    )
    size: typing.Literal[
        "256x256",
        "512x512",
        ...
        "1024x1024",
        description="The size of the generated images. Must be one of 1024x1024, 1536x1024 (landscape), 1024x1536 (portrait), or auto (default value) for gpt-image-1 and gpt-image-1.5, one of 256x256, 512x512, or 1024x1024 for dall-e-2, and one of 1024x1024, 1792x1024, or 1024x1792 for dall-e-3.",
    )
...
async def generations(
    ...
    elif body.model == "gpt-image-1.5":
        image_model = cortex.llm.openai.model.ImageModel(f"{quality}15-{size}")
    else:
    ...
    if body.model == "gpt-image-1.5":
        context.increment_prompt_tokens(
            cortex.llm.openai.model.CompletionModel.GPT_IMAGE_1_5,
            response.usage["input_tokens"],
        )
        context.increment_completion_tokens(
            cortex.llm.openai.model.CompletionModel.GPT_IMAGE_1_5,
...
async def responses(
    ...
    try:
        ...
        async def _create_response(service_tier: str | None):
            return await cortex.llm.openai.AsyncOpenAI(
                http_client=httpx.AsyncClient(
                    timeout=None,
                    mounts={
                ...
                model=model,
                input=[item.model_dump(exclude_none=True) for item in body.input],
                ...
                text=body.text if body.text is not None else openai._types.NOT_GIVEN,
                tools=body.tools if body.tools is not None else openai._types.NOT_GIVEN,
                tool_choice=body.tool_choice
                if body.tool_choice is not None
                else openai._types.NOT_GIVEN,
                parallel_tool_calls=body.parallel_tool_calls
        ...
        if body.service_tier == "flex":
            try:
                async with get_flex_circuit_breaker().call():
                    try:
                        response = await _create_response("flex")
                    except (
                            openai.InternalServerError,
                            openai.APIConnectionError,
                            openai.RateLimitError,
                ...
                get_circuit_breaker_open().add(1, {"name": "flex"})
                response = await _create_response(s.flex_circuit_breaker_fallback_tier)
            except api.circuit_breaker.CircuitBreakerFailure as e:
                get_circuit_breaker_requests_total().add(
                    1, {"name": "flex", "result": "failure"}
                )
            ...
            response = await _create_response(body.service_tier)

        if body.stream:
            stream_id = uuid.uuid4().hex
            secret = await buffer.open(stream_id)

            async def _consume_responses_stream() -> None:
                try:
                    async with buffer.produce(stream_id, secret):
                        try:
            ...
            _track_background_task(_consume_responses_stream())
            return sse_starlette.sse.EventSourceResponse(
                buffer.events(stream_id, secret),
                headers=rate_limit_headers(rate_limit_info),
            )
        else:
            context.increment_completion_tokens(model, response.usage.output_tokens)
...
async def messages(
    ...
    try:
        ...
        openai_messages = api.anthropic.adapters.convert_anthropic_messages_to_openai(
            body
        )
        openai_tools = api.anthropic.adapters.convert_anthropic_tools_to_openai(
            body.tools
        )
        openai_tool_choice = (
            api.anthropic.adapters.convert_anthropic_tool_choice_to_openai(
                body.tool_choice
            )
        )
        ...
        response = await cortex.llm.openai.AsyncOpenAI(
            ...
            top_p=body.top_p if body.top_p is not None else openai._types.NOT_GIVEN,
        ...
        if body.stream:
            ...
            async def _consume_messages_stream() -> None:
                try:
                    async with buffer.produce(stream_id, secret):
                        adapter = api.anthropic.adapters.StreamingAdapter(
                            model=str(model),
                        )
                        ...
                        await buffer.append(
                            stream_id,
                            secret,
                            adapter.create_message_start_event(
                                input_tokens=sum(context.prompt_tokens.values())
                            ).model_dump_json(),
                            event="message_start",
                        )
                        ...
                        try:
                            async for chunk in response:
                                for event in adapter.process_openai_chunk(chunk):
                                    await buffer.append(
                                        stream_id,
                                        secret,
                                        event.model_dump_json(),
                                    ...
                                    if isinstance(
                                        ...
                                        if isinstance(
                                            event.delta, api.anthropic.model.TextDelta
                                        ):
                                            context.increment_completion_tokens(
                                                model,
                                                len(
                                    ...
                                    if isinstance(
                                        event, api.anthropic.model.MessageStopEvent
                                    ):
                                        await count_token(
                                            context, rate_limiter, rate_limiter_key
                                        )
            ...
            _track_background_task(_consume_messages_stream())
            return sse_starlette.sse.EventSourceResponse(
                buffer.events(stream_id, secret),
                headers=rate_limit_headers(rate_limit_info),
            )
            ...
            anthropic_response = (
                api.anthropic.adapters.convert_openai_response_to_anthropic(
                    response, str(model)
                )
            )

            context.increment_completion_tokens(
                model, response.usage.completion_tokens if response.usage else 0
            )
