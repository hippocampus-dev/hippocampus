...
def get_flex_circuit_breaker() -> api.circuit_breaker.RedisCircuitBreaker:
    ...
    if global_flex_circuit_breaker is None:
        global_flex_circuit_breaker = api.circuit_breaker.RedisCircuitBreaker(
            ...
            latency_rate_threshold=s.flex_circuit_breaker_latency_rate_threshold,
            latency_z_score_threshold=s.flex_circuit_breaker_latency_z_score_threshold,
            latency_min_samples=s.flex_circuit_breaker_latency_min_samples,
        )
        ...
        if cortex_mode:
            ...
            async def _create_chat_completion(
                ...
                return await cortex.llm.openai.AsyncOpenAI(
                    ...
                    reasoning_effort=body.reasoning_effort
                    if body.reasoning_effort is not None and model.reasoning_supported
            ...
            if body.service_tier == "flex":
                try:
                    async with get_flex_circuit_breaker().call(
                        segment=f"{model}:{body.reasoning_effort if body.reasoning_effort is not None else 'unspecified'}",
                    ):
                        try:
...
async def realtime(
    ...
    model: str = fastapi.Query(default="gpt-realtime-mini"),
    x_auth_request_user: str = fastapi.Header(default="anonymous"),
):
    context = APIContext(
        context_id=uuid.uuid4().hex,
...
async def generations(
    ...
    try:
        async with api.cancellation.cancel_on_disconnect(request):
...
async def responses(
    ...
    try:
        ...
        if body.service_tier == "flex":
            try:
                async with get_flex_circuit_breaker().call(
                    segment=f"{model}:{body.reasoning.effort if body.reasoning is not None else 'unspecified'}",
                ):
                    try:
