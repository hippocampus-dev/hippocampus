...
async def completions(
    ...
    if last_event_id is not None and body.stream:
        try:
            stream_id, entry_id = api.sse_buffer.decode_event_id(last_event_id)
        except ValueError:
            return fastapi.responses.JSONResponse(
        ...
        return sse_starlette.sse.EventSourceResponse(
            buffer.events(stream_id, entry_id),
    ...
    try:
        if cortex_mode:
            ...
            if body.stream:
                ...
                await buffer.open(stream_id)

                async def _consume_cortex_stream() -> None:
                    try:
                        async with buffer.produce(stream_id):
                            ...
                            while queue:
                                ...
                                for future in done:
                                    ...
                                    try:
                                        await buffer.append(stream_id, await future)
                            ...
                            await buffer.append(stream_id, "[DONE]")
                        ...
                        async with buffer.produce(stream_id):
                            try:
                                async for r in response:
                                    ...
                                    await buffer.append(stream_id, r.json())
                            except (
                                httpx.RemoteProtocolError,
                                httpx.ReadTimeout,
                                openai.APIConnectionError,
                    ...
                    async with buffer.produce(stream_id):
                        try:
                            async for event in response:
                                ...
                                await buffer.append(
                                    stream_id, event.model_dump_json(), event=event.type
                                )
                        except (
                            httpx.RemoteProtocolError,
                            httpx.ReadTimeout,
