...
async def messages(
    ...
    try:
        ...
        if body.stream:
            ...
            async def sse(
                response,
            ) -> collections.abc.AsyncIterable[dict[str, typing.Any]]:
                ...
                yield {
                    "event": "message_start",
                    "data": adapter.create_message_start_event(
                ...
                try:
                    async for chunk in response:
                        for event in adapter.process_openai_chunk(chunk):
                            ...
                            if isinstance(
                                event, api.anthropic.model.ContentBlockDeltaEvent
                            ):
                                if isinstance(
                                    ...
                                    context.increment_completion_tokens(
                                        model,
                                        len(context.encoder.encode(event.delta.text)),
                                    )

                            if isinstance(event, api.anthropic.model.MessageStopEvent):
                                await count_token(
