...
GitHub Actions Exporter is a Prometheus exporter for GitHub Actions workflow metrics. It enables HPA-based autoscaling of GitHub Actions self-hosted runners by exposing the count of queued workflow runs and self-hosted runner status as Prometheus metrics.
...
2. **GitHub Client**: Queries GitHub API for queued workflow runs and self-hosted runner status on each `/metrics` request
...
- **Runner Status Tracking**: Fetches all self-hosted runners with pagination and categorizes them as online/busy, online/idle, or offline
...
# Self-hosted runner status (labels = comma-separated custom runner labels, excluding read-only defaults)
github_actions_runners{owner="kaidotio", labels="github-actions-runner-controller", busy="true", status="online"} 2
github_actions_runners{owner="kaidotio", labels="github-actions-runner-controller", busy="false", status="online"} 3
github_actions_runners{owner="kaidotio", labels="local", busy="true", status="online"} 1
...
| Organization | `GET /orgs/{org}/actions/runners?per_page=100&page={page}` |
| Repository | `GET /repos/{owner}/{repo}/actions/runs?status=queued&per_page=100` |
| Repository | `GET /repos/{owner}/{repo}/actions/runners?per_page=100&page={page}` |
