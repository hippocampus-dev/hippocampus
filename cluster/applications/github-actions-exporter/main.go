...
type RunnerCountKey struct {
	Labels string
	Status string
	Busy   string
}

type RunnerCounts map[RunnerCountKey]int

func (c *GitHubClient) GetRunners(ctx context.Context) (RunnerCounts, *RateLimitInfo, error) {
	token, err := c.tokenProvider.GetToken(ctx)
	if err != nil {
		return nil, nil, xerrors.Errorf("failed to get token: %w", err)
	}

	var baseURL string
	if c.repo == "" {
		baseURL = fmt.Sprintf("https://api.github.com/orgs/%s/actions/runners", c.owner)
	} else {
		baseURL = fmt.Sprintf("https://api.github.com/repos/%s/%s/actions/runners", c.owner, c.repo)
	}
	...
	for page <= 50 {
		...
		request, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
		if err != nil {
			return nil, rateLimitInfo, xerrors.Errorf("failed to create request: %w", err)
		}
		...
		response, err := http.DefaultClient.Do(request)
		if err != nil {
			return nil, rateLimitInfo, xerrors.Errorf("failed to do request: %w", err)
		}
		...
		if response.StatusCode >= 400 {
			responseBody, _ := io.ReadAll(response.Body)
			_ = response.Body.Close()
			return nil, rateLimitInfo, xerrors.Errorf("failed to get runners: status=%d, body=%s", response.StatusCode, string(responseBody))
		}

		var runnersResponse RunnersResponse
		if err := json.NewDecoder(response.Body).Decode(&runnersResponse); err != nil {
			_ = response.Body.Close()
			return nil, rateLimitInfo, xerrors.Errorf("failed to decode response: %w", err)
		}
		_ = response.Body.Close()

		for _, runner := range runnersResponse.Runners {
		...
		if len(runnersResponse.Runners) < 100 {
			break
		}
		page++
	}

	return counts, rateLimitInfo, nil
}
...
func main() {
	...
	mux.HandleFunc("GET /metrics", func(w http.ResponseWriter, r *http.Request) {
		count, runsRateLimit, err := client.GetQueuedRunsCount(r.Context())
		if err != nil {
		...
		runnerCounts, runnersRateLimit, err := client.GetRunners(r.Context())
		if err != nil {
		...
		var runnersMetrics []*dto.Metric
		for key, count := range runnerCounts {
			labels := []*dto.LabelPair{
				{Name: p("busy"), Value: p(key.Busy)},
				{Name: p("labels"), Value: p(key.Labels)},
				{Name: p("owner"), Value: p(owner)},
				{Name: p("status"), Value: p(key.Status)},
			}
			if repo != "" {
				labels = append(labels, &dto.LabelPair{Name: p("repo"), Value: p(repo)})
			}
			runnersMetrics = append(runnersMetrics, &dto.Metric{
				Label: labels,
		...
		families := []*dto.MetricFamily{
			...
			{
				Name:   p("github_actions_runners"),
				Help:   p("Number of GitHub Actions self-hosted runners"),
				Type:   p(dto.MetricType_GAUGE),
				Metric: runnersMetrics,
			},
		}

		rateLimit := runnersRateLimit
		if rateLimit == nil {
			rateLimit = runsRateLimit
		}
		if rateLimit != nil {
