...
func (p *AppTokenProvider) GetToken(ctx context.Context) (string, error) {
	...
	if p.cachedToken != "" && time.Now().Add(time.Minute).Before(p.expiresAt) {
	...
	accessToken, expiresAt, err := p.getAccessToken(ctx, *jwtToken)
	if err != nil {
		return "", xerrors.Errorf("failed to get access token: %w", err)
	}

	p.cachedToken = accessToken
...
func main() {
	...
	if githubToken != "" {
		...
		log.Fatal("either --github-token/--github-token-file or all GitHub App flags (--github-app-client-id, --github-app-installation-id, --github-app-private-key) are required")
	}
