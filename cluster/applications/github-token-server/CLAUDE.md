...
Path parameter `{profile}` accepts `reader`, `writer`, or `administration-reader`. Profile routes ignore request body permissions; non-profile routes accept permissions in the request body.
