...
func deriveKey(deviceCode string, salt []byte) []byte {
	return pbkdf2.Key([]byte(deviceCode), salt, IterationCount, KeySize, sha256.New)
}

func Encrypt(deviceCode string, data []byte) ([]byte, error) {
	salt := make([]byte, SaltSize)
	if _, err := rand.Read(salt); err != nil {
		return nil, errors.New("failed to generate random salt")
	}

	key := deriveKey(deviceCode, salt)
...
func Decrypt(deviceCode string, encryptedData []byte) ([]byte, error) {
