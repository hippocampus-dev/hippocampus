...
async def upsert(
    request: embedding_retrieval.model.UpsertRequest,
        ...
        try:
            async with aiohttp.ClientSession(trust_env=True) as session:
                async with session.post(
                    ...
                    return embedding_retrieval.model.UpsertResponse.model_validate(
                        await response.json()
                    )
        except (
            ...
            match line:
                ...
                case "_START_PARAGRAPH_":
                    ...
                    if current_section is None:
                        current_section = "要約"
                    articles[current_article][current_section] = paragraph.replace(
                        "_NEWLINE_", "\n"
                    )
                case _:
                    i += 1
