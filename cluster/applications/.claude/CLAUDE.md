...
### Python Applications (14 services)
```bash
...
1. **Kubernetes Controllers & Webhooks** (17 apps)
   ...
   - Controllers: `dedicated-container-ingress-controller`, `gateway-proxy`, `github-actions-runner-controller`, `grafana-manifest-controller`, `nodeport-controller`, `snapshot-controller`, `owasp-zap-controller`
   - Scheduler plugins: `distribution-scheduler`
   - Event loggers: `events-logger`, `cloudevents-logger`

2. **AI/ML Services** (9 apps)
   ...
   - `lancer` - AI memory storage with semantic vector search and graph capabilities via LanceDB
...
4. **Developer Tools** (9 apps)
   - `jupyterhub` - Interactive Python notebooks
   - `bakery` - Build automation
   - `runner` - GitHub Actions runner
   - `k6` - Load testing
   - `playwright-mcp`, `chrome-devtools-mcp` - Browser automation
   - `remotty` - Web terminal and reverse tunnel server for coding agents
   - `spotify-mcp-server` - Spotify track API via MCP
