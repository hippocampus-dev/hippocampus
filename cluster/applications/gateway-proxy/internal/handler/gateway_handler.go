...
func (h *GatewayHandler) isOwnedGateway(ctx context.Context, gateway *gatewayv1.Gateway) (bool, error) {
	gatewayClass := &gatewayv1.GatewayClass{}
	if err := h.Client.Get(ctx, types.NamespacedName{Name: string(gateway.Spec.GatewayClassName)}, gatewayClass); err != nil {
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		return false, err
	}
	return gatewayClass.Spec.ControllerName == h.ControllerName, nil
}
...
func (h *GatewayHandler) handleGateway(ctx context.Context, req admission.Request) admission.Response {
	...
	owned, err := h.isOwnedGateway(ctx, gateway)
	if err != nil {
...
func (h *GatewayHandler) isListenerSetAllowed(ctx context.Context, gateway *gatewayv1.Gateway, ls *gatewayv1.ListenerSet) bool {
	...
	case gatewayv1.NamespacesFromSelector:
		...
		ns := &corev1.Namespace{}
		if err := h.Client.Get(ctx, types.NamespacedName{Name: ls.Namespace}, ns); err != nil {
			return false
		}
...
func (h *GatewayHandler) ownedGatewayClassNames(ctx context.Context) ([]string, error) {
	var gatewayClassList gatewayv1.GatewayClassList
	if err := h.Client.List(ctx, &gatewayClassList); err != nil {
		return nil, err
	}
	var names []string
	for _, gc := range gatewayClassList.Items {
		if gc.Spec.ControllerName == h.ControllerName {
...
func (h *GatewayHandler) collectUsedPorts(ctx context.Context, excludeGatewayName string, excludeGatewayNamespace string, excludeListenerSetName string, excludeListenerSetNamespace string) (map[portProtocolKey]string, error) {
	...
	classNames, err := h.ownedGatewayClassNames(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, gw := range gatewayList.Items {
		...
		for _, ls := range listenerSets.Items {
			...
			if !h.isListenerSetAllowed(ctx, &gw, &ls) {
				continue
			}
			for _, entry := range ls.Spec.Listeners {
				key := portProtocolKey{Port: int32(entry.Port), Protocol: entry.Protocol}
