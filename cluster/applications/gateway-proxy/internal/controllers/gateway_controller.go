...
func (r *GatewayController) updateRouteStatuses(ctx context.Context, gateway *gatewayv1.Gateway, attachedListenerSets []gatewayv1.ListenerSet, conflictedPorts map[portKey]bool, resolved *resolvedRoutes) error {
	...
	for i := range resolved.gatewayTCPRoutes[gatewayKey] {
		...
		statuses := gatewayRouteParentStatuses(ctx, r.Client, gateway, tcpRoute.Spec.ParentRefs, tcpRoute.Namespace, proxy.ProtocolTCP, tcpRoute.Generation, controllerName, conflictedPorts, refsResolved, failureReason, now)
	...
	for i := range resolved.gatewayUDPRoutes[gatewayKey] {
		...
		statuses := gatewayRouteParentStatuses(ctx, r.Client, gateway, udpRoute.Spec.ParentRefs, udpRoute.Namespace, proxy.ProtocolUDP, udpRoute.Generation, controllerName, conflictedPorts, refsResolved, failureReason, now)
	...
	for i := range rejectedListenerSets.Items {
		...
		if err := r.List(ctx, &tcpRouteList, client.MatchingFields{TCPRouteListenerSetIndex: lsKey}); err != nil {
			return err
		}
		for i := range tcpRouteList.Items {
			tcpRoute := &tcpRouteList.Items[i]
			statuses := rejectedListenerSetRouteParentStatuses(ls, tcpRoute.Spec.ParentRefs, tcpRoute.Generation, controllerName, now)
			tcpRoute.Status.Parents = mergeRouteParentStatuses(tcpRoute.Status.Parents, statuses, controllerName)
			if err := r.Status().Update(ctx, tcpRoute); err != nil {
				return err
			}
		...
		if err := r.List(ctx, &udpRouteList, client.MatchingFields{UDPRouteListenerSetIndex: lsKey}); err != nil {
			return err
		}
		for i := range udpRouteList.Items {
			udpRoute := &udpRouteList.Items[i]
			statuses := rejectedListenerSetRouteParentStatuses(ls, udpRoute.Spec.ParentRefs, udpRoute.Generation, controllerName, now)
			udpRoute.Status.Parents = mergeRouteParentStatuses(udpRoute.Status.Parents, statuses, controllerName)
			if err := r.Status().Update(ctx, udpRoute); err != nil {
				return err
			}
...
func rejectedListenerSetRouteParentStatuses(ls *gatewayv1.ListenerSet, parentRefs []gatewayv1.ParentReference, generation int64, controllerName gatewayv1.GatewayController, now metav1.Time) []gatewayv1.RouteParentStatus {
	var statuses []gatewayv1.RouteParentStatus
	for _, ref := range parentRefs {
		if !isListenerSetRef(ref) {
			continue
...
func (r *GatewayController) checkBackendRefs(ctx context.Context, backendRefs []gatewayv1.BackendRef, routeNamespace string, routeKind string) (bool, string) {
	for _, ref := range backendRefs {
		backendReference, failureReason, ok := r.Resolver.backendReferenceFromRef(ctx, ref, routeNamespace, routeKind)
		if !ok {
			return false, failureReason
		}

		service := &corev1.Service{}
		if err := r.Get(ctx, types.NamespacedName{Name: backendReference.Name, Namespace: backendReference.Namespace}, service); err != nil {
			return false, string(gatewayv1.RouteReasonBackendNotFound)
		}
		if !serviceHasPort(service, backendReference.Port) {
			return false, string(gatewayv1.RouteReasonBackendNotFound)
		}
	}
	return true, ""
...
func (r *GatewayController) SetupWithManager(mgr ctrl.Manager) error {
	...
	mapReferenceGrantToGateway := handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []ctrl.Request {
		...
		gateways, err := r.Resolver.listOwnedGateways(ctx)
		if err != nil {
			return nil
		}
