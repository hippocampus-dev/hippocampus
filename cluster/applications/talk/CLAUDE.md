...
### API Layer
- `api/ResponseAuditFilter.js`: Response audit filter framework with:
  - Base `ResponseAuditFilter` class supporting intermediate audit (debounced by text length during `transcript.delta`), early reject before `transcript.done`, retry with policyHint (regeneration request to model), and configurable retry limit (`maxRetries`)
  - `NGWordFilter` subclass for NG word detection
- `api/AudioHandler.js`: Audio recording and PCM conversion
...
5. Response Audit Filter:
   - Audit runs incrementally on `transcript.delta` events (debounced by character count), not only on `transcript.done`
   - On rejection, the filter cancels current audio output and requests the model to regenerate with a policyHint reason
   - Regeneration retries are capped by `maxRetries` (default: 2); after exhaustion, the response is fully rejected
...
3. **Message Protocol**: JSON messages following OpenAI Realtime API (GA) format
4. **Audio Format**: Object format `{ type: "audio/pcm", rate: 24000 }` for both input and output
...
- Audio must be converted from WebM to PCM format before sending
