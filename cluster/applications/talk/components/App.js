...
import { NGWordResponseAuditFilter } from "../api/ResponseAuditFilter.js";
...
const App = () => {
  ...
  const decodeConfig = (configString) => {
    try {
      ...
      return {
        ...
        enableAudit: parsed.enableAudit ?? parsed.ea,
      };
    } catch (error) {
      return null;
    }
  ...
  if (compressedConfig) {
    ...
    if (params.get("temperature")) {
      initialConfig.temperature = parseFloat(params.get("temperature"));
    }
    ...
    if (params.get("enableAudit")) {
      initialConfig.enableAudit = params.get("enableAudit") === "true";
    }
  }
  ...
  const [enableAudit, setEnableAudit] = useState(
    initialConfig.enableAudit ?? false,
  );
  ...
  const connectWebSocket = async (isReconnect = false) => {
    ...
    try {
        ...
        wsRef.current.send(
          JSON.stringify({
            ...
            session: {
              type: "realtime",
              output_modalities:
                outputMode === "audio" ? ["text", "audio"] : ["text"],
      ...
      try {
        ...
        if (enableAudit) {
          auditFilterRef.current = new NGWordResponseAuditFilter(
            auditCallbacks,
          );
        } else {
          ...
          switch (data.type) {
              ...
              if (
                ...
                wsRef.current.send(
                  JSON.stringify({
                    ...
                    response: {
                      instructions:
                        "Reply with ONLY a very brief filler/backchannel. Either a short acknowledgment (1-3 words like 'うん', 'なるほど', 'I see') or a brief echo/paraphrase of what the user said (e.g. '〜ということですね', 'so you mean ...'). Keep it under 10 words. Match the conversation language. Do NOT answer the question.",
                    },
                  }),
                );
                wsRef.current.send(
            ...
            case "response.audio_transcript.done":
              if (data.transcript && currentAssistantMessageIdRef.current) {
                ...
                if (auditFilterRef.current) {
                  auditFilterRef.current.finalize(data.transcript);
                }
              }
              break;
            ...
            case "response.audio.delta":
              if (
                ...
                if (auditFilterRef.current) {
                  auditFilterRef.current.buffer(data.delta);
                } else {
    ...
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      wsRef.current.send(
        JSON.stringify({
          ...
          session: {
            type: "realtime",
            output_modalities: mode === "audio" ? ["text", "audio"] : ["text"],
