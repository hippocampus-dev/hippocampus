...
fn buffered_request_body_mode(
) -> envoy_grpc_ext_proc::envoy::extensions::filters::http::ext_proc::v3::ProcessingMode {
    envoy_grpc_ext_proc::envoy::extensions::filters::http::ext_proc::v3::ProcessingMode {
        request_body_mode:
            envoy_grpc_ext_proc::envoy::extensions::filters::http::ext_proc::v3::processing_mode::BodySendMode::Buffered
                as i32,
        ..Default::default()
    }
}
                ...
                match request.request {
                    Some(envoy_grpc_ext_proc::envoy::service::ext_proc::v3::processing_request::Request::RequestHeaders(headers)) => {
                        ...
                        let content_length = headers.headers.as_ref()
                            .and_then(|h| h.headers.iter().find(|header| header.key == "content-length"))
                            .and_then(|h| header_value(h).parse::<usize>().ok())
                            .unwrap_or(0);
                        let need_body = enable_body_hash && content_length > 0;
                        ...
                        yield envoy_grpc_ext_proc::envoy::service::ext_proc::v3::ProcessingResponse {
                            ...
                            mode_override: if need_body {
                                Some(buffered_request_body_mode())
                            } else {
                                None
                            },
                            ..Default::default()
