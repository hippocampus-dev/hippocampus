...
import (
	...
	"github-actions-exporter-controller/internal/controllers"
	"net/http"
	"os"
	"strconv"
	"time"
...
func init() {
	...
	utilruntime.Must(gaeV1.AddToScheme(scheme))
}
...
func main() {
	...
	flag.StringVar(&exporterImage, "exporter-image", envOrDefaultValue("EXPORTER_IMAGE", "ghcr.io/hippocampus-dev/hippocampus/github-actions-exporter"), "Docker image of the exporter")
	opts := zap.Options{}
	...
	m, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID: "github-actions-exporter-controller",
	})
	if err != nil {
	...
	if err := (&controllers.ExporterReconciler{
		...
		Recorder:                m.GetEventRecorderFor("github-actions-exporter-controller"),
		GitHubAppClientId:       githubAppClientId,
		GitHubAppInstallationId: githubAppInstallationId,
		GitHubAppPrivateKey:     githubAppPrivateKey,
		ExporterImage:           exporterImage,
	}).SetupWithManager(m); err != nil {
		entrypointLogger.Error(err, "unable to create controller", "controller", "Exporter")
		os.Exit(1)
	}
