      ...
      elements.push(
        h(
          "a",
          {
            ...
            target: "_blank",
            class:
              "underline hover:text-orange-800 focus:outline-none focus:text-orange-800",
          },
          part,
        ),
      );
...
const App = ({ files }) => {
  ...
  const URLButton = ({ file, text }) => {
    const classes =
      "flex-grow rounded-md font-semibold focus:outline-none focus:ring-4 focus:ring-orange-600 focus:ring-opacity-100 focus:text-orange-600 focus:bg-white py-4 px-2";
    return h(
      "button",
      {
        onClick: () => {
        ...
        class:
          file === selectedFile
            ? [
                classes,
                "text-orange-600 bg-white ring-4 ring-orange-600 ring-opacity-100 hover:bg-white",
              ].join(" ")
            : [classes, "text-white bg-orange-600 hover:bg-orange-500"].join(
                " ",
              ),
      },
      text,
  ...
  const headerPTR = useMemo(() => {
    const preferredLanguages = new Set(
      navigator.languages.map((language) => language.split("-")[0]),
    );
    const headerPTR = new Map();
    header?.forEach((column, index) => {
      if (files[selectedFile]?.mapping?.[column]?.i18n) {
        let resolvedIndex = index;
        for (const language of preferredLanguages) {
          const localizedIndex = header?.indexOf(`${column}:${language}`);
          if (localizedIndex !== -1) {
  ...
  const storedColumns = useMemo(() => {
    return header?.filter((column) => {
      return files[selectedFile]?.mapping?.[column]?.store;
    });
  }, [header, selectedFile, files]);
  const indexedColumns = useMemo(() => {
    return header?.filter((column) => {
      return files[selectedFile]?.mapping?.[column]?.index;
    });
  }, [header, selectedFile, files]);

  return h(
    "div",
    { class: "flex flex-col items-center bg-gray-100 p-6 space-y-4" },
    [
      h("div", { class: "w-full lg:w-1/2 mt-4" }, [
        h(
          "div",
          { class: "flex flex-row overflow-x-auto mb-4 py-4 px-4 space-x-2" },
              ...
              ?.filter((row) => {
                return terms.every((term) => {
                  return indexedColumns?.some((column) => {
                    ...
                    const maxDistance =
                      files[selectedFile]?.mapping?.[column]?.maxDistance ?? 0;
                    if (
                      maxDistance === 0 ||
                      [...segmenter.segment(term)].length > 64
                    ) {
                ...
                return h(
                  ...
                  storedColumns?.map((column) => {
                    ...
                    return h(
                      ...
                      {
                        class:
                          "text-lg text-orange-600 whitespace-pre-line break-all px-6 py-4",
                      },
                      row[i] ? markdownLinkify(row[i]) : "",
                    );
                  }),
                );
              });
