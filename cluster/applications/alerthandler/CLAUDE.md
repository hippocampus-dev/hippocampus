...
### Dispatcher Pattern
- **Dispatcher**: Central struct that holds dependencies (Kubernetes client, dynamic client, `*github.Client`) and routes alerts
  - `NewDispatcher(kubernetes, dynamicClient, gitHubClient)` - Constructor with dependency injection
...
- **CustomResourceStateStuckHandler**: Handles `KafkaChannelStuck` and `ConsumerGroupStuck` alerts by:
  - Holds `dynamic.Interface` (Kubernetes dynamic client)
  - `NewCustomResourceStateStuckHandler(client, cooldown)` - Constructor with 10-minute cooldown
  - Uses `customresource_group`, `customresource_version`, `customresource_kind`, `namespace`, `name` labels from the alert
  - Patches the affected custom resource with `alerthandler.kaidotio.github.io/restartedAt` annotation
  - Respects cooldown period to prevent repeated patches
...
- Requires in-cluster Kubernetes API access for pod management and custom resource patching (dynamic client)
