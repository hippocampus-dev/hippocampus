...
grafana-manifest-controller is a Kubernetes controller that manages Grafana dashboards as Kubernetes custom resources. It reconciles GrafanaDashboard resources by rendering Jsonnet to JSON and syncing the resulting dashboards to a Grafana instance via its API.
...
- `common.libsonnet` provides shared utilities (Explore links, annotations)

## Architecture

### Core Components

1. **GrafanaDashboard CRD** (`api/v1/dashboard_types.go`)
   - Defines the GrafanaDashboard custom resource
...
2. **Dashboard Controller** (`internal/controllers/dashboard_controller.go`)
   - Reconciles GrafanaDashboard resources
   ...
   - Requeues every 5 minutes for drift detection
...
5. **Drift detection**: Periodic requeue ensures dashboards stay in sync
6. **HA support**: Multi-target mode syncs dashboards to all Grafana pods in stateless HA deployments via DNS-based service discovery
