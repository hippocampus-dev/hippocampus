...
func NewClient(baseURL string, host string) *Client {
	return &Client{
		baseURL:    baseURL,
		host:       host,
...
func (c *Client) UpsertDashboard(ctx context.Context, dashboardJSON []byte, folderUID string) (*DashboardResponse, error) {
	...
	body, err := json.Marshal(dashboardRequest)
	if err != nil {
	...
	if err := json.NewDecoder(response.Body).Decode(&dashboardResponse); err != nil {
...
func (c *Client) SearchFolderByTitle(ctx context.Context, title string) (*Folder, error) {
	query := url.Values{}
	query.Set("type", "dash-folder")
	query.Set("query", title)
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, c.baseURL+"/api/search?"+query.Encode(), nil)
	if err != nil {
		return nil, xerrors.Errorf("failed to create request: %w", err)
	}
...
func (c *Client) EnsureFolder(ctx context.Context, title string) (string, error) {
	...
	folder, err := c.SearchFolderByTitle(ctx, title)
	if err != nil {
		return "", xerrors.Errorf("failed to search folder: %w", err)
	}
	if folder != nil {
		return folder.UID, nil
	}
