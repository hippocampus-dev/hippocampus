...
1. **eBPF Layer** (`src/bpf/unlink.bpf.c`)
   - Kernel-space C program using tracepoints and kprobes
   - Tracepoints on `sys_enter_unlink` and `sys_enter_unlinkat` capture pathname arguments
   - Kprobes on architecture-specific syscall wrappers (`__x64_sys_unlink`, `__arm64_sys_unlink`, etc.) intercept file deletions
   - Uses `bpf_override_return` to block unlink operations (requires syscall wrappers in ALLOW_ERROR_INJECTION whitelist; vfs_unlink is not whitelisted)
...
2. **Rust Application** (`src/main.rs`)
   ...
   - Processes file unlink events from kernel space
   - Handles directories: `remove_file` → `remove_dir` fallback → queue contents and re-queue directory if `DirectoryNotEmpty`
   - Checks Fluentd position file before allowing deletions
   - Implements delay mechanism for file deletions until Fluentd catches up
