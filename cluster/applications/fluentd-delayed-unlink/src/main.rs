        ...
        while let Some(pathname) = unlink_rx.recv().await {
                ...
                .map(|(offset, _)| *offset)
            {
                ...
                if let Ok(metadata) = path.metadata()
                    && metadata.len() != offset
                {
                    // Bound the retry count so that a stale pos file offset cannot trap a path in an infinite re-queue loop.
                    let count = retry_counts.entry(pathname.clone()).or_insert(0);
                    if *count < args.max_retry_count {
                        *count += 1;
            ...
            if let Err(e) = result {
                if e.kind() == std::io::ErrorKind::DirectoryNotEmpty {
                    ...
                    let unlink_tx = unlink_tx.clone();
                    tokio::spawn(async move {
                        tokio::time::sleep(std::time::Duration::from_secs(args.delayed_seconds))
                            .await;
                        if let Err(e) = unlink_tx.send(pathname.clone()) {
                            eprintln!("Failed to re-queue {}: {}", pathname, e);
                        }
