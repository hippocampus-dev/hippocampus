...
### Metadata Filtering
The service supports metadata filtering via `DocumentMetadataFilter` with field-specific operators:

| Field | Operator | Description |
|-------|----------|-------------|
| `source` | exact match | `source="web"` |
| `source_any` | match any | `source_any=["web", "pdf"]` |
| `source_not` | exclude | `source_not=["internal"]` |
| `source_id` | exact match | `source_id="https://..."` |
| `document_id` | exact match | `document_id="doc-1"` |
| `created_at` | range | `created_at=Range(gte=1704067200.0)` |
| `updated_at` | range | `updated_at=Range(lt=1735689600.0)` |

Timestamp fields (`created_at`, `updated_at`) use Unix timestamp (float) and support range filtering with `gte`, `gt`, `lte`, `lt` operators.
...
filter = DocumentMetadataFilter(
    source_any=["web", "pdf"],
    document_id="doc-1",
    created_at=Range(gte=1704067200.0),  # After 2024-01-01
)
```
