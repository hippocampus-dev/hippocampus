...
def _is_range_value(value: object) -> bool:
    if not isinstance(value, dict):
        return False
    return any(key in value for key in ["gte", "gt", "lte", "lt"])
...
def _convert_metadata_filter_to_qdrant_filter(
    ...
    for key, value in metadata_filter.model_dump(exclude_none=True).items():
        if key.endswith("_any"):
            base_key = key.removesuffix("_any")
            must_conditions.append(
                qdrant_client.http.models.FieldCondition(
                    key=f"metadata.{base_key}",
                    match=qdrant_client.http.models.MatchAny(any=list(value)),
                ),
            )
        elif key.endswith("_not"):
            base_key = key.removesuffix("_not")
            must_conditions.append(
                qdrant_client.http.models.FieldCondition(
                    key=f"metadata.{base_key}",
                    match=qdrant_client.http.models.MatchExcept(except_=list(value)),
                ),
            )
        elif _is_range_value(value):
            must_conditions.append(
                qdrant_client.http.models.FieldCondition(
                    key=f"metadata.{key}",
                    range=qdrant_client.http.models.Range(
                        gte=value.get("gte"),
                        gt=value.get("gt"),
                        lte=value.get("lte"),
                        lt=value.get("lt"),
...
class QdrantDataStore(embedding_retrieval.datastore.DataStore, abc.ABC):
    ...
    def __init__(
        ...
        super().__init__(
            default_chunk_size, embedding_batch_size, embedding_model, encoder
        )
    ...
    async def _upsert(
        ...
        document_chunks: collections.abc.Mapping[
            ...
            collections.abc.Sequence[
                embedding_retrieval.datastore.DocumentChunkWithEmbedding
            ],
        ],
        ...
        def run_in_context(ctx: opentelemetry.context.Context):
            ...
            with embedding_retrieval.telemetry.tracer.start_as_current_span(
                "qdrant.upsert"
            ):
    ...
    async def _query(
        self,
        queries: collections.abc.Sequence[
            embedding_retrieval.datastore.QueryWithEmbedding
        ],
            ...
            embedding_retrieval.model.QueryResult(
                ...
                results=[
                    embedding_retrieval.model.DocumentChunkWithScore(
                        id=scored_point.payload.get(
                            "id"
                        ),  # scored_point.id has been converted to uuid format
