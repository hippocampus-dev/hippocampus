...
2. **Sidecar** (`cmd/sidecar.go`, `pkg/sidecar/`)
   ...
   - On SIGTERM:
     - Immediately captures metrics from backend before app terminates
     - Caches the response (body and content-type) for fallback
     - Waits for final scrape if recently scraped (within 1 second)
   - If backend becomes unavailable, serves cached metrics to Prometheus
...
2. The webhook reads the original metrics port from `prometheus.io/port` and path from `prometheus.io/path` (default: /metrics)
3. The `prometheus.io/port` annotation is rewritten to `65532` (the proxy port)
4. The `traffic.sidecar.istio.io/excludeInboundPorts` annotation is updated to include `65532`, excluding the proxy port from Istio sidecar interception
5. A sidecar container is injected with the target URL (e.g., `http://127.0.0.1:9090/metrics`)
6. The sidecar:
   - Listens on port 65532
   - Forwards requests to the original metrics endpoint
   - Records scrape timestamps
   - On SIGTERM:
     - Captures and caches current metrics from backend
     - Serves cached metrics if backend becomes unavailable
     - Waits for a final scrape before shutting down

### Key Annotations

Pods must have these annotations:
- `prometheus.io/wait: "true"` - Enable the metrics proxy
- `prometheus.io/port: "9090"` - Original metrics port (required)
- `prometheus.io/path: "/metrics"` - Original metrics path (optional, default: /metrics)
...
- `--termination-grace-period` - Minimum termination grace period for pods (default: 30s). Set based on Prometheus scrape_interval to ensure at least one scrape during shutdown. The webhook sets `pod.Spec.TerminationGracePeriodSeconds = max(existing, this value)`.
- `--metrics-bind-address` - Metrics endpoint address
- `--health-probe-bind-address` - Health probe endpoint address

#### Sidecar Command
Usage: `sidecar TARGET_URL [flags]`

- `TARGET_URL` - Full URL to proxy metrics from (e.g., `http://127.0.0.1:9090/metrics`)
- `--address` - Address to listen on (default: 0.0.0.0:18080)
- `--termination-grace-period` - Grace period for shutdown (default: 30s)
- `--lameduck` - Period to stop accepting new requests before shutdown (default: 1s)
- `--scrape-wait-threshold` - Wait for next scrape if time since last scrape exceeds this threshold (default: 1s)
