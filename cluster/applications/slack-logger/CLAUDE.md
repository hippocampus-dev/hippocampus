...
## Query Plan Analysis (CI)

The `40_slack-logger-explain.yaml` workflow runs on PRs that modify `queries/`, `schema.hcl`, or `migrations/`. It:
1. Starts a TiDB instance and applies all migrations
2. Loads production table statistics from http-kvs (if available)
3. Runs `EXPLAIN` on all SQLC queries and outputs results to Job Summary

This detects query plan regressions when queries or schema change. Statistics are fetched from `https://http-kvs-external.kaidotio.dev` using GitHub Actions OIDC token for JWT authentication.
...
4. **Changefeed job DNS resolution failure**: TiDB Operator creates only headless peer services (`slack-logger-ticdc-peer`), NOT ClusterIP services. Use the pod FQDN:
   ```
   http://slack-logger-ticdc-0.slack-logger-ticdc-peer.slack-logger.svc.cluster.local:8301
   ```

5. **Changefeed job blocked by NetworkPolicy**: Ensure a NetworkPolicy allows ingress from the changefeed job (label `app.kubernetes.io/component: ticdc-changefeed`) to TiCDC pods on port 8301.
