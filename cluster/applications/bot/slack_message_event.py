...
async def main():
    ...
    response = await slack_sdk.web.async_client.AsyncWebClient(token=s.slack_bot_token).conversations_replies(
        channel=channel,
        ts=ts,
        limit=1,
    )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        request = SlackEvent(
            event=Event(
                type="app_mention" if f"<@{s.slack_bot_member_id}>" in message["text"] else "message",
        ...
        body = json.dumps(request.dict(exclude_none=True), ensure_ascii=False)
