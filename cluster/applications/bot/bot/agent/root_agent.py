...
def boost(
    ...
    for pattern, source_id_boost in _BOOSTS["source_id"].items():
...
def filter(
    ...
    if result.metadata.source is not None:
        for pattern, capability in _REQUIRED_CAPABILITIES["source"].items():
            if isinstance(pattern, str) and pattern == result.metadata.source:
    ...
    if result.metadata.source_id is not None:
        for pattern, capability in _REQUIRED_CAPABILITIES["source_id"].items():
            if isinstance(pattern, str) and pattern == result.metadata.source_id:
                return capability & context.capability > 0
...
class RootAgent(cortex.llm.openai.agent.Agent):
    ...
    def __init__(
        ...
        verbosity: typing.Literal["low", "medium", "high"],
        ...
        self.functions = {
            ...
            "web_search": cortex.llm.openai.agent.FunctionDefinition(
                ...
                if self.google_custom_search_api_key
                and self.google_custom_search_engine_id
    ...
    async def launch_browser_agent(
        ...
        query: str,
        context: cortex.llm.openai.agent.Context,
    ) -> typing.AsyncIterator[openai.types.chat.ChatCompletionChunk]:
        ...
        return self._stream_playwright_agent(query, context)

    async def _stream_playwright_agent(
        self,
        query: str,
        context: cortex.llm.openai.agent.Context,
    ) -> typing.AsyncIterator[openai.types.chat.ChatCompletionChunk]:
