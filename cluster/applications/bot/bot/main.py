    ...
    with bot.telemetry.tracer.start_as_current_span(
        ...
        match s.brain_type:
            case bot.settings.BrainType.S3:
                ...
                await bot.slack.context_manager.register(bolt, cortex.brain.S3Brain(s3_client, s.s3_bucket))
                await bot.slack.customize.register(bolt, cortex.brain.S3Brain(s3_client, s.s3_bucket))
                await bot.slack.reporter.register(bolt, cortex.brain.S3Brain(s3_client, s.s3_bucket))
            case _:
        ...
        match s.rate_limiter_type:
            case cortex.llm.openai.agent.MemoryType.Redis:
                ...
                global_rate_limiter = cortex.rate_limit.RedisSlidingRateLimiter(
                    redis_client,
                    limit=s.rate_limit_per_interval,
...
def calculate_model_price(
    ...
    embedding_tokens: collections.abc.Mapping[cortex.llm.openai.model.EmbeddingModel, int],
    generated_images: collections.abc.Mapping[cortex.llm.openai.model.ImageModel, int],
    processed_audio_seconds: collections.abc.Mapping[cortex.llm.openai.model.AudioModel, float],
    converted_text_characters: collections.abc.Mapping[cortex.llm.openai.model.AudioModel, int],
) -> float:
    price = 0.0
...
async def handle_event(
    ...
    match s.brain_type:
        case bot.settings.BrainType.S3:
            brain = cortex.brain.S3Brain(boto3.client("s3", endpoint_url=s.s3_endpoint), s.s3_bucket)
        case _:
            raise NotImplementedError
    ...
    if brain is not None:
        ...
        avatars = (
            bot.slack.customize.Avatars.parse_raw(restored_avatars)
            if restored_avatars is not None
            else bot.slack.customize.Avatars.parse_obj({})
        )
        avatar = avatars.get(event["channel"])
        if avatar is not None:
            if avatar.locale is not None:
    ...
    for i, message in enumerate(replies["messages"]):
        role = "assistant" if message.get("user") == s.slack_bot_member_id else "user"
    ...
    try:
        ...
        streaming = not s.disable_streaming
        try:
        ...
        if streaming:
            ...
            async def chat_update():
                if len(queue) > 0:
                    fragment_text = bot.slack.transform_message("".join(queue))
                    try:
                        if len(fragment_text.encode("utf-8")) > bot.slack.CHAT_UPDATE_MAX_BYTES:
                            return
            ...
            context.increment_completion_tokens(s.model, len(encoder.encode("".join(queue))))
            ...
            async def immediately_execute_with_backoff_loop(next_delay: int, max_delay: int | None = None):
                try:
                    ...
                    match cause:
                        case slack_sdk.errors.SlackApiError as cause:
                            if cause.response.status_code == 429 and "Retry-After" in cause.response.headers:
                    ...
                    if next_delay <= (2 ** 63 - 1) / 2:
                        ...
                        next_delay = 2 ** 63 - 1
                    await immediately_execute_with_backoff_loop(next_delay=next_delay, max_delay=max_delay)

            text = bot.slack.transform_message("".join(queue))
            if len(text.encode("utf-8")) > bot.slack.CHAT_UPDATE_MAX_BYTES:
                tasks.append(
                    retryable_client.chat_delete(  # Tier 3 (50+ per minute)
                        channel=progress["channel"],
                        ts=progress["ts"],
                    )
                )
                tasks.append(
                    retryable_client.chat_postMessage(  # Special Tier (1 per second per channel)
                        channel=event["channel"],
                        thread_ts=event.get("thread_ts") or event["ts"],
                        text=text,
                        username=avatar.name if avatar is not None else None,
                ...
                if throttled_chat_update.is_throttled:
                    tasks.append(immediately_execute_with_backoff_loop(next_delay=1, max_delay=10))
        else:
            ...
            context.increment_completion_tokens(s.model, response.usage.completion_tokens)

            text = bot.slack.transform_message(choice.message.content)
            if len(text.encode("utf-8")) > bot.slack.CHAT_UPDATE_MAX_BYTES:
                tasks.append(
                    retryable_client.chat_delete(  # Tier 3 (50+ per minute)
                        channel=progress["channel"],
        ...
        if event.get(
            "user",
            event.get("username", ""),
        ) != "USLACKBOT" and "user" in event:  # Slack API does not allow USLACKBOT to post ephemeral messages
            retryable_original_client = bot.slack.RetryableAsyncWebClient(context.original_client)
            text = bot.slack.i18n.translate(
        ...
        messages_token = len(encoder.encode(
            "".join([json.dumps(message, ensure_ascii=False, separators=(",", ":")) for message in consumed_messages])
        ))
        ...
        if s.callback_reaction is not None:
            tasks.append(
                retryable_client.reactions_add(  # Tier 3 (50+ per minute)
                    channel=event["channel"],
        ...
        tasks.append(rate_limiter.take(rate_limiter_key, sum_prompt_tokens + sum_completion_tokens))
...
async def handle_message_im(
    ...
    if event["channel_type"] != "im" or ("subtype" in event and event["subtype"] not in ["file_share", "bot_message"]):
        return

    if event.get("user") == s.slack_bot_member_id:
        return
    ...
    with bot.telemetry.tracer.start_as_current_span(
        ...
        if (user := context.channel.get("user")) is not None:
...
async def handle_app_mention(
    ...
    with bot.telemetry.tracer.start_as_current_span(
        ...
        if (user := event.get("user")) is not None or (user := context.channel.get("creator")) is not None:
            if user_cache.get(user) is None:
                user_info = await retryable_client.users_info(  # Tier 4 (100+ per minute)
                    user=user,
                )
