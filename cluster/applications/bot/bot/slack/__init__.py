        ...
        if self.progress is not None:
            match stage:
                case cortex.llm.openai.agent.ProgressStage.FunctionCalling:
                    ...
                    header = translate(
                        "The following task is being executed:", locale=self.locale
                    )
                    text = f"{header}```{tasks}```"

                    if len(text.encode("utf-8")) > CHAT_UPDATE_MAX_BYTES:
                        omitted_text = translate("(truncated)... ", locale=self.locale)
                        max_tasks_bytes = CHAT_UPDATE_MAX_BYTES - len(
                            f"{header}```{omitted_text}```".encode("utf-8")
                        )

                        tasks_bytes = tasks.encode("utf-8")
                        truncated_tasks = omitted_text + tasks_bytes[
                            -max_tasks_bytes:
                        ].decode("utf-8")
                        text = f"{header}```{truncated_tasks}```"
...
class SlackMarkdownRenderer(mistune.renderers.markdown.MarkdownRenderer):
    ...
    def text(
        self, token: collections.abc.Mapping[str, typing.Any], state: mistune.BlockState
    ) -> str:
        token = token["raw"]
...
_PATTERNS: collections.abc.Mapping[str, tuple[str, str]] = {
    "external_domain": (r"minikube.127.0.0.1.nip.io", r"kaidotio.dev"),
    "url_shortener_url": (
        r"http://url-shortener.url-shortener.svc.cluster.local:8080",
        r"https://url-shortener.minikube.127.0.0.1.nip.io",
    ),
}
_COMPILED_PATTERNS = {
    pattern_name: re.compile(pattern)
    for pattern_name, (pattern, _) in _PATTERNS.items()
}


def transform_message(
    message: str, renderer: mistune.renderers.markdown.MarkdownRenderer
) -> str:
    transformed_message = mistune.Markdown(renderer=renderer)(message)

    for pattern_name, compiled_pattern in _COMPILED_PATTERNS.items():
        transformed_message = compiled_pattern.sub(
            _PATTERNS[pattern_name][1], transformed_message
        )
...
def retry(attempts: int, next_delay: int, max_delay: int | None = None):
    def decorator(f):
        ...
        async def wrapper(*args, **kwargs):
            ...
            async def _retry(
                _attempts: int, _next_delay: int, _max_delay: int | None = None
            ):
                nonlocal i
                ...
                try:
                    ...
                    i += 1
                    return await _retry(_attempts, _next_delay, _max_delay=_max_delay)
            ...
            return await _retry(attempts, next_delay, _max_delay=max_delay)
...
class RetryableAsyncWebClient(slack_sdk.web.async_client.AsyncWebClient):
    ...
    def __getattribute__(self, name):
        ...
        if asyncio.iscoroutinefunction(attr):
            ...
            @retry(attempts=10, next_delay=1, max_delay=10)
