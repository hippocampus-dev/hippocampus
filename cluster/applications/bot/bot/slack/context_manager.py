...
class Contexts(pydantic.BaseModel, collections.abc.MutableMapping):
    __root__: collections.abc.MutableMapping[str, Context]
...
async def register(bolt: slack_bolt.async_app.AsyncApp, brain: cortex.brain.Brain):
    ...
    async def handle_add_context(
        ...
        if len(context.title) > OPTION_VALUE_LIMIT:
            errors[TITLE_BLOCK_ID] = f"タイトルは{OPTION_VALUE_LIMIT}文字以下にしてください。"
        ...
        contexts = Contexts.parse_raw(restored_contexts) if restored_contexts else Contexts.parse_obj({})
        ...
        await brain.save(
            "contexts",
            json.dumps(contexts.__root__, ensure_ascii=False, default=lambda x: x.dict()).encode("utf-8"),
        )
    ...
    async def handle_use_confirm(
        ...
        await retryable_client.chat_postMessage(channel=private_metadata.channel_id, text=context.body)
                ...
                "blocks": [
                    {
                        ...
                        "element": {
                            ...
                            "placeholder": {"type": "plain_text", "text": "タイトルを入力してください"},
                        },
                    },
                ],
                "close": {"type": "plain_text", "text": "キャンセル"},
    ...
    async def handle_edit_context(
        ...
        values = body["view"]["state"]["values"]
        title = values[EDIT_CONTEXT_BLOCK_ID][EDIT_CONTEXT_ACTION_ID]["selected_option"]["value"]
        ...
        options = [
            {
                "text": {
                    "type": "plain_text",
                    "text":
                        title[:OPTION_TEXT_LIMIT - 3] + "..."
                        if len(title) > OPTION_TEXT_LIMIT else title,
                },
                "value": title,
            }
            for title, context in contexts.items()
    ...
    async def handle_delete_context(
        ...
        values = body["view"]["state"]["values"]
        title = values[DELETE_CONTEXT_BLOCK_ID][DELETE_CONTEXT_ACTION_ID]["selected_option"]["value"]
    ...
    async def handle_select_context(
        ...
        values = body["view"]["state"]["values"]
        title = values[SELECT_CONTEXT_BLOCK_ID][SELECT_CONTEXT_ACTION_ID]["selected_option"]["value"]
