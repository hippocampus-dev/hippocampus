...
const App = () => {
  ...
  const [versions, setVersions] = useState(
    supportedVersions[selectedGroup] ?? [],
  );
  ...
  useEffect(() => {
    ...
    fetch(
      `${HOST}/${selectedNamespace}/${selectedGroup}/${selectedVersion}/${selectedKind.toLowerCase()}`,
      {
        credentials: "include",
        signal: abortController.signal,
      },
  ...
  useEffect(() => {
    ...
    fetch(
      `${HOST}/${selectedNamespace}/${selectedGroup}/${selectedVersion}/${selectedKind.toLowerCase()}/${selectedResource}`,
      {
        credentials: "include",
        signal: abortController.signal,
      },
      ...
      .then((json) => {
        setResource(json);
        const editable = rendererFactory(
          selectedGroup,
          selectedVersion,
          selectedKind,
        ).editable(json);
  ...
  const handleCreateJob = () => {
    ...
    if (
      !window.confirm("Do you really want to create a job from this cronjob?")
    ) {
      return;
    }
    ...
    fetch(
      `${HOST}/${selectedNamespace}/${selectedGroup}/${selectedVersion}/job/manual-${selectedResource}-${Date.now()}/from/cronjob/${selectedResource}`,
      {
        method: "POST",
        headers: {
  ...
  return h(
    "div",
    {
      class: "flex flex-col min-h-screen items-center bg-blue-50 p-6 space-y-6",
    },
    [
      h(
        "div",
        { class: "flex flex-row space-x-6 bg-white p-6 rounded-lg shadow-lg" },
        [
          h("div", { class: "flex flex-col" }, [
            h(
              "label",
              { for: "namespace-select", class: "sr-only" },
              "Namespace",
            ),
            h(
              "select",
              ...
              namespaces.map((namespace) =>
                h("option", { value: namespace }, namespace),
              ),
            ),
          ]),
          h("div", { class: "flex flex-col" }, [
          ...
          h("div", { class: "flex flex-col" }, [
            h(
              "label",
              { for: "version-select", class: "sr-only" },
              "API Version",
            ),
            h(
              "select",
              ...
              versions.map((version) =>
                h("option", { value: version }, version),
              ),
            ),
          ]),
          h("div", { class: "flex flex-col" }, [
          ...
          h("div", { class: "flex flex-col" }, [
            h(
              "label",
              { for: "resource-select", class: "sr-only" },
              "Resource",
            ),
            h(
              "select",
      ...
      !isEditMode
        ? h(
            ...
            [
              h(
                ...
                {
                  class:
                    "border border-blue-300 rounded px-4 py-2 bg-blue-500 text-white mb-4",
        ...
        : h(
            ...
            [
              ...
              selectedKind === "CronJob" &&
                h(
                  "button",
                  {
                    class:
                      "border border-blue-300 rounded px-4 py-2 mt-4 bg-green-500 text-white",
              ...
              h(
                ...
                {
                  class:
                    "border border-blue-300 rounded px-4 py-2 bg-red-500 text-white mt-4",
              ...
              h(
                ...
                {
                  class:
                    "border border-blue-300 rounded px-4 py-2 bg-gray-500 text-white mt-4",
