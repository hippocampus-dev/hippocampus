...
_DEFAULT_EXPIRE = 60 * 60 * 24 * 7
_OPENAI_FUNCTION_CALLING_NAME_REGEXP = re.compile(r"^[a-zA-Z0-9_-]{1,64}$")
...
class FunctionDefinition(pydantic.BaseModel):
    func: typing.Callable[
        ...,
        typing.Awaitable[
            FunctionResult | typing.AsyncIterator[openai.types.chat.ChatCompletionChunk]
        ],
    ]
...
class Context(abc.ABC):
    ...
    prompt_tokens: collections.abc.MutableMapping[
        cortex.llm.openai.model.CompletionModel, int
    ]
    completion_tokens: collections.abc.MutableMapping[
        cortex.llm.openai.model.CompletionModel, int
    ]
    embedding_tokens: collections.abc.MutableMapping[
        cortex.llm.openai.model.EmbeddingModel, int
    ]
    generated_images: collections.abc.MutableMapping[
        cortex.llm.openai.model.ImageModel, int
    ]
    processed_audio_seconds: collections.abc.MutableMapping[
    ...
    async def cache_history(
        ...
        context_id = (
            self.context_id if namespace is None else f"{namespace}:{self.context_id}"
        )
        match self.memory_type:
            case MemoryType.Redis:
                result = await cortex.llm.openai.AsyncOpenAI(
                    ...
                    dimensions=cortex.llm.openai.model.OPENAI_VECTOR_SIZE
                ...
                embedding_tokens = len(
                    self.encoder.encode(query, disallowed_special=())
                )
        ...
        match self.memory_type:
            case MemoryType.Redis:
                try:
                    if pk is not None:
                        history_memory = (
                            await cortex.llm.openai.agent.memory.RedisMemory.find(
                    ...
                    if query is not None:
                        ...
                        memories = await cortex.llm.openai.agent.memory.RedisMemory.find(
                            cortex.llm.openai.agent.memory.RedisMemory.context_id
                            == context_id,
                        ...
                        self.increment_embedding_tokens(
                            self.embedding_model, embedding_tokens
                        )

                        return [
                            History(
                                body=base64.b64decode(m.history),
    ...
    async def save_messages(
        ...
        match self.memory_type:
            case MemoryType.Redis:
                length = await cortex.llm.openai.agent.memory.RedisMemory.db().zcard(
    ...
    async def get_budget(self) -> int:
        match self.memory_type:
            case MemoryType.Redis:
                budget = await cortex.llm.openai.agent.memory.RedisMemory.db().get(
        ...
        match self.memory_type:
            case MemoryType.Redis:
                ...
                if remaining_budget < budget:
                    return False
                await cortex.llm.openai.agent.memory.RedisMemory.db().decr(
                    f"{self.context_id}:budget", budget
                )
                return True
            case _:
                raise NotImplementedError

    def increment_prompt_tokens(
        self, model: cortex.llm.openai.model.CompletionModel, prompt_tokens: int
    ):
        if model not in self.prompt_tokens:
            self.prompt_tokens[model] = 0
        self.prompt_tokens[model] += prompt_tokens

    def increment_completion_tokens(
        self, model: cortex.llm.openai.model.CompletionModel, completion_tokens: int
    ):
        if model not in self.completion_tokens:
            self.completion_tokens[model] = 0
        self.completion_tokens[model] += completion_tokens

    def increment_embedding_tokens(
        self, model: cortex.llm.openai.model.EmbeddingModel, embedding_tokens: int
    ):
        if model not in self.embedding_tokens:
            self.embedding_tokens[model] = 0
        self.embedding_tokens[model] += embedding_tokens

    def increment_generated_images(
        self, model: cortex.llm.openai.model.ImageModel, count: int
    ):
        if model not in self.generated_images:
            self.generated_images[model] = 0
        self.generated_images[model] += count

    def increment_processed_audio_seconds(
        self, model: cortex.llm.openai.model.AudioModel, duration: float
    ):
    ...
    def increment_converted_text_characters(
        self, model: cortex.llm.openai.model.AudioModel, characters: int
    ):
...
class Agent(abc.ABC):
    ...
    reasoning_effort: openai.types.chat.ChatCompletionReasoningEffort | None = None
    ...
    input_guardrails: collections.abc.Sequence[cortex.llm.Guardrail] = ()
    output_guardrails: collections.abc.Sequence[cortex.llm.Guardrail] = ()

    async def chat_completion_loop(
        self,
        messages: collections.abc.Sequence[collections.abc.Mapping[str, typing.Any]],
        context: Context,
        response_format: openai.types.shared_params.ResponseFormatText
        ...
        await self._run_guardrails(self.input_guardrails, messages)
        return await self._chat_completion_loop(
            messages,
            context,
    ...
    async def _chat_completion_loop(
        ...
        functions = [
            ...
            if definition.capability & context.capability
            and (
                len(definition.dependencies) == 0
                or definition.dependencies & context.call_stack
            )
        ]
        ...
        if self.summarize_history and prompt_tokens > self.model.max_tokens:
            summarizer = cortex.llm.openai.summarizer.refine.RefineSummarizer(
                cortex.llm.openai.model.CompletionModel.GPT_5_MINI_ALIAS,
                self.encoder,
            )
        ...
        try:
            response = await cortex.llm.openai.AsyncOpenAI(
                ...
                if self.reasoning_effort is not None and self.model.reasoning_supported
                else openai._types.NOT_GIVEN,
                verbosity=self.verbosity
                if self.model.verbosity_supported
                else openai._types.NOT_GIVEN,
                tools=[
                    openai.types.chat.ChatCompletionToolParam(**function)
                    for function in functions
                ]
                if self.model.tools_supported
                else openai._types.NOT_GIVEN,
                tool_choice="auto"
                if self.model.tools_supported
                else openai._types.NOT_GIVEN,
        ...
        if len(function_calls) == 0:
            ...
            if self.output_guardrails:
                if isinstance(response, openai.AsyncStream):
                    return self._collect_stream_with_output_guardrails(response)
        ...
        for function_call in function_calls:
            ...
            if not _OPENAI_FUNCTION_CALLING_NAME_REGEXP.match(name):
                replaced_function_call = (
                    openai.types.chat.ChatCompletionMessageToolCall(
                ...
                assistant_message["tool_calls"] = [
                    replaced_function_call.model_dump()
                    if tool_call["id"] == function_call.id
                    else tool_call
            ...
            if not await context.consume_budget(
                self.functions[name].consumption_budget
            ):
            ...
            cache_key = json.dumps(
                {"name": name, "arguments": raw_arguments}, ensure_ascii=False
            )
            ...
            if len(function_calls) == 1 and self.functions[name].direct_return:
                if isinstance(function_result, collections.abc.AsyncIterator):
                    if self.output_guardrails:
                        return self._collect_stream_with_output_guardrails(
                            function_result
                        )
                ...
                if self.output_guardrails:
                    await self._run_guardrails(
                        self.output_guardrails,
                        [{"role": "assistant", "content": str(function_result)}],
                    )
                return await construct_function_result_from_response(
        ...
        _ = asyncio.create_task(
            context.save_messages([assistant_message, *tool_messages])
        )
    ...
    def _calculate_consumed_prompt_tokens(
        ...
        prompt_tokens = len(
            self.encoder.encode("".join(prompts), disallowed_special=())
        )
    ...
    async def _execute_function_or_cache(
        ...
        if set(self.functions[name].parameters.get("required", [])) - set(
            actual_arguments.keys()
        ):
            return FunctionResult(
    ...
    async def _summary_function_result_with_cache(
        ...
        tokens = len(
            self.encoder.encode(function_result.response, disallowed_special=())
        )
        if tokens > remaining_tokens:
            ...
            function_result.response = await summarizer.summarize(
                function_result.response
            )
            context.increment_prompt_tokens(
                cortex.llm.openai.model.CompletionModel.GPT_5_MINI_ALIAS,
        ...
        if self.functions[name].cache.enabled:
            await context.cache_history(
                ...
                cortex.llm.openai.agent.History(
                    body=bytes(function_result.response, encoding="utf-8")
                ),
            )
    ...
    async def _run_guardrails(
        self,
        guardrails: collections.abc.Iterable[cortex.llm.Guardrail],
        messages: collections.abc.Sequence[collections.abc.Mapping[str, typing.Any]],
    ) -> None:
        items = list(guardrails)
        results = await asyncio.gather(*(g.check(messages) for g in items))
        for guardrail, result in zip(items, results):
            if result.tripwire:
                raise cortex.exceptions.GuardrailTripwireError(guardrail, result)

    async def _collect_stream_with_output_guardrails(
        self,
        response: typing.AsyncIterator[openai.types.chat.ChatCompletionChunk],
    ) -> typing.AsyncGenerator[openai.types.chat.ChatCompletionChunk, None]:
