...
class AudioAgent(cortex.llm.openai.agent.Agent):
    ...
    async def transcribe(
                ...
                match e.status_code:
                    case 400:
                        return cortex.llm.openai.agent.FunctionResult(
                            response="Content is not an audio file"
                        )
                    case 404:
        ...
        context.increment_processed_audio_seconds(
            self.audio_model, d.get("audio_seconds", 0)
        )
    ...
    async def text_to_speech(
        ...
        context.increment_converted_text_characters(
            cortex.llm.openai.model.AudioModel.TTS1, len(content)
        )
        return cortex.llm.openai.agent.FunctionResult(
