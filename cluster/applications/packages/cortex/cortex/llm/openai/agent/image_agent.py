...
class ImageAgent(cortex.llm.openai.agent.Agent):
    ...
    def __init__(
        ...
        if self.image_model.model_name == "dall-e-2":
            self.functions["generate_image_variation"] = (
                cortex.llm.openai.agent.FunctionDefinition(
    ...
    async def explain_image(
        ...
        if (
            len(latest_history.body)
            > cortex.llm.openai.model.CompletionModel.GPT_5_MINI.limit_size_of_image
        ):
        ...
        try:
            response = await cortex.llm.openai.AsyncOpenAI(
                ...
                model=cortex.llm.openai.model.CompletionModel.GPT_5_MINI,
                messages=[
                    {
                        "role": "user",
                        "content": [
            ...
            match e.status_code:
                case 400:
                    return cortex.llm.openai.agent.FunctionResult(
                        response="Content is not an image"
                    )
                case 404:
        ...
        context.increment_prompt_tokens(
            cortex.llm.openai.model.CompletionModel.GPT_5_MINI,
    ...
    async def generate_image(
        ...
        try:
            response = await cortex.llm.openai.AsyncOpenAI(
                ...
                quality=self.image_model.quality,
                response_format="b64_json"
                if self.image_model.model_name not in ("gpt-image-1", "gpt-image-1.5")
        ...
        await context.cache_history(
            ...
            cortex.llm.openai.agent.History(
                body=base64.b64decode(response.data[0].b64_json)
            ),
            namespace="content",
        )

        if self.image_model.model_name == "gpt-image-1":
            context.increment_prompt_tokens(
        ...
        elif self.image_model.model_name == "gpt-image-1.5":
            context.increment_prompt_tokens(
        ...
        return cortex.llm.openai.agent.FunctionResult(
            instruction=f"Then you MUST upload this image using `file_upload` with `{key}`.",
            response=(f"Generated image with `{key}` is successfully saved.\n"),
        )
    ...
    async def generate_image_variation(
        ...
        with tempfile.NamedTemporaryFile(suffix=".png") as f:
            try:
                PIL.Image.open(io.BytesIO(latest_history.body)).convert("RGBA").save(
                    f.name
                )
            except Exception:
                return cortex.llm.openai.agent.FunctionResult(
