...
class ImageAgent(cortex.llm.openai.agent.Agent):
    ...
    async def explain_image(
        ...
        if len(latest_history.body) > cortex.llm.openai.model.CompletionModel.GPT_5_MINI.limit_size_of_image:
    ...
    async def generate_image(
        ...
        try:
            response = await cortex.llm.openai.AsyncOpenAI(
                ...
                response_format="b64_json" if self.image_model.model_name not in ("gpt-image-1", "gpt-image-1.5") else openai._types.NOT_GIVEN,
            )
        except openai.APIConnectionError as e:
            raise cortex.exceptions.RetryableError(e) from e
        except openai.APIStatusError as e:
        ...
        key = response.data[0].revised_prompt if response.data[0].revised_prompt else prompt
        ...
        return cortex.llm.openai.agent.FunctionResult(
            ...
            response=(
                f"Generated image with `{key}` is successfully saved.\n"
            ),
        )
