...
class URLOpenAgent(cortex.llm.openai.agent.Agent):
    ...
    def __init__(
        ...
        self.functions = {
            ...
            "open_slack_message_url": cortex.llm.openai.agent.FunctionDefinition(
                func=self.open_slack_message_url,
                cache=cortex.llm.openai.agent.FunctionCache(enabled=True, similarity_threshold=1.0),
                direct_return=True,
                description="Open the slack message URL and retrieve its content. e.g. https://workspace.slack.com/archives/C01UJ9KQJ1D/p1638960000000100",
                parameters={
                    "type": "object",
                    "properties": {
                        "url": {
            ...
            async with aiohttp.ClientSession(headers=headers, trust_env=True) as session:
                async with session.get(
                    ...
                    try:
                        ...
                        return cortex.llm.openai.agent.FunctionResult(
                            instruction=(
                                f"MUST include `{path} content` as arguments to call other function about this content.\n"
                                "\n"
                                "e.g.\n"
                                f"Generate an image of `{path}` other variations.\n"
                            ),
            ...
            async with aiohttp.ClientSession(headers=headers, trust_env=True) as session:
                async with session.get(url, allow_redirects=True) as response:
                    ...
                    try:
                        ...
                        return cortex.llm.openai.agent.FunctionResult(
                            instruction=(
                                f"MUST include `{url} content` as arguments to call other function about this content.\n"
                                "\n"
                                "e.g.\n"
                                f"Generate an image of `{url}` other variations.\n"
                            ),
    ...
    async def open_slack_message_url(
        self,
        url: str,
        context: cortex.llm.openai.agent.Context,
    ) -> cortex.llm.openai.agent.FunctionResult:
        ...
        if m is None:
            return cortex.llm.openai.agent.FunctionResult(response=f"Bad request: {url}")

        client = slack_sdk.web.async_client.AsyncWebClient(token=self.slack_token)
        ...
        try:
            ...
            is_private = (response["channel"].get("is_im")) or (response["channel"].get("is_private"))
            if is_private and (
                not (hasattr(context, "channel") and context.channel["id"] == response["channel"]["id"])):
                return cortex.llm.openai.agent.FunctionResult(response=f"Channel is not public: {url}")
        ...
        response = [
            {
                "user": f"<@{message.get('user', message.get('username', ''))}>",
                "text": message["text"],
                "files": [
                    {
                        "name": file["name"],
    ...
    async def open_other_url(
        ...
        with opentelemetry.trace.get_tracer("playwright").start_as_current_span(
            ...
            browser_context: playwright.async_api.BrowserContext = await self.browser.new_context()
