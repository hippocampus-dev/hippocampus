...
class URLOpenAgent(cortex.llm.openai.agent.Agent):
                ...
                async with session.get(
                    ...
                    if response.status != 200:
                        ...
                        match response.status:
                            case 404:
                                return cortex.llm.openai.agent.FunctionResult(
                                    response=f"Not found: #{issue_number}"
                                )
                            case 409 | 429 | 502 | 503 | 504:
                                raise cortex.exceptions.RetryableError(e) from e
                        raise e
    ...
    async def open_slack_message_url(
        ...
        try:
            ...
            is_private = (response["channel"].get("is_im")) or (
                response["channel"].get("is_private")
            )
            if is_private and (
                not (
                    hasattr(context, "channel")
                    and context.channel["id"] == response["channel"]["id"]
                )
            ):
                return cortex.llm.openai.agent.FunctionResult(
                    response=f"Channel is not public: {url}"
                )
    ...
    async def open_google_drive_url(
        ...
        try:
            await asyncio.get_running_loop().run_in_executor(
                None, run_in_context, opentelemetry.context.get_current()
            )
        except googleapiclient.errors.HttpError as e:
            match e.resp.status:
                case 403:
                    return cortex.llm.openai.agent.FunctionResult(
                        response=f"Forbidden: {url}"
                    )
                case 404:
                    return cortex.llm.openai.agent.FunctionResult(
                        response=f"Not found: {url}"
    ...
    async def open_other_url(
        ...
        with opentelemetry.trace.get_tracer("playwright").start_as_current_span(
            ...
            try:
                ...
                return cortex.llm.openai.agent.FunctionResult(
                    response="Error when navigating"
                )
