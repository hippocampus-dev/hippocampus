...
class BrowserAgent(cortex.llm.openai.agent.Agent):
    ...
    async def navigate(
        ...
        try:
            ...
            return cortex.llm.openai.agent.FunctionResult(
                response="Timeout when navigating"
            )
    ...
    async def navigate_back(
        self, context: cortex.llm.openai.agent.Context
    ) -> cortex.llm.openai.agent.FunctionResult:
        p = await self._get_page(self.browser)
        try:
        ...
        if response:
            ...
            return cortex.llm.openai.agent.FunctionResult(
                response="Unable to navigate back"
            )
    ...
    async def extract_text(
        ...
        return cortex.llm.openai.agent.FunctionResult(
            response=soup.get_text(separator=" ", strip=True)
        )
    ...
    async def screenshot(
        ...
        return cortex.llm.openai.agent.FunctionResult(
            response="Screenshot taken is succeeded but it need to upload"
        )
