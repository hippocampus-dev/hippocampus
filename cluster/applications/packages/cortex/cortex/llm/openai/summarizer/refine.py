...
_REFINE_SYSTEM_PROMPT_TEMPLATE = (
    "Your task is to create a concise running summary of actions and information results in the provided text while keeping the input language, focusing on key and potentially important information to remember.\n"
...
class RefineSummarizer(cortex.llm.openai.summarizer.Summarizer):
    ...
    async def summarize(self, content: str) -> str:
        initial_prompt_tokens = self.encoder.encode(
            _INITIAL_SYSTEM_PROMPT_TEMPLATE.format(text=""), disallowed_special=()
        )
        initial_prompt_tokens_count = len(initial_prompt_tokens)
        refine_prompt_tokens = self.encoder.encode(
            _REFINE_SYSTEM_PROMPT_TEMPLATE.format(existing_answer="", text=""),
            disallowed_special=(),
        )
        refine_prompt_tokens_count = len(refine_prompt_tokens)
        messages_format_token = len(
            self.encoder.encode('{"role":"system","content":""}', disallowed_special=())
        )

        default_chunk_size = int(
            (
                self.model.max_tokens
                - initial_prompt_tokens_count
                - refine_prompt_tokens_count
                - messages_format_token
            )
            / 2
        )
        ...
        if len(tokens) < chunk_size:
            return await self._summary(
                _INITIAL_SYSTEM_PROMPT_TEMPLATE.format(text=content)
            )
        ...
        while tokens:
            ...
            prompt = (
                _INITIAL_SYSTEM_PROMPT_TEMPLATE.format(text=chunk_text)
                if summary == ""
                else _REFINE_SYSTEM_PROMPT_TEMPLATE.format(
                    existing_answer=summary, text=chunk_text
                )
            )
            summary = await self._summary(prompt)

            chunk_size = default_chunk_size - len(
                self.encoder.encode(summary, disallowed_special=())
            )
