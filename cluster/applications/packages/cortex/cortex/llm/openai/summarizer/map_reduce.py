...
class MapReduceSummarizer(cortex.llm.openai.summarizer.Summarizer):
    ...
    async def summarize(self, content: str) -> str:
        initial_prompt_tokens = self.encoder.encode(
            _SYSTEM_PROMPT_TEMPLATE.format(text=""), disallowed_special=()
        )
        ...
        mapped_summaries = await asyncio.gather(
            *[
                self._summary(_SYSTEM_PROMPT_TEMPLATE.format(text=chunk))
                for chunk in chunks
            ]
        )
        ...
        challenge = 1
        while (
            len(self.encoder.encode(summary, disallowed_special=()))
            > self.model.max_tokens
        ):
            # Avoid infinite loop
