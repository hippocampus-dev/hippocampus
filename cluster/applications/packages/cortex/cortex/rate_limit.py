...
class RedisSlidingRateLimiter(RateLimiter):
    ...
    async def remaining(self, key: str, limit: int) -> RateLimitInfo:
        ...
        try:
            await self.redis_client.zremrangebyscore(
                self._redis_key(key), 0, window_seconds
            )
            results = await self.redis_client.zrangebyscore(
                self._redis_key(key), window_seconds, ts
            )
        except (redis.exceptions.ConnectionError, redis.exceptions.ReadOnlyError) as e:
            raise cortex.exceptions.RetryableError(e)
        ...
        if len(results) == 0:
            return RateLimitInfo(
                limit=limit, remaining=limit, reset_timestamp=ts + self.interval_seconds
            )

        used_tokens = sum(
            [int(tokens.decode("utf-8").split(":")[0]) for tokens in results]
        )
        remaining_tokens = max(0, limit - used_tokens)

        oldest_timestamp = min(
            [int(tokens.decode("utf-8").split(":")[1]) for tokens in results]
        )
    ...
    async def take(self, key: str, amount: int):
        ...
        try:
            if (
                await self.redis_client.zcount(self._redis_key(key), "-inf", "+inf")
                == 0
            ):
                p = self.redis_client.pipeline()
                p.multi()
                ...
                await self.redis_client.zadd(
                    self._redis_key(key), {f"{amount}:{ts}": ts}
                )
        except (redis.exceptions.ConnectionError, redis.exceptions.ReadOnlyError) as e:
            raise cortex.exceptions.RetryableError(e)
