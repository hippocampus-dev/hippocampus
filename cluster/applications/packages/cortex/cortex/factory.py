...
class RoundRobinFactory(LoadBalancingFactory[T]):
    ...
    def __init__(
        self,
        factories: collections.abc.Sequence[
            typing.Callable[..., typing.Awaitable[T | None]]
        ],
    ):
...
class ConsistentHashRing:
    ...
    def remove_node(self, node_index: int) -> None:
        ...
        self.ring = [
            (hash_value, node) for hash_value, node in self.ring if node != node_index
        ]
...
class ConsistentHashFactory(LoadBalancingFactory[T]):
    ...
    def __init__(
        ...
        self.ring = (
            ConsistentHashRing(len(factories), virtual_nodes_count=virtual_nodes_count)
            if factories
            else None
        )
