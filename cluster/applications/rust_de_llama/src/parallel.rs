...
/// Iterations a retained prompt-cache prefix may be carried before it is
/// dropped: attention spans `GGML_PAD(used_max_p1(), 256)` cells
/// (`llama.cpp/src/llama-kv-cache.cpp:1005-1010`), so a prefix above the live
/// sequences' cells widens the span every batch reads -- and costs nothing at
/// all when they already reach above it, which this does not check. Charged per
/// batch rather than per token in it: a 1908-token prefill measured the same
/// after other requests had filled the pool as on a fresh one. The rate, from a
/// single stream at n_ctx 4096 on gemma-3-4b-it-Q4_K_M, is ~2.9 ms per extra
/// 1000 cells in 256-cell steps, against ~0.22 ms saved per prefix token on a
/// hit, putting break-even at ~76 iterations. This is `DEFAULT_MAX_TOKENS / 2`:
/// under the break-even because nobody has asked for this prefix, and under
/// `DEFAULT_MAX_TOKENS` or a default-length request sheds the tax only on its
/// last iteration.
const RETAINED_PREFIX_DECODE_BUDGET: usize = 64;
/// Budget for the prefix the queue head is about to reuse. A match raises the
/// odds of the hit, not the point where the span tax outgrows what a hit can pay
/// back, so this is the break-even above rather than a multiple of it.
const RETAINED_PREFIX_CREDITED_DECODE_BUDGET: usize = 76;
...
impl ParallelProcessor {
    pub fn new(
        ...
        // With kv_unified = true the whole buffer is addressable by every
        // sequence, so the per-sequence context is n_ctx itself, mirroring
        // llama.cpp/src/llama-context.cpp:170-176.
        let n_ctx_seq = pad_to_kv_boundary(n_ctx);
    ...
    /// Per-sequence context size: the padded n_ctx, since kv_unified = true
    /// lets every sequence address the whole cache. It bounds one sequence, not
    /// the sum of the concurrent ones, which share those same cells.
    pub fn n_ctx_seq(&self) -> i32 {
        self.n_ctx_seq
    }

    /// The loaded weights, so another context can be created against them
    /// instead of loading the same GGUF a second time. Holding the returned
    /// `Arc` keeps the weights resident past this processor's eviction.
    pub fn model(&self) -> std::sync::Arc<rust_de_llama::LlamaModel> {
        self.model.clone()
    }
        ...
        loop {
            ...
            while !pending_tasks.is_empty() {
                ...
                if slots.iter().any(|slot| slot.is_idle()) {
                    // Under a unified KV cache the sequences share one pool, so a
                    // free slot is not free capacity: admit against the cells the
                    // in-flight and retained sequences already hold. Retained
                    // prompt-cache cells are given up first -- a cache miss costs
                    // a prefill, whereas over-admitting fails the decode for
                    // every sequence in the batch.
                    let prompt_tokens = &pending_tasks[0].prompt_tokens;
                    let needed = Self::cells_needed(&pending_tasks[0], self.n_ctx_seq);
                    // Retained cells are recoverable, an in-flight sequence's
                    // reservation is not. Spending the prompt cache on a request
                    // that still would not fit loses the cache and admits
                    // nothing, so check feasibility before paying for it.
                    if Self::reserved_by_active(&slots, self.n_ctx_seq).saturating_add(needed)
                        <= self.n_ctx_seq.max(0) as usize
                    {
                        while !self.has_kv_capacity(&slots, needed, prompt_tokens)
                            && self.release_retained_cells(&mut slots, &context, prompt_tokens)
                        {
                        }
                    }
                    if !self.has_kv_capacity(&slots, needed, prompt_tokens) {
                        // Only in-flight work can free what is missing -- a
                        // request too large to ever fit is already refused by
                        // the handler's prompt-length check. Retry next round.
                        break;
                    }
                ...
                .await
            {
                Ok(batched) => {
                    if batched {
                        let pending_prompt = pending_tasks
                            .front()
                            .map(|task| task.prompt_tokens.as_slice());
                        self.expire_retained_cells(&mut slots, &context, pending_prompt);
                    }
                }
                Err(e) => {
                    opentelemetry_tracing::error!("Failed to process active slots: {}", e);
                    // A failed combined decode produced no logits for any slot
                    // in it, so every active sequence now holds a stale
                    // `logits_index`; sampling from one aborts the process
                    // inside llama.cpp. Fail them instead of looping back into
                    // that state. The shared KV cache is the reachable cause --
                    // admission bounds a single request, not the sum of the
                    // concurrent ones -- so the client is told exactly that.
                    Self::fail_active_slots(&mut slots, &context, &mut draft_proposer, &e);
                }
            }
        }
    }

    /// KV cells a task will hold at its peak: the whole prompt plus the tokens
    /// it is allowed to generate, capped at what one sequence may address.
    fn cells_needed(task: &Task, n_ctx_seq: i32) -> usize {
        Self::cells_for(task.prompt_tokens.len(), task.request.max_tokens, n_ctx_seq)
    }

    /// Peak cells a sequence with this prompt and token budget will hold.
    fn cells_for(prompt_tokens: usize, max_tokens: Option<u32>, n_ctx_seq: i32) -> usize {
        let budget = max_tokens
            .map(|max_tokens| max_tokens as usize)
            .unwrap_or(DEFAULT_MAX_TOKENS);
        prompt_tokens
            .saturating_add(budget)
            .min(n_ctx_seq.max(0) as usize)
    }

    /// Cells spoken for: an in-flight sequence is counted at the peak it will
    /// grow to, not the prefix it happens to have decoded, because admitting
    /// against current usage would let four requests in while none of them has
    /// decoded anything yet. An idle slot holds whatever prefix it retained for
    /// prompt-cache reuse.
    fn committed_cells(slots: &[Slot], n_ctx_seq: i32) -> usize {
        slots
            .iter()
            .map(|slot| match &slot.sequence {
                Some(sequence) => Self::cells_for(
                    sequence.prompt_token_count,
                    sequence.task.request.max_tokens,
                    n_ctx_seq,
                )
                .max(sequence.n_past),
                None => slot.cached_tokens().len(),
            })
            .sum()
    }

    /// Cells held by in-flight sequences alone. Unlike retained prompt-cache
    /// cells these cannot be reclaimed on demand, so they set the floor on what
    /// admission can ever hope to free right now.
    fn reserved_by_active(slots: &[Slot], n_ctx_seq: i32) -> usize {
        slots
            .iter()
    ...
    /// Whether `needed` cells fit once this prompt takes a slot. The chosen
    /// slot's retained prefix is subtracted: `assign_task_to_slot` either reuses
    /// those cells as the shared prefix or drops the divergent tail, so counting
    /// them alongside the full prompt would bill the same cells twice and reject
    /// exactly the multi-turn requests the prompt cache exists to serve.
    fn has_kv_capacity(&self, slots: &[Slot], needed: usize, prompt_tokens: &[i32]) -> bool {
        let reusable = self
            .select_slot(slots, prompt_tokens)
            .map(|(slot_id, _)| slots[slot_id].cached_tokens().len())
            .unwrap_or(0);
        Self::committed_cells(slots, self.n_ctx_seq)
            .saturating_sub(reusable)
            .saturating_add(needed)
            <= self.n_ctx_seq.max(0) as usize
    }

    /// Drop the prefixes idle slots kept for prompt-cache reuse, returning their
    /// cells to the shared pool. Only worth doing when admission is short of
    /// capacity: it trades a future prefill for a request that can run now.
    fn release_retained_cells(
        &self,
        slots: &mut [Slot],
        context: &rust_de_llama::LlamaContext,
        prompt_tokens: &[i32],
    ) -> bool {
        // Never the slot this prompt would land on: `has_kv_capacity` already
        // credits its retained cells back, so releasing it moves the capacity
        // sum by nothing and only costs the cache.
        let credited = self.select_slot(slots, prompt_tokens).map(|(id, _)| id);

        // One slot per call, the least useful first: the caller re-checks
        // capacity between releases so a request that fits after freeing one
        // slot does not cost the others their cache. Mirrors llama.cpp's
        // try_clear_idle_slots, which clears "slots one by one" for the same
        // reason (llama.cpp/tools/server/server-context.cpp:1027-1041).
        let victim = slots
            .iter()
            .enumerate()
            .filter(|(slot_idx, slot)| {
                Some(*slot_idx) != credited && slot.is_idle() && !slot.cached_tokens().is_empty()
            })
            .min_by_key(|(_, slot)| common_prefix_len(slot.cached_tokens(), prompt_tokens))
            .map(|(slot_idx, _)| slot_idx);

        match victim {
            Some(slot_idx) => {
                tracing::debug!("Releasing retained prompt cache on slot {}", slot_idx);
                slots[slot_idx].stop_task();
                context.clear_sequence(slot_idx as i32);
                true
            }
    ...
    /// Charge this iteration to every retained prefix and drop the ones past budget.
    /// `release_retained_cells` only runs when admission is short of capacity, so
    /// without this a prefix nobody asks for stays resident for the life of the
    /// process. Ageing is by iterations rather than wall clock because those are
    /// what a retained prefix charges for; `pending_prompt` is the queue head's,
    /// and the prefix it would reuse earns the wider budget.
    fn expire_retained_cells(
        &self,
        slots: &mut [Slot],
        context: &rust_de_llama::LlamaContext,
        pending_prompt: Option<&[i32]>,
    ) {
        if slots.iter().all(|slot| slot.cached_tokens().is_empty()) {
            return;
        }

        // Only a real prefix match is worth sparing: `select_slot` falls back to
        // the first idle slot at `common_len == 0`, and `assign_task_to_slot`
        // drops that slot's whole prefix anyway.
        let credited = pending_prompt
            .and_then(|prompt_tokens| self.select_slot(slots, prompt_tokens))
            .filter(|(_, common_len)| *common_len > 0)
            .map(|(slot_id, _)| slot_id);

        for (slot_idx, slot) in slots.iter_mut().enumerate() {
            slot.charge_decode_step();
            // `start_task` hands retained tokens to the sequence it starts, so
            // this gate is what keeps a live request out.
            if slot.cached_tokens().is_empty() {
                continue;
            }
            let budget = if Some(slot_idx) == credited {
                RETAINED_PREFIX_CREDITED_DECODE_BUDGET
            } else {
                RETAINED_PREFIX_DECODE_BUDGET
            };
            if slot.decode_steps_while_retained() > budget {
                tracing::debug!("Dropping retained prompt cache on slot {}", slot_idx);
                slot.stop_task();
                context.clear_sequence(slot_idx as i32);
            }
        }
    ...
    /// End every in-flight sequence with `cause`, releasing its KV cells and
    /// slot. Used when the batch they shared failed and their sampling state can
        ...
        for slot_idx in 0..slots.len() {
            ...
            if let Some(seq) = &slots[slot_idx].sequence {
                let _ = seq
                    .task
                    .response_tx
                    .try_send(Err(error::error!("{}", cause)));
            }
            slots[slot_idx].stop_task();
            context.clear_sequence(slot_idx as i32);
            if let Some(draft) = draft.as_mut() {
...
/// Round a context size up the way llama.cpp pads one before deriving the
/// per-sequence context (`GGML_PAD(n_ctx, 256)`). Computing it here keeps
/// admission control and the completion boundary on the same value the context
/// was actually built with, rather than the unpadded request.
pub(crate) fn pad_to_kv_boundary(n_ctx: i32) -> i32 {
    const KV_BOUNDARY: i32 = 256;
    n_ctx.saturating_add(KV_BOUNDARY - 1) / KV_BOUNDARY * KV_BOUNDARY
}
