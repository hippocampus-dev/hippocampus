...
use slot::{ActiveSequence, CompletionReason, Slot, StopMatcher};
...
impl ParallelProcessor {
    ...
    async fn assign_task_to_slot(
        ...
        let response_tx = task.response_tx.clone();
        let setup_result = async {
            let tokens = self.tokenize_prompt(&task.request.prompt).await?;
        ...
        match setup_result {
            ...
            Err(e) => {
                ...
                let _ = response_tx.send(Err(error::error!("{}", error_msg))).await;
                Err(e)
            }
        }
    }
    ...
    pub async fn run_processing_loop(
        ...
        for slot_id in 0..self.n_parallel {
            let sampler = match rust_de_llama::LlamaSampler::new() {
                Ok(s) => s,
                Err(e) => {
                    tracing::error!("Failed to create sampler for slot {}: {}", slot_id, e);
                    return;
                }
            };
        ...
        loop {
            ...
            while !pending_tasks.is_empty() {
                if let Some(slot_id) = Self::get_available_slot(&slots) {
                    ...
                    match self.assign_task_to_slot(slot_id, task, &mut slots).await {
                        Ok(()) => {}
                        Err(e) => {
                            tracing::error!("Failed to assign task to slot: {}", e);
                        }
                    }
                } else {
                    break;
                ...
                .await
            {
                Ok(_) => {}
                Err(e) => {
                    tracing::error!("Failed to process active slots: {}", e);
                }
            }
        }
    }
