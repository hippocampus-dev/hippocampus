...
/// Find the first full occurrence of `stop` anywhere in `text`, returning the
/// byte offset where it starts.
///
/// Unlike [`find_partial_stop`], the match need not end at the text end: this
/// catches a complete stop embedded inside a token piece whose trailing bytes
/// belong to the same piece (e.g. a piece `"</s>\n"`), which the suffix-anchored
/// search misses. Mirrors llama.cpp server's substring stop scan.
pub fn find_full_stop(text: &str, stop: &str) -> Option<usize> {
    if stop.is_empty() {
        return None;
    }
    text.find(stop)
...
mod tests {
    ...
    #[test]
    fn test_find_full_stop_embedded_with_trailing_bytes() {
        let text = "Hello world</s>\n";
        let stop = "</s>";
        let result = find_full_stop(text, stop);
        assert_eq!(result, Some(11));
    }

    #[test]
    fn test_find_full_stop_at_end() {
        let text = "Hello world</s>";
        let stop = "</s>";
        let result = find_full_stop(text, stop);
        assert_eq!(result, Some(11));
    ...
    #[test]
    fn test_find_full_stop_no_match() {
        let text = "Hello world<";
        let stop = "</s>";
        let result = find_full_stop(text, stop);
        assert_eq!(result, None);
    ...
    #[test]
    fn test_find_full_stop_empty_stop() {
        let text = "Hello world";
        let stop = "";
        let result = find_full_stop(text, stop);
        assert_eq!(result, None);
