...
8. **Varnish Caching Layer**: External-facing Varnish cache with dynamic director and Istio TLS origination for OpenAI API calls. The `/v1/embeddings` endpoint responses are cached (embedding requests are deterministic for the same input), while `/v1/chat/completions` requests pass through uncached. The request path is: memory-bank → ext-proc-proxy → Varnish → Istio TLS origination → api.openai.com

### Service Workflow
1. **Upsert**:
   - Documents → Categorization → Chunking (tiktoken) → Generate category UUID
   - Merge with existing memories if present
   - Generate OpenAI embeddings in batches (via ext-proc-proxy → Varnish cache → api.openai.com) → Generate BM25/BM42 sparse embeddings
   - Store in Qdrant with both dense and sparse vectors

2. **Query**:
   - Empty queries with category filter → Retrieve all memories for category
   - Non-empty queries → Generate OpenAI embeddings (via ext-proc-proxy → Varnish cache → api.openai.com) → Hybrid search in Qdrant
...
### Docker Deployment
- Multi-stage build with Python 3.14-slim-bookworm base
