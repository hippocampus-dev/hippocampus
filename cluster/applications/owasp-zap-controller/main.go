...
import (
	...
	"flag"
	"os"
	garV1 "owasp-zap-controller/api/v1"
	"owasp-zap-controller/internal/controllers"
	"owasp-zap-controller/internal/runnable"
	"strconv"
	"time"
...
func main() {
	...
	flag.StringVar(&callbackHost, "callback-host", envOrDefaultValue("CALLBACK_HOST", "owasp-zap-controller.owasp-zap-controller.svc.cluster.local:8082"), "The callback host")
	opts := zap.Options{}
	...
	m, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID: "owasp-zap-controller",
	})
	if err != nil {
	...
	if err := (&controllers.ScanReconciler{
		...
		Recorder:     m.GetEventRecorderFor("owasp-zap-controller"),
		CallbackHost: callbackHost,
	}).SetupWithManager(m); err != nil {
		entrypointLogger.Error(err, "unable to create controller", "controller", "Scan")
		os.Exit(1)
	}
	...
	if err := m.Add(runnable.NewServer()); err != nil {
