...
opt="--version 1.16.19"
# Enable metrics
opt+=" --helm-set prometheus.enabled=true --helm-set operator.prometheus.enabled=true --helm-set hubble.metrics.enableOpenMetrics=true"
...
opt+=" --helm-set localRedirectPolicy=true"
# only `cilium install` auto-detects this, so an upgrade without it silently falls back to "default"
opt+=" --helm-set cluster.name=minikube"
...
${t}/cilium install ${opt} --set hubble.enabled=true --helm-set hubble.metrics.enabled="{dns,drop,tcp,flow,flows-to-world,port-distribution,icmp,httpV2:exemplars=true;labelsContext=source_ip\,source_namespace\,source_workload\,destination_ip\,destination_namespace\,destination_workload\,traffic_direction}" --set hubble.export.dynamic.enabled=true --set hubble.export.dynamic.config.content[0].name=allowlist --set hubble.export.dynamic.config.content[0].filePath=/var/run/cilium/hubble/events.log --set hubble.export.dynamic.config.content[0].includeFilters[0].source_label[0]="k8s:hubble.cilium.io/export.source=true" --set hubble.export.dynamic.config.content[0].includeFilters[1].destination_label[0]="k8s:hubble.cilium.io/export.destination=true" --helm-set ipam.operator.clusterPoolIPv4MaskSize=20
