...
DEFAULT_OAUTH2_PROXY_COOKIE_SECRET=$(openssl rand -hex 16)
...
DEFAULT_JUPYTERHUB_CONFIGPROXY_AUTH_TOKEN=$(openssl rand -hex 32)
DEFAULT_DOCKER_CONFIG_JSON="data:;base64,$(cat ~/.docker/config.json | base64 -w0)"
DEFAULT_SLACK_LOGGER_TIDB_SLACK_USER_PASSWORD=$(openssl rand -hex 32)
DEFAULT_SLACK_OAUTH_BRIDGE_CLIENT_SECRET=""
DEFAULT_SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET=""
DEFAULT_DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY=$(openssl rand -hex 32)
...
read -e -i "${SLACK_OAUTH_BRIDGE_CLIENT_SECRET:-$DEFAULT_SLACK_OAUTH_BRIDGE_CLIENT_SECRET}" -p "Enter slack-oauth-bridge's OAuth Client Secret: " SLACK_OAUTH_BRIDGE_CLIENT_SECRET
read -e -i "${SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET:-$DEFAULT_SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET}" -p "Enter spotify-oauth-bridge's OAuth Client Secret: " SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET
read -e -i "${DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY:-$DEFAULT_DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY}" -p "Enter dedicated-container-ingress-controller's Cookie Secret Key: " DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY
...
export SLACK_OAUTH_BRIDGE_CLIENT_SECRET
export SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET
export DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY
...
SLACK_OAUTH_BRIDGE_CLIENT_SECRET="$SLACK_OAUTH_BRIDGE_CLIENT_SECRET"
SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET="$SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET"
DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY="$DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY"
EOS
