...
DEFAULT_OAUTH2_PROXY_COOKIE_SECRET=$(dd if=/dev/urandom bs=256 count=1 2>/dev/null | tr -cd 'a-zA-Z0-9' | head -c 32)
...
DEFAULT_JUPYTERHUB_CONFIGPROXY_AUTH_TOKEN=$(dd if=/dev/urandom bs=256 count=1 2>/dev/null | tr -cd 'a-zA-Z0-9' | head -c 64)
DEFAULT_DOCKER_CONFIG_JSON="data:;base64,$(cat ~/.docker/config.json | base64 -w0)"
DEFAULT_SLACK_LOGGER_TIDB_SLACK_USER_PASSWORD=$(dd if=/dev/urandom bs=256 count=1 2>/dev/null | tr -cd 'a-zA-Z0-9' | head -c 64)
DEFAULT_DEVICE_FLOW_BRIDGE_CLIENT_SECRET=""
...
read -e -i "${DEVICE_FLOW_BRIDGE_CLIENT_SECRET:-$DEFAULT_DEVICE_FLOW_BRIDGE_CLIENT_SECRET}" -p "Enter device-flow-bridge's OAuth Client Secret: " DEVICE_FLOW_BRIDGE_CLIENT_SECRET
...
export DEVICE_FLOW_BRIDGE_CLIENT_SECRET
...
DEVICE_FLOW_BRIDGE_CLIENT_SECRET="${DEVICE_FLOW_BRIDGE_CLIENT_SECRET}"
EOS
