...
vault kv put kv/hippocampus-dev-device-flow-bridge CLIENT_SECRET="$DEVICE_FLOW_BRIDGE_CLIENT_SECRET"
...
vault policy write hippocampus-dev-device-flow-bridge - <<EOSS
path "kv/data/hippocampus-dev-device-flow-bridge" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/hippocampus-dev-device-flow-bridge.hippocampus-dev-device-flow-bridge policies=hippocampus-dev-device-flow-bridge bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h
