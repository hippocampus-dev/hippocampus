...
vault kv put kv/memory-bank OPENAI_API_KEY="$OPENAI_API_KEY"
...
vault kv put kv/github-actions-exporter-controller GITHUB_APP_PRIVATE_KEY="$HIPPOCAMPUS_DEV_GITHUB_APP_PRIVATE_KEY"
vault kv put kv/hippocampus-dev-github-token-server GITHUB_APP_PRIVATE_KEY="$HIPPOCAMPUS_DEV_GITHUB_APP_PRIVATE_KEY"
vault kv put kv/alerthandler GITHUB_TOKEN="$GITHUB_TOKEN"
vault kv put kv/hippocampus-dev-slack-oauth-bridge CLIENT_SECRET="$SLACK_OAUTH_BRIDGE_CLIENT_SECRET"
vault kv put kv/hippocampus-dev-spotify-oauth-bridge CLIENT_SECRET="$SPOTIFY_OAUTH_BRIDGE_CLIENT_SECRET"
vault kv put kv/dedicated-container-ingress-controller COOKIE_SECRET_KEY="$DEDICATED_CONTAINER_INGRESS_CONTROLLER_COOKIE_SECRET_KEY"
...
vault policy write memory-bank - <<EOSS
path "kv/data/memory-bank" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/memory-bank.memory-bank policies=memory-bank bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h
...
vault policy write github-actions-exporter-controller - <<EOSS
path "kv/data/github-actions-exporter-controller" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/github-actions-exporter-controller.github-actions-exporter-controller policies=github-actions-exporter-controller bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h
...
vault policy write alerthandler - <<EOSS
path "kv/data/alerthandler" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/alerthandler.alerthandler policies=alerthandler bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h

vault policy write hippocampus-dev-slack-oauth-bridge - <<EOSS
path "kv/data/hippocampus-dev-slack-oauth-bridge" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/hippocampus-dev-slack-oauth-bridge.hippocampus-dev-slack-oauth-bridge policies=hippocampus-dev-slack-oauth-bridge bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h

vault policy write hippocampus-dev-spotify-oauth-bridge - <<EOSS
path "kv/data/hippocampus-dev-spotify-oauth-bridge" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/hippocampus-dev-spotify-oauth-bridge.hippocampus-dev-spotify-oauth-bridge policies=hippocampus-dev-spotify-oauth-bridge bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h

vault policy write dedicated-container-ingress-controller - <<EOSS
path "kv/data/dedicated-container-ingress-controller" {
  capabilities = ["read"]
}
EOSS
vault write auth/kubernetes/role/dedicated-container-ingress-controller.dedicated-container-ingress-controller policies=dedicated-container-ingress-controller bound_service_account_names=${SERVICE_ACCOUNT_NAME} bound_service_account_namespaces=${SERVICE_ACCOUNT_NAMESPACE} ttl=1h
