...
2. Define the condition using expressions in the `when` field
3. Add corresponding template in `data.template.<template-name>`
4. Reference the template in the trigger's `send` field

#### Time-Based Conditions

Triggers support time-based conditions to suppress alerts during transient states (e.g., cluster bootstrapping). Use `time.Now().Sub(time.Parse(...)).Minutes()` to gate alerts based on resource age:

```
when: app.status.sync.status == 'Unknown' && time.Now().Sub(time.Parse(app.metadata.creationTimestamp)).Minutes() >= 10 && (none(app.status.conditions, {.type == 'ComparisonError'}) || any(app.status.conditions, {.type == 'ComparisonError' && .lastTransitionTime != nil && time.Now().Sub(time.Parse(.lastTransitionTime)).Minutes() >= 30}))
```

This delays the `on-sync-status-unknown` notification until 10 minutes after Application creation. If a `ComparisonError` condition exists (typical for repo-server issues such as transient SSH connection failures to GitHub), it must have persisted for at least 30 minutes before notifying. If no `ComparisonError` exists, the notification fires immediately (10-minute creation gate still applies), avoiding silent suppression for unexpected Unknown causes. Additionally, `oncePer: app.status.sync.status` prevents repeated notifications while the status remains Unknown.
