...
```
when: app.status.sync.status == 'Unknown' && time.Now().Sub(time.Parse(app.metadata.creationTimestamp)).Minutes() >= 10
```

This delays the `on-sync-status-unknown` notification until 10 minutes after Application creation, avoiding noise during initial cluster setup.
