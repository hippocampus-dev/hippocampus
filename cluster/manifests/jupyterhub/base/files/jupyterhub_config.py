...
c.ConfigurableHTTPProxy: jupyterhub.proxy.ConfigurableHTTPProxy = (
    c.ConfigurableHTTPProxy
)  # noqa
...
class HeaderAuthenticator(jupyterhub.auth.Authenticator):
    ...
    def authenticate(
        self, handler: tornado.web.RequestHandler, data: collections.abc.Mapping
    ):
        ...
        for init_container in self.init_containers:
            for volume_mount in init_container.get("volumeMounts", []):
                if "mountPath" in volume_mount:
                    volume_mount["mountPath"] = volume_mount["mountPath"].format(
                        **params
                    )
                if "subPath" in volume_mount:
                    volume_mount["subPath"] = volume_mount["subPath"].format(**params)
                if "readOnly" in volume_mount:
                    volume_mount["readOnly"] = _strtobool(
                        volume_mount["readOnly"].format(**params)
                    )
...
c.ConfigurableHTTPProxy.api_url = (
    "http://jupyterhub-proxy.jupyterhub.svc.cluster.local:8000"
)
...
c.JupyterHub.hub_connect_url = (
    f"http://jupyterhub-hub.jupyterhub.svc.cluster.local:{hub_container_port}"
)
