...
Image digests are updated automatically by the `snapshot-diff-server` workflow (`.github/workflows/00_snapshot-diff-server.yaml`), which builds `snapshot-diff-server` from the snapshot-controller source and updates digests in `base/kustomization.yaml`.
