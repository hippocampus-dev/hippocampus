...
├── custom_resource_state.yaml # Custom resource metrics configuration (configMapGenerator)
...
├── peer_authentication.yaml # Istio STRICT mTLS configuration
├── sidecar.yaml           # Istio sidecar egress configuration
├── telemetry.yaml         # Istio tracing, metrics, and access logging
└── patches/               # Environment-specific modifications
```

The manifests use the mirrored image `ghcr.io/hippocampus-dev/hippocampus/mirror/registry.k8s.io/kube-state-metrics/kube-state-metrics` with digest `sha256:53d0bbbb108f4922e26aae60e292ac2278be14dc2e4bde368e67aa530c8472eb`.
