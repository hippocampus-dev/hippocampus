...
func (p *plugin) Generate() (resmap.ResMap, error) {
	...
	for _, vaultSecret := range p.Spec.VaultSecrets {
		...
		if secret == nil {
			return nil, xerrors.New("failed to read secret")
		}

		data, ok := secret.Data["data"].(map[string]any)
		if !ok {
			return nil, xerrors.New("failed to cast secret data")
		}

		value, ok := data[key].(string)
		if !ok {
			return nil, xerrors.New("failed to cast secret value")
		}
...
func initializeVaultClient() (*api.Client, error) {
	...
	if !ok {
		return nil, xerrors.New("failed to lookup VAULT_ADDR")
	}
	...
	if !ok {
		...
		if !ok {
			return nil, xerrors.New("failed to lookup vault role")
		}
...
func getTokenFromVault(addr string, bearerToken string, role string) (string, error) {
	...
	response, err := http.Post(fmt.Sprintf("%s%s", addr, "/v1/auth/kubernetes/login"), "application/json", bytes.NewReader(b))
	if err != nil {
