...
- **Base Layer**: References official ArgoCD v2.14.15 manifests from GitHub
...
1. **Custom Kustomize Build**: The repo-server uses an init container to build Kustomize v5.5.0 with a custom Vault plugin (`secretsfromvault`). This plugin:
   ...
   - `ARGOCD_EXEC_TIMEOUT` is set to `3m` (default: 90s) to prevent timeout during cold-start kustomize builds when Redis cache is empty
...
- Redis authentication disabled (`argocd-redis` Secret with empty `auth` generated to satisfy upstream `REDIS_PASSWORD` references)
