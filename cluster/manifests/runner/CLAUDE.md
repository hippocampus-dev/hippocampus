...
- **overlays/dev/** - Development environment specific configurations
  ...
  - `exporter.yaml` - Exporter custom resource for GitHub Actions metrics
...
3. **Network Security**
   ...
   - Only allows Prometheus metrics collection (port 15020 for Istio sidecar merged metrics)
...
4. **Resource Configuration**
   - Runner spec defines:
     - Image: `ghcr.io/hippocampus-dev/hippocampus/runner`
     - Owner: `kaidotio`
     - Repo: `hippocampus`
...
6. **GitHub Actions Exporter**
   - Exporter CR placed in consumer namespace (same pattern as Runner CR)
   - Managed by github-actions-exporter-controller in its own namespace
   - Exposes GitHub Actions workflow metrics on port 8080
   - Uses Istio sidecar proxy for metrics collection

7. **Autoscaling Strategy**
   - HPA uses two external metrics from prometheus-adapter:
     - `github_actions_runs_queued`: Queued workflow runs (includes all runner types)
     - `github_actions_runners_busy`: Busy self-hosted runners filtered by `labels: "github-actions-runner-controller"`
   - The busy runners metric targets `value: "1"` — combined with `minReplicas: 1`, this ensures at least one idle runner is always available
   - Scale-up is aggressive (0s stabilization, max of 100% or 4 pods per 15s)
   - Scale-down is conservative (300s stabilization window)

### Dependencies

- **github-actions-runner-controller** - Manages Runner lifecycle
- **github-actions-exporter-controller** - Manages Exporter lifecycle and metrics collection
- **Istio** - Service mesh (ambient mode for runners, sidecar mode for exporters)
- **Vault** - Secret management
- **Kustomize** - Manifest templating
- **Kaniko** - Container image building (used by runner controller)
