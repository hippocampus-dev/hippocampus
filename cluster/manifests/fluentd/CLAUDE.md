...
3. **High Availability**:
   - HorizontalPodAutoscaler for aggregator and proxy (1-3 replicas each)
...
- **metrics.conf** - Prometheus metrics definitions and log-based monitoring (simple string-match patterns are migrated to Loki Ruler alerting rules; only patterns requiring JSON field access remain here)
