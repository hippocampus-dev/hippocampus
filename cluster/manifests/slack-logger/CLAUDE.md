...
- **Atlas Operator**: Automated schema migrations via `AtlasSchema` CRD (sync-wave `1`) with development database liveness monitoring. AtlasSchema must start after TidbInitializer (sync-wave `0`) completes, because Atlas Operator does not retry `VerifyingFirstRun` errors when the database is not yet ready

#### ArgoCD Sync-Wave Ordering

| Wave | Resource | Reason |
|------|----------|--------|
| `-2` | Kafka, KafkaNodePool | Kafka cluster must be ready before topics |
| `-1` | TidbCluster, KafkaTopic | Database cluster and topics before initialization |
| `0` | TidbInitializer | Database user/schema initialization must complete before Atlas connects |
| `1` | AtlasSchema | Atlas Operator does not retry VerifyingFirstRun errors; TiDB must be fully initialized |

#### TiCDC Access Pattern

TiDB Operator creates only headless peer services (`slack-logger-ticdc-peer`) for TiCDC, NOT ClusterIP services. To access TiCDC from Jobs or other pods, use the StatefulSet pod FQDN via the headless service:

```
http://slack-logger-ticdc-0.slack-logger-ticdc-peer.slack-logger.svc.cluster.local:8301
```

A NetworkPolicy must also allow ingress from the calling pod (e.g., changefeed job with label `app.kubernetes.io/component: ticdc-changefeed`) to TiCDC on port 8301.

#### Development Database Health Monitoring

The `AtlasSchema` CRD in development includes a liveness probe on the dev MySQL container that monitors for dirty database state:
- Checks if there are tables in the dev database
- Verifies there are no active connections (besides the probe itself)
- If tables exist with no active connections, the container is restarted
- This prevents permanent Degraded state from stale dev database artifacts

The liveness probe configuration:
- `initialDelaySeconds: 120` - Wait for initial migrations to complete
- `periodSeconds: 30` - Check every 30 seconds
- `failureThreshold: 3` - Restart after 3 consecutive failures (90 seconds)

This mechanism prevents cascading failures where a dirty dev database causes all schema validation retries to fail.
...
4. **Building Container**:
   - Multi-stage Dockerfile using Go 1.25 and distroless base
