...
4. **Ext-Proc Proxy** (dev overlay):
   ...
   - EnvoyFilter configures `SIDECAR_INBOUND` with two HTTP filters:
     ...
     - `envoy.filters.http.lua`: Saves original Host as `X-Original-Host` header, then rewrites Host to `embedding-gateway-varnish` for Envoy sidecar routing (Varnish restores the original Host in `vcl_backend_fetch`)
...
5. **Varnish Cache Layer** (dev overlay):
   ...
   - Cache key: `vcl_hash` uses `X-Original-Host` (set by ext-proc-proxy Lua filter) for the host component of the cache key, falling back to `req.http.host` if absent
   - Host header restore: `vcl_backend_fetch` restores `Host` from `X-Original-Host` (with `:80`/`:443` port suffix stripped via `regsub`) for correct Envoy sidecar routing to the backend, falling back to `embedding-gateway-backend` if `X-Original-Host` is absent
