...
4. **Varnish Cache Layer** (dev overlay):
   ...
   - Request body buffered up to 1MB for POST caching
   - Host header rewrite: Varnish sets `Host: embedding-gateway-backend:8080` in `vcl_backend_fetch` to ensure Envoy sidecar routes to the correct backend (original request Host header would cause routing failures)
   - Cache invalidation (Fastly-compatible):
     - `PURGE /path`: Invalidate specific URL
     - `BAN` with `X-Purge-All: true`: Clear all cache
     - `BAN` with `Surrogate-Key: tag`: Invalidate by tag (requires backend to set `Surrogate-Key` header)
   - Access control (AuthorizationPolicy `varnish` with multiple rules):
     - Rule 1: Allows all methods except PURGE/BAN from `istio-ingressgateway` (using `notMethods`)
     - Rule 2: Allows only PURGE/BAN from `endpoint-broadcaster`
...
5. **Endpoint Broadcaster** (dev overlay):
   ...
   - Purpose: Broadcasts BAN requests to all Varnish pods for bulk cache invalidation
   - Target: `embedding-gateway-varnish-headless` service (headless service for direct pod access)
   - Access control: Only PURGE/BAN methods are allowed from `istio-ingressgateway`
   - External access:
...
| -2 | `minio/`, `varnish/`, `endpoint-broadcaster/` | Infrastructure dependencies |
