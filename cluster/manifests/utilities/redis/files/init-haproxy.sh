#!/usr/bin/env bash

set -Eeo pipefail
trap 'echo "exit $?: $BASH_COMMAND(line $LINENO)" >&2' ERR

HAPROXY_CONF=/usr/local/etc/haproxy/haproxy.cfg
cp /mnt/haproxy.cfg "$HAPROXY_CONF"

echo >> "$HAPROXY_CONF"
echo >> "$HAPROXY_CONF"

cat <<EOS >> "$HAPROXY_CONF"
frontend redis_master
  bind *:6380
  use_backend redis_master

backend redis_master
  mode tcp
  option tcp-check
  tcp-check connect
  tcp-check send PING\r\n
  tcp-check expect string +PONG
  tcp-check send info\ replication\r\n
  tcp-check expect string role:master
  tcp-check expect rstring connected_slaves:[$((QUORUM - 1))-9]([0-9]*)?
  tcp-check send QUIT\r\n
  tcp-check expect string +OK
EOS
for i in $(seq 0 $((REDIS_REPLICAS - 1))); do
  while ! getent hosts ${SERVICE_NAME}-${i}.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local; do
    sleep 1
  done
  cat <<EOS >> "$HAPROXY_CONF"
  server R${i} ${SERVICE_NAME}-${i}.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local:6379 check inter 1s fall 1 rise 1 on-error mark-down resolvers kube-dns resolve-prefer ipv4 init-addr none
EOS
done

if [ "$REDIS_REPLICAS" -gt 1 ]; then
  SLAVE_ROLE_CHECK=$'\n  tcp-check expect string role:slave'
else
  SLAVE_ROLE_CHECK=""
fi

cat <<EOS >> "$HAPROXY_CONF"
frontend redis_slave
  bind *:6381
  use_backend redis_slave

backend redis_slave
  mode tcp
  option tcp-check
  tcp-check connect
  tcp-check send PING\r\n
  tcp-check expect string +PONG
  tcp-check send info\ replication\r\n$SLAVE_ROLE_CHECK
  tcp-check send QUIT\r\n
  tcp-check expect string +OK
EOS
for i in $(seq 0 $((REDIS_REPLICAS - 1))); do
  while ! getent hosts ${SERVICE_NAME}-${i}.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local; do
    sleep 1
  done
  cat <<EOS >> "$HAPROXY_CONF"
  server R${i} ${SERVICE_NAME}-${i}.${SERVICE_NAME}.${NAMESPACE}.svc.cluster.local:6379 check inter 1s fall 1 rise 1 on-error mark-down resolvers kube-dns resolve-prefer ipv4 init-addr none
EOS
done
