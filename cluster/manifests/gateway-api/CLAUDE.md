...
The manifests deploy Gateway API v1.5.0 CRDs (experimental channel) to enable advanced traffic management capabilities across the cluster, including ListenerSet, TCPRoute, and UDPRoute support.
...
│   └── kustomization.yaml  # References upstream Gateway API experimental install
└── overlays/
...
- **Experimental Channel**: Uses the experimental install which includes ListenerSet, TCPRoute, and UDPRoute in addition to stable resources
- **Version**: Currently pinned to v1.5.0 of Gateway API
