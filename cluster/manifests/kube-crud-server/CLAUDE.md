...
This directory contains Kustomize-based Kubernetes manifests for deploying the kube-crud-server application. The manifests are organized using Kustomize overlays to support different environments.
...
- **base/** - Base Kustomization that references the application manifests
  - Points to `../../../applications/kube-crud-server/manifests`
...
3. **Audit Logging**:
   ...
   - Audit logs are written to `/var/log/audit/audit.log` and automatically uploaded to `kube-crud-server-audit-logs` bucket
   - Shared volume (emptyDir) for audit log exchange between kube-crud-server and fluent-bit containers
...
1. Base manifests should be edited in `/opt/hippocampus/cluster/applications/kube-crud-server/manifests/`
...
- Application source code: `/opt/hippocampus/cluster/applications/kube-crud-server/`
- Application CLAUDE.md with development commands: `/opt/hippocampus/cluster/applications/kube-crud-server/CLAUDE.md`