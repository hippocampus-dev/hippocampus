...
**Sharding**:
- Prometheus uses hashmod-based sharding to distribute targets across replicas
- `SHARD_COUNT` is automatically derived from `spec.replicas` via Kustomize replacements
- Each replica scrapes targets where `hash(address) % SHARD_COUNT == SHARD_ID`
