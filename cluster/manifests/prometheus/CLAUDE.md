...
- Resource requests/limits: 30m-2000m CPU, 1Gi-5Gi memory

**Sharding with Replication**:
- Prometheus uses hashmod-based sharding to distribute targets across shards
- Each shard has multiple replicas for high availability
- Formula: `TOTAL_REPLICAS = SHARD_COUNT x REPLICA_COUNT` (e.g., 4 pods = 2 shards x 2 replicas)
- Pod distribution: `SHARD_ID = ordinal % SHARD_COUNT`, `REPLICA_ID = ordinal / SHARD_COUNT`
  - Example with 4 pods: prometheus-0/2 scrape shard 0, prometheus-1/3 scrape shard 1
- External labels: `prometheus_shard` and `prometheus_replica` identify each instance
- Mimir deduplication handles duplicate samples from replicas of the same shard
...
- Uses mirrored container images from `ghcr.io/hippocampus-dev/hippocampus/mirror/`
