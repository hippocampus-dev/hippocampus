...
sub vcl_init {
    new external = dynamic.director(port = "80");
}
...
sub vcl_hash {
    ...
    # custom: use original host for cache key (Host is rewritten by ext-proc-proxy)
    hash_data(req.url);
...
sub vcl_recv {
    if (req.http.X-Original-Host) {
        set req.http.X-Original-Host = regsub(req.http.X-Original-Host, ":80$", "");
        set req.backend_hint = external.backend(req.http.X-Original-Host);
    }
    ...
    # custom: skip for api.openai.com (does not return user-specific content)
    if (req.http.Authorization && req.http.X-Original-Host != "api.openai.com") {
        return (pass);
    }
...
sub vcl_backend_response {
    ...
    # custom: skip for api.openai.com (Cloudflare bot management cookies)
    if (beresp.http.Set-Cookie && bereq.http.X-Original-Host != "api.openai.com") {
        set beresp.ttl = 120s;
        set beresp.uncacheable = true;
        return (deliver);
    }
