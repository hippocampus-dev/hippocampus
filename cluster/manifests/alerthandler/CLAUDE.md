...
2. **allow-knative-serving** - Permits:
   - cluster-local-gateway for direct routing when Pod is running
   - activator for scale-from-zero behavior
   - autoscaler for metrics collection
   - mimir-alertmanager for alert webhook delivery
...
2. **Sidecar** - REGISTRY_ONLY egress policy, allowing access to:
   ...
   - api.github.com (for CriticalAlertHandler GitHub issue creation via GITHUB_TOKEN environment variable)
