...
### Activator NetworkPolicy

The activator NetworkPolicy allows all Istio mesh pods (`security.istio.io/tlsMode: istio`) to reach the activator. This is required because:

1. When a Knative Service scales to zero, the VirtualService routes traffic to the activator
2. The activator buffers requests while scaling up the service
3. Any mesh pod may need to call a Knative Service, so the activator must accept traffic from all mesh sources
...
- **HPA**: CPU-based autoscaling (1-3 replicas, 80% CPU target)
