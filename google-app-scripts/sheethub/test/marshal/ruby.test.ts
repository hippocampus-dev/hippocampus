...
describe("dump", () => {
  ...
  describe("fixnum", () => {
    ...
    test("zero", () => {
      ...
      expect(dump(n)).toStrictEqual(
        `${majorVersionCode}${minorVersionCode}${typeFixnumCode}${String.fromCharCode(0)}`,
      );
    });
    ...
    test("short", () => {
      ...
      expect(dump(n)).toStrictEqual(
        `${majorVersionCode}${minorVersionCode}${typeFixnumCode}${String.fromCharCode(n + OFFSET)}`,
      );
    });
    ...
    test("long", () => {
      ...
      expect(dump(n)).toStrictEqual(
        `${majorVersionCode}${minorVersionCode}${typeFixnumCode}${String.fromCharCode(index)}${Array.from(
          long.subarray(0, index),
        )
  ...
  describe("string", () => {
    ...
    test("short", () => {
      ...
      expect(dump(s)).toStrictEqual(
        `${majorVersionCode}${minorVersionCode}${typeStringCode}${length}${s}`,
      );
    });
    ...
    test("long", () => {
      ...
      const index =
        ...
        Array.from(long)
          .reverse()
          .findIndex((value) => value !== 0);
      const length = Array.from(long.subarray(0, index))
        .map((value) => String.fromCharCode(value))
        .join("");

      expect(dump(s)).toStrictEqual(
        `${majorVersionCode}${minorVersionCode}${typeStringCode}${String.fromCharCode(index)}${length}${s}`,
      );
    });
    ...
    test("multibyte", () => {
      ...
      const length = String.fromCharCode(
        utf8ByteLength("テ".codePointAt(0)!) +
          utf8ByteLength("ス".codePointAt(0)!) +
          utf8ByteLength("ト".codePointAt(0)!) +
          5,
      );
...
describe("load", () => {
  ...
  describe("string", () => {
    ...
    test("short", () => {
      ...
      expect(
        load(
          `${majorVersionCode}${minorVersionCode}${typeStringCode}${String.fromCharCode(length)}${s}`,
        ),
      ).toStrictEqual(s);
    });
