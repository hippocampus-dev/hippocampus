...
class GitClient {
  ...
  checkout(branch: string): GitHubRef {
    const result = UrlFetchApp.fetch(
      `https://api.github.com/repos/${this.repository}/git/refs/heads/${branch}`,
      {
        method: "get",
        headers: {
  ...
  add(path: string, content: string, mode: string = "100644") {
    const result = UrlFetchApp.fetch(
      `https://api.github.com/repos/${this.repository}/git/blobs`,
      {
        method: "post",
        headers: {
          Accept: "application/vnd.github+json",
  ...
  commit(message: string, sha: string): GitHubCommit {
    ...
    const treesResult = UrlFetchApp.fetch(
      `https://api.github.com/repos/${this.repository}/git/trees`,
      {
        method: "post",
        headers: {
          Accept: "application/vnd.github+json",
    ...
    const commitResult = UrlFetchApp.fetch(
      `https://api.github.com/repos/${this.repository}/git/commits`,
      {
        method: "post",
        headers: {
          Accept: "application/vnd.github+json",
  ...
  push(branch: string, sha: string): GitHubRef {
    ...
    if (response.getResponseCode() === 200) {
      ...
      const result = UrlFetchApp.fetch(
        `https://api.github.com/repos/${this.repository}/git/refs`,
        {
          method: "post",
          headers: {
            Accept: "application/vnd.github+json",
...
const render = (
  ...
  sheet
    ...
    .clearContent();
  sheet
    .getRange(cursor, 1, sheet.getMaxRows() - cursor, sheet.getMaxColumns())
    .clearContent();
};

export const customOnOpen = () => {
  const repository =
    PropertiesService.getScriptProperties().getProperty("repository")!;
  const token = PropertiesService.getScriptProperties().getProperty("token")!;

  const masterKey =
    PropertiesService.getScriptProperties().getProperty("master.key");
  ...
  switch (e.changeType) {
    case "INSERT_ROW":
      saveHistory(
        ...
        e.source.getActiveRange().getRowIndex(),
        {
...
export const sync = () => {
  ...
  keys.forEach((sheet) => {
    const rowHistory: RowHistory = JSON.parse(
      documentProperties.getProperty(sheet),
    );
