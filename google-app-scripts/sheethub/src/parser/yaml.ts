...
export const parseYAMLDocument = (yaml: string, path?: string): any[][][] => {
  ...
  const rows: any[][][] = [
    [commentHeader, ...headers.map((header) => [header, undefined])],
  ];
...
export const stringifyYAMLDocument = (
  rows: any[][][],
  path?: string,
): string => {
  const _commentHeader = rows[0][0];
  const headers = rows[0].slice(1);

  const data = new YAMLSeq();
  rows.slice(1).forEach((row) => {
    const o: YAMLMap = new YAMLMap();
    ...
    headers.forEach(([header, _], i) => {
      ...
      if (
        ...
        document.contents instanceof YAMLSeq
      ) {
        v.set(key, document);
      } else {
