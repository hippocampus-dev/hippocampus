...
export const doGet = (e: GoogleAppsScript.Events.DoGet) => {
  ...
  switch (format) {
    case "json":
      return ContentService.createTextOutput(stringifyJSON(rows)).setMimeType(
        ContentService.MimeType.TEXT,
      );
    case "yaml":
      return ContentService.createTextOutput(stringifyYAML(rows)).setMimeType(
        ContentService.MimeType.TEXT,
      );
    default:
      return ContentService.createTextOutput(
        stringifyRFC4180(rows),
