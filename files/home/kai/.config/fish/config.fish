...
test -e ~/.config/gcloud/configurations/config_default || gcloud auth login
test -s ~/.config/gcloud/configurations/config_default || gcloud config set project rich-gift-314809
test -s ~/.config/gcloud/application_default_credentials.json || gcloud auth application-default login
test -s ~/.config/.wrangler/config/default.toml || CLOUDFLARE_API_TOKEN="" npx wrangler login
GITHUB_TOKEN="" gh auth status > /dev/null 2>&1 || GITHUB_TOKEN="" gh auth login
if ! systemctl status cloudflared > /dev/null 2>&1
  echo (set_color red)"cloudflared service is not running"(set_color normal)
end
if test -z $GITHUB_TOKEN
  echo (set_color red)"GITHUB_TOKEN is not set"(set_color normal)
else
  for tool_name in hippocampus taurin
    if ! asdf where $tool_name > /dev/null 2>&1
      asdf install $tool_name
    end
  end
