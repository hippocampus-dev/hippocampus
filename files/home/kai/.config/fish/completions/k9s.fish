...
complete -c k9s -x -l cluster -d 'The name of the kubeconfig cluster to use' -a '(__fish_kubectl config get-clusters --no-headers | awk \'{print $1}\')'
complete -c k9s -x -s c -l command -d 'Specify the default command to view when the application launches'
complete -c k9s -x -l context -d 'The name of the kubeconfig context to use' -a '(__fish_kubectl config get-contexts --no-headers | awk \'{print $(NF-2)}\')'
...
complete -c k9s -x -s n -l namespace -d 'If present, the namespace scope for this CLI request' -a '(__fish_kubectl get namespace --no-headers | awk \'{print $1}\')'
