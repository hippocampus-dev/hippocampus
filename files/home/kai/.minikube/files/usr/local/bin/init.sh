...
[ -f /var/run/crio/version ] && cp /var/run/crio/version ${CACHE_DIRECTORY}/var/run/crio/version
mkdir -p ${CACHE_DIRECTORY}/var/lib/crio
[ -f /var/lib/crio/version ] && cp /var/lib/crio/version ${CACHE_DIRECTORY}/var/lib/crio/version
...
while IFS= read -r file; do
  ...
  cp ${file} ${target}
done < <(find ${CACHE_DIRECTORY} -type f)
