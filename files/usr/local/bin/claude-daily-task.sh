...
#Before adjusting any workload, check git log for the past 7 days for the specific manifest file. If the same container in the same manifest file had its resource requests or limits adjusted within the past 7 days, skip it unless the workload currently has a Pod in OOMKilled or CrashLoopBackOff state, observable via kubectl get pod -o yaml (check container.lastState.terminated.reason for OOMKilled within the past 24h).
#Because workloads tend to be over-provisioned, bias request adjustments toward the lower bound of the past 1 day observed usage, and only add headroom justified by max_over_time memory spikes or by Pods currently in OOMKilled or CrashLoopBackOff state.
...
#Skip any adjustment whose new value is within +/-30% of the current value (i.e., 0.7 <= new/current <= 1.3) to avoid minor tweaks. If any container is over-provisioned or under-provisioned beyond that range, fix the manifests in this repository.
...
#${log_format}
#
#Log path: ${HOME}/brain/report/${today}/tasks/resources.md
#EOS
#)" &
#pids+=($!)
