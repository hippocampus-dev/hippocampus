...
RESOURCE_OPTION="--extra-config=kubelet.max-pods=500 --cpus=8 --memory=64g --disk-size=300g --extra-config=kubelet.system-reserved=cpu=500m,memory=1Gi,ephemeral-storage=10Gi,pid=1000 --extra-config=kubelet.container-log-max-size=1Mi --extra-config=kubelet.container-log-max-files=2"
