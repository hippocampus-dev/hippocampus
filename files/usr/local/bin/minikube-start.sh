...
# NOTE: OIDC discovery advertises the node IP by default, which REGISTRY_ONLY sidecars blackhole
SERVICE_ACCOUNT_OPTION="--extra-config=apiserver.service-account-jwks-uri=https://kubernetes.default.svc.cluster.local/openid/v1/jwks"

OPTION="--vm-driver=kvm2 $RUNTIME_OPTION $CNI_OPTION $FEATURE_GATE_OPTION $AUDIT_OPTION $EVICTION_OPTION $IMAGE_PULL_OPTION $RESOURCE_OPTION $ENCRYPTION_OPTION $APF_OPTIONS $DISABLE_LEADER_ELECT_OPTION $IMAGE_GC_OPTION $POD_GC_OPTION $SCRAPE_OPTION $SERVICE_ACCOUNT_OPTION --extra-config=kubelet.housekeeping-interval=15s --iso-url=file:///home/${USER}/.minikube/x86_64.iso"

mkdir -p ~/.minikube/machines
# NOTE: Isolate the machine dir in its own subvolume so backup.sh's root snapshot
# skips the rawdisk (a snapshot forces CoW even on NoCow files, filling the pool).
# A machine dir means a rawdisk is at stake; minikube delete leaves only its certs.
machine_directories=$(find ~/.minikube/machines -mindepth 1 -maxdepth 1 -type d)
if [ "$(stat -c %i ~/.minikube/machines)" != "256" ] && [ -z "$machine_directories" ]; then
  [ -d ~/.minikube/machines.new ] || btrfs subvolume create ~/.minikube/machines.new
  find ~/.minikube/machines -mindepth 1 -maxdepth 1 -exec mv -t ~/.minikube/machines.new {} +
  rmdir ~/.minikube/machines
  mv ~/.minikube/machines.new ~/.minikube/machines
fi
# NOTE: Disable btrfs CoW so rawdisk inherits NoCow.
chattr +C ~/.minikube/machines

started_at=$(date +%s)
# NOTE: started_at is reset by the recovery path below; this is TimeoutStartSec=1800 less 100s of slack
deadline=$((started_at + 1700))
...
if minikube ssh -- "sudo grep -q '^maxParallelImagePulls:' /var/lib/kubelet/config.yaml" 2>/dev/null; then
  minikube ssh -- "sudo sed -i 's/^maxParallelImagePulls:.*/maxParallelImagePulls: 5/' /var/lib/kubelet/config.yaml"
else
  minikube ssh -- "echo 'maxParallelImagePulls: 5' | sudo tee -a /var/lib/kubelet/config.yaml > /dev/null"
fi
...
EOS
  ...
  sudo systemctl set-property --runtime "$scope" MemorySwapMax=0
fi
...
# Recreate any knative-kafka dispatcher pod still on the emptyDir "contract-resources" volume (a cold-boot
# race where it started before the pod-defaulting webhook wired the ConfigMap), so the webhook re-wires it.
if minikube kubectl -- get deployment kafka-controller -n knative-eventing > /dev/null 2>&1; then
  # kafka-webhook-eventing settles last, so wait for it before deleting the pods it has yet to wire
  remaining=$((deadline - $(date +%s)))
  if [ "$remaining" -gt 0 ] && minikube kubectl -- wait --for=condition=Available deployment/kafka-controller deployment/kafka-webhook-eventing -n knative-eventing --timeout="${remaining}s" 2> /dev/null; then
    for name in kafka-source-dispatcher kafka-broker-dispatcher kafka-channel-dispatcher; do
      empty=$(minikube kubectl -- get pod "${name}-0" -n knative-eventing -o jsonpath='{.spec.volumes[?(@.name=="contract-resources")].emptyDir}' 2> /dev/null || true)
      if [ -n "$empty" ]; then
        minikube kubectl -- delete pod "${name}-0" -n knative-eventing --wait=false 2> /dev/null || true
      fi
    done
  fi
fi
