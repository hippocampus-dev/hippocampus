...
if minikube ssh -- "sudo journalctl -u containerd -b --since=@${started_at} --no-pager | grep -qE 'failed to (rename|mount).*snapshot'" 2>/dev/null; then
...
EOS
  ...
  sudo systemctl set-property --runtime "$scope" MemoryLow=64G
fi
