...
minikube ssh -- "sudo sed -i 's/^shutdownGracePeriod: .*/shutdownGracePeriod: 120s/' /var/lib/kubelet/config.yaml && sudo sed -i 's/^shutdownGracePeriodCriticalPods: .*/shutdownGracePeriodCriticalPods: 30s/' /var/lib/kubelet/config.yaml && sudo systemctl restart kubelet"
...
if minikube ssh -- "sudo journalctl -u containerd -b --no-pager | grep -qE 'failed to (rename|mount).*snapshot'" 2>/dev/null; then
  ...
  minikube ssh -- "sudo systemctl start containerd && sudo systemctl start kubelet"
fi
