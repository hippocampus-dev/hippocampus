...
if minikube ssh -- "sudo journalctl -u containerd -b --no-pager | grep -qE 'failed to (rename|mount).*snapshot'" 2>/dev/null; then
  minikube ssh -- "sudo systemctl stop kubelet && sudo systemctl stop containerd && sudo pkill -9 -f containerd-shim-runc-v2 && sudo rm -f /var/lib/containerd/io.containerd.metadata.v1.bolt/meta.db && sudo rm -rf /var/lib/containerd/io.containerd.snapshotter.v1.overlayfs/*"
