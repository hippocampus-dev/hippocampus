...
RESOURCE_OPTION="--extra-config=kubelet.max-pods=300 --cpus=16 --memory=56g --disk-size=200g --extra-config=kubelet.system-reserved=cpu=500m,memory=1Gi,ephemeral-storage=10Gi,pid=1000 --extra-config=kubelet.container-log-max-size=1Mi --extra-config=kubelet.container-log-max-files=2"
...
# workaround: etcd extra-config causes invalid kubeadm.yaml on minikube v1.38.1
#ETCD_SCRAPE_OPTION="--extra-config=etcd.listen-metrics-urls=http://0.0.0.0:2381"
# https://github.com/etcd-io/etcd/blob/0f5cee16488816ad9e9ecb7cbdfd2aa74b163aeb/server/storage/quota.go#L33
# https://github.com/etcd-io/etcd/blob/0f5cee16488816ad9e9ecb7cbdfd2aa74b163aeb/server/etcdserver/api/v3rpc/quota.go#L41
#ETCD_QUOTA_OPTION="--extra-config=etcd.quota-backend-bytes=8589934592"
...
if minikube ssh -- "sudo journalctl -u containerd -b --no-pager | grep -qE 'failed to (rename|mount).*snapshot'" 2>/dev/null; then
  minikube ssh -- "sudo systemctl stop kubelet && sudo systemctl stop containerd && sudo rm -f /var/lib/containerd/io.containerd.metadata.v1.bolt/meta.db && sudo rm -rf /var/lib/containerd/io.containerd.snapshotter.v1.overlayfs/*"
...
# Wait for API server to be ready after kubelet restart
