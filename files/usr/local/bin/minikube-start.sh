...
if [ "$(stat -c %i ~/.minikube/machines 2>/dev/null || echo 0)" != "256" ] && [ -z "$(ls -A ~/.minikube/machines 2>/dev/null)" ]; then
  rm -rf ~/.minikube/machines
  btrfs subvolume create ~/.minikube/machines
fi
...
if minikube kubectl -- get deployment kafka-controller -n knative-eventing > /dev/null 2>&1; then
  minikube kubectl -- wait --for=condition=Available deployment/kafka-controller deployment/kafka-webhook-eventing -n knative-eventing --timeout=180s 2> /dev/null || true
