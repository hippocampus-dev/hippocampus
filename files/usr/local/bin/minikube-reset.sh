...
minikube kubectl -- get mutatingwebhookconfiguration -l release!=istio -o name \
  | awk '!/\/pods\.defaulting\.webhook\.kafka\.eventing\.knative\.dev$/' \
  | xargs -r minikube kubectl -- delete --ignore-not-found
