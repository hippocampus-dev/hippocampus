...
function extractTrustedSection(header: string): string | null {
  ...
  const afterAuthservId = normalized
    .slice(TRUSTED_AUTHSERV_ID.length)
    .trimStart();
...
function extractEmailAddress(from: string): string {
  ...
  return (match !== null ? match[1] : from).toLowerCase().trim();
}
...
function parseQuotedText(body: string): Message[] {
  ...
  for (const line of lines) {
    ...
    const depth = match !== null ? (match[0].match(/>/g) ?? []).length : 0;
    const content = match !== null ? line.slice(match[0].length) : line;
  ...
  for (let depth = maxDepth; depth >= 0; depth--) {
    ...
    const role: "user" | "assistant" =
      stepsFromMax % 2 === 0 ? "user" : "assistant";
...
export default {
  async email(
    ...
    try {
      ...
      const aiOutput = await environment.AI.run(
      ...
      await sendReply(
        ...
        environment,
        subject,
        aiOutput.response ?? "Please try again later.",
        body,
      );
    } catch (error) {
