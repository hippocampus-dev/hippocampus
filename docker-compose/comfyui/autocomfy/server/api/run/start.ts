...
import { getComfyUIWebSocketUrl, queuePrompt } from "@/lib/comfyui";
...
export default defineEventHandler(async (event) => {
  ...
  const targetCount =
    mode === "count" ? Math.max(1, Number(body.count) || 1) : 0;
  ...
  runLoop(prompt, seedControls, concept, topologyHash, controller.signal).catch(
    (error) => {
  ...
  try {
    while (true) {
      ...
      try {
        ...
        if (concept) {
          try {
            iterationPrompt = await generateAndInjectPrompts(
              prompt,
              concept,
              topologyHash,
              signal,
            ...
            broadcast({
              type: "error",
              data: { message: `Prompt generation failed: ${message}` },
            });
          }
        }
        const effectivePrompt = applySeedControls(
          iterationPrompt,
          seedControls,
        );
        const { prompt_id } = await queuePrompt(effectivePrompt, signal);
    ...
    const onMessage = (data: WebSocket.Data) => {
      try {
        ...
        if (
          ...
          reject(
            new Error(
              message.data?.exception_message || "ComfyUI execution error",
            ),
          );
        }
      } catch {}
