...
export async function convertUIToAPI(
  ...
  function resolveLink(
    ...
    | { type: "node"; nodeId: number; slot: number }
    ...
    while (true) {
      ...
      return {
        type: "node",
        nodeId: linkInfo.sourceNodeId,
        slot: linkInfo.sourceSlot,
      };
    }
  }
      ...
      for (const input of node.inputs) {
        if (input.link !== null) {
          ...
          if (resolved.type === "value") {
            ...
            inputs[input.name] = [String(resolved.nodeId), resolved.slot];
          }
        }
      }
    }
