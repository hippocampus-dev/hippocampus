...
export const TOTAL_WHITE_KEYS = whiteCount;
export const WHITE_KEY_WIDTH = 100 / TOTAL_WHITE_KEYS;
export const WHITE_KEY_UNIT = 1 / TOTAL_WHITE_KEYS;
export const BLACK_KEY_UNIT = WHITE_KEY_UNIT * 0.6;

let whiteIndex = 0;
for (let midi = PIANO_LOW; midi <= PIANO_HIGH; midi++) {
  if (isBlackKey(midi)) {
    BLACK_KEYS.push({ midiNote: midi, whiteIndex });
    KEY_POSITIONS[midi - PIANO_LOW] = {
      x: whiteIndex * WHITE_KEY_UNIT - BLACK_KEY_UNIT / 2,
      w: BLACK_KEY_UNIT,
    };
  } else {
    WHITE_KEYS.push({ midiNote: midi, index: whiteIndex });
    KEY_POSITIONS[midi - PIANO_LOW] = {
