...
function lowFrequencyCompensation(midiNote: number): number {
  const reference = 60;
  if (midiNote >= reference) return 1;
  return 1 + (reference - midiNote) * 0.08;
}
...
export class AudioEngine {
  ...
  private stopImmediately(active: ActiveNote): void {
    ...
    for (const node of active.intermediateGains) {
      node.disconnect();
    }
  ...
  noteOn(midiNote: number, velocity = 0.7): void {
    const existing = this.activeNotes.get(midiNote);
    if (existing) {
      this.stopImmediately(existing);
    ...
    const compensation = lowFrequencyCompensation(midiNote);
    const peak = 0.3 * velocity * compensation;
    const sustain = 0.08 * velocity * compensation;
