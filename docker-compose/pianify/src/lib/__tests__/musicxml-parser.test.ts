...
describe("parseMusicXml", () => {
  ...
  it("parses notes with correct pitches", () => {
    const result = parseMusicXml(SIMPLE_SCORE);
    const rightHandNotes = result.notes.filter((n) => n.staff === 1);
  ...
  it("parses left hand notes", () => {
    const result = parseMusicXml(SIMPLE_SCORE);
    const leftHandNotes = result.notes.filter((n) => n.staff === 2);
...
describe("getActiveNotes", () => {
  ...
  it("returns tied continuation note at tie boundary", () => {
    const result = parseMusicXml(TIE_SCORE);
    const notes = result.notes;
    const boundaryMs = notes[0].startMs + notes[0].durationMs;
    const active = getActiveNotes(notes, boundaryMs);
    expect(active).toHaveLength(1);
    expect(active[0].tied).toBe(true);
  });

  it("returns first note before tie boundary", () => {
    const result = parseMusicXml(TIE_SCORE);
    const notes = result.notes;
    const beforeBoundaryMs = notes[0].startMs + notes[0].durationMs - 1;
    const active = getActiveNotes(notes, beforeBoundaryMs);
    expect(active).toHaveLength(1);
    expect(active[0].tied).toBe(false);
  });
});
