  ...
  useEffect(() => {
    ...
    osmd
      ...
      .then(() => {
        ...
        const svgWidth =
          svg?.viewBox?.baseVal?.width || svg?.width?.baseVal?.value || endX;
        const svgHeight =
          svg?.viewBox?.baseVal?.height || svg?.height?.baseVal?.value || 0;

        measureLayoutRef.current = { positions, endX, svgWidth, svgHeight };
        cursor.hide();
      })
      .catch((error) => {
        if (!cancelled) {
