...
- `pianify` - Pianify OMR pipeline (sheet music to MusicXML via HOMR + optional MIDI pitch correction)
- `yue` - Yue server
...
3. **Observability Stack**:
   ...
   - OpenTelemetry Collector (contrib) - Telemetry pipeline for traces (→ Tempo), metrics (→ Prometheus on :8889), and logs (→ Loki). Accessed via Envoy at https://opentelemetry-collector.127.0.0.1.nip.io, requires mkcert CA trust
...
1. **Service-Specific Directories**: Each service has its own directory under `docker-compose/` containing service-specific files (e.g., `n8n/init.sh` for automated owner setup and workflow import via REST API, `n8n/workflows/` for declarative workflow JSON definitions, `yue/entrypoint.sh`, `envoy/envoy.yaml`, `comfyui/autocomfy/` for the automated workflow execution and result labeling web UI)
...
1. **Testing AI Models**:
   ...
   # autocomfy web UI (TanStack Start) - automated workflow execution and result labeling
   # Runs as part of the comfyui profile at http://autocomfy.127.0.0.1.nip.io (via Envoy)
   # Or run standalone:
   cd docker-compose/comfyui/autocomfy
