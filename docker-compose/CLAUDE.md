...
1. **Service-Specific Directories**: Each service has its own directory under `docker-compose/` containing service-specific files (e.g., `n8n/init.sh`, `yue/entrypoint.sh`, `envoy/envoy.yaml`, `comfyui/autocomfy/` for the automated workflow execution and result labeling web UI)
