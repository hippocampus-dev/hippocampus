...
1. **Service-Specific Directories**: Each service has its own directory under `docker-compose/` containing service-specific files (e.g., `n8n/init.sh`, `yue/entrypoint.sh`, `envoy/envoy.yaml`, `comfyui/promptgen/` for the prompt generation web UI)
...
1. **Testing AI Models**:
   ...
   # promptgen web UI (TanStack Start) - generates LLM prompts and submits to ComfyUI
   # Runs as part of the comfyui profile at http://localhost:3001
   # Or run standalone:
   cd docker-compose/comfyui/promptgen
   npm ci && npm run dev  # Dev server at http://localhost:3001
   ```
