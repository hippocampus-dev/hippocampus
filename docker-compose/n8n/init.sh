...
token=$(jq -n --arg email "$N8N_OWNER_EMAIL" --arg password "$N8N_OWNER_PASSWORD" '{
  ...
  password: $password
}' | curl -fsSL -D - -o /dev/null -XPOST -H "Content-Type: application/json" "${N8N_BASE}/rest/login" -d @- | tr -d '\r' | sed -n 's/.*n8n-auth=\([^;]*\).*/\1/p')
...
existing_workflows=$(curl -fsSL -H "Cookie: n8n-auth=${token}" "${N8N_BASE}/rest/workflows" | jq -re '[.data[].name]' || echo '[]')
