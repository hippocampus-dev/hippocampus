...
When two skills have genuinely distinct activation boundaries but overlapping keywords (e.g., both handle grafana, prometheus), add explicit disambiguation: "For X scenario, use Y skill instead." (If the boundaries do not actually differ, consolidate instead of disambiguating - see `.claude/rules/.claude/skills.md`.)

## Heading Hierarchy

Use `##` as the top-level heading in SKILL.md.
Use heading levels to reflect logical hierarchy:
