...
4. Set `env.KUSTOMIZATION` to the directory where the image digest should be updated.
   In the deploy script, always derive the branch name as `branch="${IMAGE//\//-}"` (the substitution is a no-op when IMAGE has no slash). Git cannot create a ref `refs/heads/foo` when `refs/heads/foo/bar` already exists (e.g., IMAGE `snapshot-controller/snapshot-diff-server`); using `foo-bar` avoids the namespace conflict.
...
7. Add `GIT_CONFIG_GLOBAL` isolation for self-hosted local runners before `actions/checkout`:
   ...
   - If the workflow performs `git commit` (e.g., to update image digests), add `git config --global user.email "kaidotio@gmail.com"` and `git config --global user.name "kaidotio"` before the commit. The isolated gitconfig lacks user identity, causing "Author identity unknown" errors without this
...
9. If the workflow uses `bash .github/scripts/cleanup.sh` to free disk space for large Docker builds, add a condition to skip it on self-hosted local runners:
   ...
   - The system directories it removes break the host toolchain on persistent local runners — read `.github/scripts/cleanup.sh` for what it deletes
   - This is only needed for builds requiring extra disk space (large images, multi-stage builds with heavy dependencies)

10. Add `id-token: write` to the top-level `permissions` block:
   - Required by the reusable workflow chain (`reusable_dynamic-runner.yaml` → `reusable_issue-github-token.yaml`) regardless of whether a scan job is present
   - Without this, GitHub Actions rejects the workflow with `startup_failure` because caller permissions must be a superset of the reusable workflow's permissions

11. Add security scanning (see "Security Scanning" section below):
...
The controller pulls the latest `:main` image when creating pods.
Digest pinning is not needed because:
...
Add a `scan` job after the build job (`id-token: write` is already declared in `permissions` per step 10):
...
Note: Mirrored images use the same scan pattern as application builds.
Each `99_mirror-{name}.yaml` workflow includes its own scan job calling `reusable_scan-image.yaml`.
