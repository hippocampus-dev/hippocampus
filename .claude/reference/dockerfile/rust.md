...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release && \
    cp /opt/builder/target/release/app-name /usr/local/bin/app-name
...
**Note:** Static linking (`RUSTFLAGS="-C target-feature=+crt-static"`) is incompatible with proc-macro crates (e.g., `async-stream`, `tokio-macros`).
If using proc-macros, use `distroless/cc` instead of `distroless/static`.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cp /opt/builder/target/wasm32-unknown-unknown/release/plugin.wasm /plugin.wasm
...
Example: `cluster/applications/envoy-request-hasher/Dockerfile`
