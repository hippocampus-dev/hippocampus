...
When a dependency builds a C++ extension from source (e.g. a pybind11 module), install `g++` and `libc6-dev` and set `ENV CXX=g++` before `pip install`.
The base image's sysconfig reports `CXX=gcc`, so setuptools links the extension without `-lstdc++`; the build succeeds but libstdc++ ABI symbols stay unresolved until import time.
...
The runtime stage must also install the shared libraries the extension links against — `libstdc++6` (C++ ABI runtime) and `libgomp1` (OpenMP), which the `slim` base omits (same `undefined symbol` failure mode, e.g. `_ZTVN10__cxxabiv120__function_type_infoE`).
This applies to prebuilt native wheels too, not only extensions built from source above.
