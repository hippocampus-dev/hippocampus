...
COPY package.json package-lock.json .npmrc /opt/builder/
...
COPY vite.config.ts tsconfig.json /opt/builder/
COPY src /opt/builder/src
COPY server /opt/builder/server
COPY public /opt/builder/public
RUN npm run build
...
COPY --link --from=builder /opt/builder/package.json /opt/builder/package-lock.json /opt/builder/.npmrc ./
...
Note: Set `NODE_ENV` in Kubernetes manifests or docker-compose.yaml, not in the Dockerfile. This allows environment-specific configuration.
