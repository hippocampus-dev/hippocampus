...
Drain the body before `Close()` so `http.Transport` can reuse the TCP connection (keep-alive). Without draining, the transport discards the connection even when `MaxIdleConnsPerHost` is set.
...
|-----------------|---------|
| Equality (`==`, `!=`) | Use `http.Status*` constants (`http.StatusConflict`) |
| Range (`>=`, `<`) | Use numeric literals (`>= 400`, `< 500`) |
