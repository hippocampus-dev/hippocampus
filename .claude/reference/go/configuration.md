...
Use positional arguments for required primary inputs (paths, identifiers, URLs). Use flags for optional or configurable settings.
...
**Flat command** — use when the command has no siblings. Define in `cmd/{command}.go` and register directly on the root command:
...
**Subcommand group** — use when multiple related subcommands share a common noun (e.g., `oauth device`, `oauth pkce`). Create a `cmd/{group}/` package:
