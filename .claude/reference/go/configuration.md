...
For required flags with no meaningful default, use `""` as default and validate after `flag.Parse()`.
Include both the flag name and environment variable in the error message:
...
For Cobra applications, use `Args` struct with `DefaultArgs()`.
Environment variables are resolved in `DefaultArgs()`, NOT at flag registration.

### Positional Arguments vs Flags

Use positional arguments for required primary inputs (paths, identifiers, URLs).
Use flags for optional or configurable settings.

| Input Type | Pattern | Example |
|------------|---------|---------|
| Required primary input | Positional arg with `ExactArgs(N)` + `PreRun` | directory path, endpoint, URL |
| Optional/configurable | Flag (`cmd.Flags().StringVar`) | `--grafana-url`, `--output` |
| No input needed | `cobra.NoArgs` | echo, selfupdate |

Assign positional args to the `Args` struct in `PreRun`, not in `RunE`:
...
Include positional argument names (uppercase) in `Use` field: `"call ADDRESS ENDPOINT BODY"`, `"query DIRECTORY"`.
...
type Args struct {
    ...
    MetricsAddress string `validate:"required,tcp_addr"`
    EnableHTTP2 bool
...
Two layouts exist depending on whether a command stands alone or belongs to a group of related subcommands.

**Flat command** — use when the command has no siblings.
Define in `cmd/{command}.go` and register directly on the root command:
...
**Subcommand group** — use when multiple related subcommands share a common noun (e.g., `oauth device`, `oauth pkce`).
Create a `cmd/{group}/` package:

`cmd/{group}/root.go` — exports `GetRootCmd` and registers subcommands:

```go
func GetRootCmd(args []string) *cobra.Command {
    cmd := &cobra.Command{
        Use:          "{group}",
        Short:        "{Group} utilities",
        SilenceUsage: true,
    }
...
`cmd/{group}/{sub}.go` — defines each subcommand function:

```go
func subCmd() *cobra.Command {
    subArgs := sub.DefaultArgs()
    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            if err := sub.Run(subArgs); err != nil {
                return xerrors.Errorf("failed to run sub.Run: %w", err)
            }
            return nil
        },
    }
...
`pkg/{group}/{sub}/args.go` — `Args` struct lives under the group namespace:
...
Register the group on the root command via `GetRootCmd`:

```go
cmd.AddCommand(oauth.GetRootCmd(args))
```
