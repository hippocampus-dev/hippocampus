...
### Logging Before HTTP 500

Always log errors before returning HTTP 500 (StatusInternalServerError):

```go
// Good: Log error with context before returning HTTP 500
if err != nil {
...
// Bad: No error logging, makes debugging difficult
if err != nil {
    http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
    return
}
...
Use the appropriate logger based on the service type:
- Services with OpenTelemetry: `slog.Error()` (includes trace/span context)
- Simple HTTP servers/exporters: `log.Printf()` (if in handlers; `log.Fatalf` only for startup errors)
- Controllers/webhooks: `ctrl.Log.Error()` (from controller-runtime)
