...
`-public` is an **additional** domain, not a replacement for the regular domain.
All external services have `{service}.kaidotio.dev` with ext-authz.
Services that also need unauthenticated access add `{service}-public.kaidotio.dev`:
