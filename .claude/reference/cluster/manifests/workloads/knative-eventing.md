...
When the Knative Service pod is running, the kafka-broker-dispatcher sends events directly to the pod (bypassing the activator).
Add an ingress rule for the dispatcher:
...
This cluster uses `SINK_BINDING_SELECTION_MODE=inclusion` on the eventing-webhook.
In this mode, SinkBinding only injects environment variables into pods in namespaces that have the `bindings.knative.dev/include: "true"` label.
