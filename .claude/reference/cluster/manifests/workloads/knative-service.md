...
The activator probes ksvc pods directly by pod IP. Its own Sidecar egress is restricted to its own namespace, so its istio-proxy has no Istio cluster routes for cross-namespace ksvc pods and falls through to PassthroughCluster (plain HTTP). With `STRICT` on the queue-proxy port, the destination istio-proxy closes the connection (`EOF`) before the probe can complete, so the revision never becomes ready.
