...
Add specific ingress rules for known callers.
Example for alerthandler (called by mimir-alertmanager):
...
When a Knative Service injects an Istio sidecar (`sidecar.istio.io/inject: "true"`), the activator's probe path forces a port-level relaxation on `8012`.

| Sidecar Injected | mtls.mode | portLevelMtls."8012".mode |
|------------------|-----------|---------------------------|
| No (default, e.g. alerthandler) | `STRICT` | not needed |
| Yes (e.g. loganomaly's ksvcs) | `STRICT` | `PERMISSIVE` |

The activator probes ksvc pods directly by pod IP.
Its own Sidecar egress is restricted to its own namespace, so its istio-proxy has no Istio cluster routes for cross-namespace ksvc pods and falls through to PassthroughCluster (plain HTTP).
With `STRICT` on the queue-proxy port, the destination istio-proxy closes the connection (`EOF`) before the probe can complete, so the revision never becomes ready.

Restricting `PERMISSIVE` to port `8012` (queue-proxy data port) keeps mTLS enforcement on every other port (metrics, admin, telemetry) and on outbound calls from the ksvc.

Example: `cluster/manifests/loganomaly/overlays/dev/{cloudevents-alertmanager,loganomaly-adapter,loganomaly-deduplicator}/peer_authentication.yaml`
...
Knative creates per-Revision services (e.g., `alerthandler-00001`, `alerthandler-00002`).
...
Note: Knative Eventing dispatchers use `ALLOW_ANY` by design.
See [Knative Eventing](knative-eventing.md) for details.
...
Note: `cluster-local-gateway` is NOT required for mesh-to-mesh traffic.
The mesh VirtualService routes directly to activator or pod.
