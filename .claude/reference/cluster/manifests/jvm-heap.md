...
| Absolute `-Xms`/`-Xmx` (via the image's own options env var) | The chart or image already exposes a heap setting; the ceiling then needs no `limits.memory` at all, so `## When to Define Resources` in `.claude/rules/cluster/manifests.md` alone decides whether the container carries any `resources` |
...
Copy from: `cluster/manifests/knative-eventing-kafka/overlays/dev/patches/deployment.yaml` (percentage), `cluster/manifests/adhoc/elasticsearch/patches/deployment.yaml` (absolute `-Xms`/`-Xmx` via `ES_JAVA_OPTS`, alongside the `requests.cpu` its HPA needs), `cluster/manifests/seata/base/stateful_set.yaml` (absolute via `JVM_XMS`/`JVM_XMX`, with no container `resources` in either the base or the overlay patch, whose only `resources` is the `volumeClaimTemplates` storage request)
