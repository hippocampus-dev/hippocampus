...
`list-panes` prints one line per pane in the target's window, so dropping the `awk` leaves a multi-line value in the variable with nothing raised to show for it, and the extra process earns its place only where the exit code is the guard as well (`files/usr/local/bin/resume-pending-claudex.sh:67`).
