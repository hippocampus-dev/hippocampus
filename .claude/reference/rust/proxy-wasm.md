...
Proxy-wasm cannot modify headers during body phases. Calling `set_http_request_header` in `on_http_request_body` returns `Status::BadArgument` (status 2). Calling `set_http_response_header` in `on_http_response_body` causes Envoy to return 503.
...
For body-to-header use cases, use Envoy External Processor (ext_proc). See `.claude/reference/rust/ext-proc.md`.
...
Always configure fail-open unless the filter is security-critical. Without fail-open, a WASM module load failure or runtime error causes all requests through that filter chain to return 500.
