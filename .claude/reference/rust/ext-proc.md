...
Use ext_proc when you need to modify headers based on body content.
Proxy-wasm cannot modify headers during body phases (request or response).
...
Always set `failure_mode_allow: true` unless the ext_proc is security-critical.
Without it, an ext_proc crash, timeout, or gRPC error causes all requests through that filter chain to fail.
...
Note: `failure_mode_allow` only covers ext_proc gRPC failures.
It does NOT help with `per_connection_buffer_limit_bytes` overflow, which operates at the connection level before ext_proc is involved.

### Stream Initiation

`request_header_mode: SEND` is required even if you only process the body.
When set to `SKIP`, Envoy does not open the gRPC stream at all.
...
Prefer `SKIP` for phases the ext-proc server does not need to process.
This reduces unnecessary gRPC round-trips.
...
### Buffer Limit for Large Bodies

When using `request_body_mode: BUFFERED` or `response_body_mode: BUFFERED`, set `per_connection_buffer_limit_bytes` on the listener.
Envoy's default per-connection buffer limit (1MB) silently causes 500 errors or gRPC stream timeouts for bodies that exceed it.
This limit cannot be bypassed via `failure_mode_allow` or `mode_override` — it operates at the connection level before ext_proc is involved.

| Body Size Expected | Action |
|--------------------|--------|
| Small (< 1MB) | Default is sufficient |
| Large (> 1MB) | Set `per_connection_buffer_limit_bytes` on the listener |
...
Istio EnvoyFilter (LISTENER patch required — cannot be set via HTTP_FILTER):
...
- applyTo: LISTENER
  ...
  patch:
    operation: MERGE
    value:
      per_connection_buffer_limit_bytes: 1073741824
```

### gRPC Message Size Limits

tonic's default `max_decoding_message_size` is 4MB.
When bodies exceed 4MB, set both decoding and encoding limits on the `ExternalProcessorServer`:

| Body Size Expected | Action |
|--------------------|--------|
| Small (< 4MB) | tonic defaults are sufficient |
| Large (> 4MB) | Set `max_decoding_message_size` and `max_encoding_message_size` |

```rust
ExternalProcessorServer::new(processor)
    .max_decoding_message_size(1024 * 1024 * 1024)
    .max_encoding_message_size(1024 * 1024 * 1024)
```

Note: This is complementary to `per_connection_buffer_limit_bytes`.
Envoy's limit controls what it buffers before sending to ext_proc, while tonic's limits control what the gRPC server accepts and sends.

## Implementation Pattern

ext_proc receives body chunks via `ProcessingRequest::RequestBody` or `ProcessingRequest::ResponseBody`.
When `end_of_stream` is true, compute the value and return `BodyResponse` with `header_mutation` (and optionally `body_mutation` for response body replacement).

### Defensive Response Pattern

Always respond to all request types, even with empty responses.
Using `_ => {}` causes 504 timeouts when EnvoyFilter is misconfigured:
...
Envoy 1.25+ uses `raw_value` (bytes) instead of `value` (string).
Setting both fields simultaneously causes internal server errors on Envoy 1.31+.
...
Use only `raw_value`.
Do NOT set both `value` and `raw_value`:
