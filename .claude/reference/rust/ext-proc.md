...
Always set `failure_mode_allow: true` unless the ext_proc is security-critical. Without it, an ext_proc crash, timeout, or gRPC error causes all requests through that filter chain to fail.
...
Note: `failure_mode_allow` only covers ext_proc gRPC failures. It does NOT help with `per_connection_buffer_limit_bytes` overflow, which operates at the connection level before ext_proc is involved.
...
When using `request_body_mode: BUFFERED` or `response_body_mode: BUFFERED`, set `per_connection_buffer_limit_bytes` on the listener. Envoy's default per-connection buffer limit (1MB) silently causes 500 errors or gRPC stream timeouts for bodies that exceed it. This limit cannot be bypassed via `failure_mode_allow` or `mode_override` — it operates at the connection level before ext_proc is involved.
...
tonic's default `max_decoding_message_size` is 4MB. When bodies exceed 4MB, set both decoding and encoding limits on the `ExternalProcessorServer`:
...
Note: This is complementary to `per_connection_buffer_limit_bytes`. Envoy's limit controls what it buffers before sending to ext_proc, while tonic's limits control what the gRPC server accepts and sends.
