...
| Practice | Reason |
|----------|--------|
| Return `Result<(), E>` when using `?`, with `E` the crate's error type per `.claude/rules/rust.md` | Propagate fallible I/O through the error type the crate already uses |
