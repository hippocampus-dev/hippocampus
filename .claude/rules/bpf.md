...
* Treat `insight/src/vfs/vfs.bpf.c` and `insight/src/mysql/mysql.bpf.c` as predating these conventions - between them they carry every tracked counter-example below, so reformatting either is a deliberate choice rather than a compliance fix
...
* Open with `#include "vmlinux.h"` ahead of every `<bpf/...>` header, or with the shared `#include "../helpers.h"` that already does so for a program under `insight/src/`, and close the file with `char LICENSE[] SEC("license") = "GPL";`
...
If a variable-length list has to reach the program:
  `cluster/applications/connectracer/src/bpf/connect.bpf.c` pairs a fixed-size array with a `_len` field that the matching loop bounds itself by, while `cluster/applications/fluentd-delayed-unlink/src/bpf/unlink.bpf.c` passes a single NUL-terminated buffer instead. Whether an empty list means "match nothing" or "match everything" is the program's choice - connectracer's filters return early on `_len == 0` to mean everything - and the loader has to enforce whichever the program implements.
