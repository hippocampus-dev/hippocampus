...
| e2e/ | deployment.yaml | e2e pod (Envoy + ext-proc + backend) |
...
The e2e deployment uses a single pod with containers. The base pattern includes three containers:
...
Applications may add additional containers (e.g., nginx for serving generated content) and initContainers as needed for test scenarios.
