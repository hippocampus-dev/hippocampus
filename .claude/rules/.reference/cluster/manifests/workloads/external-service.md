...
| System cannot support OAuth2 (e.g., browser-generated W3C Reporting API) | Exclude from ext-authz; use `{service}-public.kaidotio.dev` |
| Service has its own AuthorizationPolicy with fine-grained access control | Exclude from ext-authz; use `{service}-public.kaidotio.dev` |

### Domain Naming Convention

Services excluded from ext-authz must use the `-public` suffix to distinguish them from ext-authz-protected domains:

| ext-authz | Host Pattern | Example |
|-----------|--------------|---------|
| Yes (default) | `{service}.kaidotio.dev` | `bakery.kaidotio.dev` |
| No (excluded) | `{service}-public.kaidotio.dev` | `reporting-server-public.kaidotio.dev` |
