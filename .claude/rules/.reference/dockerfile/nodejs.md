...
| Build step required (TypeScript, bundler) | Framework | `docker-compose/comfyui/promptgen/` |
...
COPY --link --from=builder /opt/builder/dist ./dist
COPY --link --from=builder /opt/builder/server.production.js ./
...
|-----------|---------------|
| No `node_modules` needed at runtime | `gcr.io/distroless/nodejs22:nonroot` |
| Runtime `node_modules` needed (`npm ci --omit=dev`) | `node:22-slim` + manual nonroot user |
