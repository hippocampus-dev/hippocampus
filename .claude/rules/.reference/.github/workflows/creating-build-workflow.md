...
8. Add auto-retry job (see "Auto-Retry" section below):
   - Add `actions: write` to `permissions`
   - Add `rerun` job that depends on all other jobs
...
permissions:
  ...
  actions: write   # Required for rerun job
...
Add `actions: write` to `permissions` and a `rerun` job that depends on all other jobs in the workflow:
...
rerun:
  ...
  timeout-minutes: 5
  steps:
    - run: gh run rerun --failed ${{ github.run_id }} --repo ${{ github.repository }}
      env:
...
| Element | Value |
|---------|-------|
| `if: failure() && github.run_attempt == 1` | Only retry on first attempt to prevent infinite loops |
| `needs` | List all other jobs in the workflow |
| `actions: write` | Required permission for `gh run rerun` |
| `--failed` | Only reruns failed jobs, not the entire workflow |
