...
|-------|------------|-------------|-------|
| `value` | Works | Silently ignored | String field (deprecated in newer Envoy) |
| `raw_value` | Silently ignored | Works | Bytes field (preferred in newer Envoy) |

Set both `value` and `raw_value` fields for compatibility across Envoy versions:
