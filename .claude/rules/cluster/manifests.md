...
| Upstream defaults too large for dev | Upstream ships production-sized resources that cause resource contention in dev | knative-eventing-kafka |

If none of these conditions apply, do not add `resources` to the patch. When adjusting existing resource values, do not add `resources.requests` or `resources.limits` to containers that do not already have them set. When overriding upstream defaults, add a comment on the container entry indicating the upstream values being overridden (e.g., `# Override upstream defaults (requests: cpu 200m, memory 450Mi)`).
