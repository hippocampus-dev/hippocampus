...
The `default-deny` NetworkPolicy uses an empty `podSelector` with no `policyTypes`, which restricts ingress only. Do NOT add `egress` rules to non-utility NetworkPolicies — adding an `egress` section implicitly activates egress enforcement, blocking all egress not explicitly allowed (including istiod xDS connections).
...
When adding a `{service}.kaidotio.dev` host to a Gateway and VirtualService, determine ext-authz disposition:

| Condition | Action | Host Pattern |
|-----------|--------|--------------|
| Default | Add host to `cluster/manifests/istio-gateways/overlays/dev/patches/authorization_policy.yaml` | `{service}.kaidotio.dev` |
| OAuth2 incompatible or own AuthorizationPolicy | Exclude from ext-authz | `{service}-public.kaidotio.dev` |

Both paths require an explicit decision — do not leave the host absent from the authorization policy without using the `-public` suffix.
