...
| Upstream defaults diverge from observed usage | Upstream ships resources that are too large (causes contention in dev) or too small (risks OOMKill) relative to actual observed usage | knative-eventing-kafka |

If none of these conditions apply, do not add `resources` to the patch. When adjusting existing resource values, do not add `resources.requests` or `resources.limits` to containers that do not already have them set. When only one field (e.g., `requests`) needs correction and upstream already provides acceptable values for the other (e.g., `limits`), patch only the diverging field — Kustomize strategic merge will preserve the upstream values for unpatched fields. When overriding upstream defaults, add a comment on the container entry indicating the upstream values being overridden (e.g., `# Override upstream defaults (requests: cpu 200m, memory 450Mi)`).
