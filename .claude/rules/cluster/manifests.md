...
Before adding or expanding a change to fix a problem, confirm it against the running cluster. Do not apply a fix inferred from a past report or a known failure pattern without checking that it still applies now — the cluster changes, so a fix needed during one incident may be a no-op later.
...
* Do not codify a pattern into a rule without confirmation and at least two existing in-tree precedents.
...
SecretsFromVault is an exec plugin. When running `kustomize build`, always pass `--enable-alpha-plugins`:
...
Under `readOnlyRootFilesystem: true`, libraries that cache to a home-relative default (e.g. `$HOME/.cache/{tool}`) cannot write. A single library often has several independent cache locations, so redirecting only the primary one can still crash — enumerate them from the library's docs.
...
Values in the table are baselines. Deviate only when measured proxy usage requires it, and add an inline comment on the annotation stating the reason (e.g., `sidecar.istio.io/proxyMemory: 256Mi # Use proxy-wasm`).
...
| Rust (tokio) | `available_parallelism()` reads cgroup CPU quota via `requests.cpu` | proxy-wasm |
...
If none of these conditions apply, do not add `resources` to the patch. When adjusting existing resource values, do not add `resources.requests` or `resources.limits` to containers that do not already have them set. In `overlays/dev/patches/`, when only one field (e.g., `requests`) needs correction and upstream already provides acceptable values for the other (e.g., `limits`), patch only the diverging field — Kustomize strategic merge will preserve the upstream values for unpatched fields. This does not hold for operator-managed CRs, where defaulting is typically per block (all-or-nothing on `requests`/`limits`) — restate the upstream values being kept, and confirm the operator's defaulting granularity in its source before omitting a field. When overriding upstream defaults, add a comment on the container entry indicating the upstream values being overridden (e.g., `# Override upstream defaults (requests: cpu 200m, memory 450Mi)`).
...
Use an integer for `rollingUpdate.maxUnavailable`, not a percentage. Percentages round down for Deployment and StatefulSet (`20%` of `replicas: 1` = `0`), which blocks rollouts because the controller cannot tolerate any unavailable pod and refuses to terminate the only existing pod. Default to `1`; `0` is acceptable when paired with `maxSurge: 1` for zero-downtime Deployment rollouts.
...
All external services must have `{service}.kaidotio.dev` with ext-authz. The `-public` suffix is an **additional** domain, not a replacement:
