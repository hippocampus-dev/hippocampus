...
* All containers require secure defaults (non-root UID 65532, no privilege escalation, read-only filesystem, emptyDir with `medium: Memory` for `/tmp` if application needs writable temp); mount runtime socket volumes under `/run/` and temporary data volumes under `/tmp/` (not at root-level paths like `/sock` or `/backing`)
