...
When the SecretsFromVault `metadata.name` matches a name in a Role's `resourceNames` (e.g., leader election Lease names for controllers), add `behavior: merge` to the spec.
Without it, kustomize appends a hash suffix to the generated Secret name, and the nameReference transformer propagates that suffix to the Role's `resourceNames`.
