...
When using `namePrefix` with a Gateway and VirtualService, the VirtualService `gateways` reference and `destination.host` must use the full prefixed name (e.g., `memory-bank-cloudevents-ingress`). Kustomize `namePrefix` does not auto-update cross-resource references in VirtualService specs.
