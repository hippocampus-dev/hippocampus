...
With kind, the Makefile `e2e` target pattern is: `kind create cluster` → `./e2e.sh` → kind is deleted by trap on exit. Without kind the target is a bare `./e2e.sh`.
...
For docker-only runtimes (no Kubernetes needed), e2e uses docker directly without kind (e.g., chrome-devtools-protocol-server). For runtimes needing neither — a binary plus local assets — `e2e.sh` drives the build's own tests in-process and no `e2e/` is created (e.g., rust_de_llama). The test client keeps its tool-named directory either way.
