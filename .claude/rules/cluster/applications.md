...
* Source code, Dockerfile, Makefile, README.md for applications built in this repository
* Leave `subjects[].namespace` out of `manifests/cluster_role_binding.yaml` and `manifests/role_binding.yaml` - the same base is built by `skaffold/` (`namespace: skaffold-{app}`) and by `cluster/manifests/{app}/overlays/dev/` (`namespace: {app}`), and a hardcoded value survives both namespace transformers, silently binding a ServiceAccount in the wrong namespace with no build error
...
| No (vendored image, assets built inside Docker) | **Do not create** | None — no-op `all` stub only | `k6`, `jupyterhub`, `talk` |

Applications that can run without Kubernetes have their manifests ONLY in `cluster/manifests/` (either `cluster/manifests/{app}/` or `cluster/manifests/utilities/{app}/`).
Do NOT add `manifests/`, `skaffold/`, or `skaffold.yaml` to `cluster/applications/{app}/` for these.
...
├── main.go (or main.py, etc.), Dockerfile, Makefile, README.md
...
2. In `cluster/applications/{app-name}/`:
   - Source code, Dockerfile, and README.md
...
Controllers and webhooks that expose a user-facing contract (annotation pattern, CR, resource convention) provide example manifests.
Whether to create one or both directories depends on whether the skaffold overlay prefixes the annotation domain or CRD API group with `skaffold.`:
...
With kind, the Makefile `e2e` target pattern is: `kind create cluster` → `./e2e.sh` → kind is deleted by trap on exit.
Without kind the target is a bare `./e2e.sh`.
...
For runtimes needing neither — a binary plus local assets — `e2e.sh` drives the build's own tests in-process and no `e2e/` is created (e.g., rust_de_llama).
The test client keeps its tool-named directory either way.
