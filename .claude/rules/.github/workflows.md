...
* When running multiple `aquasecurity/trivy-action` steps in parallel (e.g. under a `parallel:` block), set a distinct `cache-dir` in each step's `with:` block rather than `env: TRIVY_CACHE_DIR`. The action's `action.yaml` always re-exports `TRIVY_CACHE_DIR` from its own `cache-dir` input (default `${{ github.workspace }}/.cache/trivy`) immediately before running, silently overriding any caller-set `env:` value; parallel steps that end up sharing the same default cache directory fail with `cache may be in use by another process: timeout` (see `reusable_scan-image.yaml`)
...
When the job must access `environment_secrets` and is triggered by a Dependabot PR (or any external contributor), use `pull_request_target` instead of `pull_request`. GitHub's environment protection rules block the `refs/pull/N/merge` context produced by `pull_request` events; `pull_request_target` runs in the base branch context, satisfying protection rules. Do not check out or execute any PR code in these workflows.
