...
* Jobs depending on `runner` reusable workflow must use `!cancelled()` in their `if:` condition to handle partial failures (e.g., token server unavailable). See `.claude/rules/.reference/.github/workflows/creating-build-workflow.md` for exact patterns
...
* Before `uses: ./.github/actions/setup-docker`, add a conditional step `echo "DOCKER_CONFIG=${{ runner.temp }}/.docker" >> $GITHUB_ENV` that runs only on self-hosted local runners (`if: contains(needs.runner.outputs.runner, 'local')`). This prevents `docker login` from overwriting `~/.docker/config.json` with short-lived `ghs_` GitHub App installation tokens
...
`github-actions-runner-controller` runners have a read-only root filesystem; install binaries to `${{ runner.temp }}` instead of `/usr/local/bin`
