...
* `files/home/kai/.config/claudex/config/CLAUDE*.md` feeds `bin/sync-agent-files.sh` which generates the `AGENTS.md` files; agent (`@name`) or tool references not matched by its removal filters leak verbatim - read the script's `# Global user prompt` section before adding them, then re-run it
