...
Match keys between `store.get()` in `setup()`, `get_settings()`, and `handle_*()` functions.
Read existing keys from `src-tauri/src/commands/settings.rs`.

## Transcription Filtering

| Stage | Filter |
|-------|--------|
| Before transcription | RMS silence gate |
