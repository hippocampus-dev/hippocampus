...
Re-enabling selfHeal restores the manifest ArgoCD manages, not what an operator derives from it. After reverting an experiment on a CR, check the operator-managed Deployment/StatefulSet directly — operators that merge resource fields rather than replacing them leave removed keys behind.
