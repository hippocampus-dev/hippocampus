...
Send an email from an allowlisted sender to `agent@kaidotio.dev`.
A draft appears in Slack `#email` within seconds.
Click **Approve** to send the reply to the sender, **Edit** to revise the draft in a modal, or **Deny** to dismiss.
Unresolved drafts auto-expire after 24 hours.
...
Local `wrangler dev` uses Miniflare simulators for Durable Objects.
The `AI` binding calls production Workers AI (no local simulator available).

## Deployment

Deployment is automated via `.github/workflows/20_agent.yaml` on pushes to `main` that touch `agent/**`.
Do NOT run `wrangler deploy` manually — the PreToolUse hook blocks it.
