...
export const isRunning = computed(() => timerGroup.value.status === "running");
...
export function startTimer(): void {
  ...
  const startIndex =
    group.status === "idle" ? viewIndex.value : group.currentIndex;

  const newTimers = group.timers.map((timer: Timer, i: number) =>
    i === startIndex ? { ...timer, status: "running" as const } : timer,
  );
  timerGroup.value = {
    ...group,
    timers: newTimers,
...
export function pauseTimer(): void {
  ...
  const newTimers = group.timers.map((timer: Timer, i: number) =>
    i === group.currentIndex ? { ...timer, status: "paused" as const } : timer,
  );
...
export function setRemainingSeconds(seconds: number): void {
  const group = timerGroup.value;
  if (group.status !== "running") return;
  const currentIdx = group.currentIndex;
  const timer = group.timers[currentIdx];
  if (!timer) return;
  const newTimers = [...group.timers];
  newTimers[currentIdx] = { ...timer, remainingSeconds: seconds };
  timerGroup.value = { ...group, timers: newTimers };
}

export function completeCurrentTimer(): boolean {
  const group = timerGroup.value;
  if (group.status !== "running") return false;
  const currentIdx = group.currentIndex;
  const timer = group.timers[currentIdx];
...
export function deleteSavedGroup(id: string): void {
  const newSavedGroups = savedGroups.value.filter(
    (g: SavedGroup) => g.id !== id,
  );
  savedGroups.value = newSavedGroups;
  localStorage.setItem(STORAGE_KEY, JSON.stringify(newSavedGroups));
}
