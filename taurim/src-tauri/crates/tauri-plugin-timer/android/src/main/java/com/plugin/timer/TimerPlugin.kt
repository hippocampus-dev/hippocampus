...
class TimerPlugin(private val activity: Activity) : Plugin(activity) {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.action) {
                TimerService.ACTION_TICK -> {
                    ...
                    payload.put("remaining", remainingSeconds)
    ...
    fun stopAlarm(invoke: Invoke) {
        Log.d("TimerPlugin", "Stopping alarm")
        ...
        scope.launch {
            try {
                ...
                Log.e("TimerPlugin", "Failed to stop alarm", e)
                invoke.reject(e.message ?: "Failed to stop alarm")
            }
        }
    }
