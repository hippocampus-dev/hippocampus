locals {
  claude_code_outbound_cidr = "160.79.104.0/21"
  public_hosts = [
    "cortex-api-public.kaidotio.dev",
    "http-kvs-public.kaidotio.dev",
    "spotify-mcp-server-public.kaidotio.dev",
    "token-request-server-public.kaidotio.dev",
    "hippocampus-dev-github-token-server-public.kaidotio.dev",
  ]
  public_hosts_expression = "http.host in {${join(" ", [for host in local.public_hosts : "\"${host}\""])}}"
}
...
resource "cloudflare_ruleset" "rate_limiting" {
  ...
  rules = [
    {
      ref         = "device_flow_bridge_rate_limit"
      expression  = "http.host eq \"hippocampus-dev-device-flow-bridge-public.kaidotio.dev\""
      description = "Strict rate limit for device-flow-bridge"
      action      = "block"
      enabled     = true
...
resource "cloudflare_ruleset" "firewall_custom" {
  ...
  rules = [
    {
      ref         = "skip_sbfm_public"
      expression  = local.public_hosts_expression
      description = "Skip Super Bot Fight Mode for public services used by GitHub Actions"
      action      = "skip"
      enabled     = true
    ...
    {
      ref         = "block_non_japan"
      expression  = "(ip.geoip.country ne \"JP\") and not cf.client.bot and not (${local.public_hosts_expression})"
      description = "Block traffic from outside Japan (except verified bots and public services)"
      action      = "block"
      enabled     = true
    },
  ]
...
resource "cloudflare_ruleset" "firewall_managed" {
  ...
  description = "Execute managed WAF rulesets"
  kind        = "zone"
...
resource "cloudflare_zone_setting" "webp" {
  zone_id    = cloudflare_zone.main.id
  setting_id = "webp"
  value      = "on"
  depends_on = [cloudflare_zone_setting.polish]
}
