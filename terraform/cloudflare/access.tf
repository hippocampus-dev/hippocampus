...
resource "cloudflare_zero_trust_access_application" "spotify_mcp_server_public" {
  account_id   = var.account_id
  allowed_idps = []
  destinations = [
    {
      type = "public"
      uri  = "spotify-mcp-server-public.kaidotio.dev"
    },
  ]
  domain                   = "spotify-mcp-server-public.kaidotio.dev"
  name                     = "Spotify MCP Server (Public)"
  options_preflight_bypass = false
  policies = [
    {
      decision = "bypass"
      ...
      include = [
        ...
        {
          ip = {
            ip = local.claude_code_outbound_cidr
          }
        },
      ]
