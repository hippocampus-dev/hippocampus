...
Masking is best-effort rather than a boundary, and a matching file is still served in the clear when it is:

- mapped with `mmap`, which fuse-bpf passes straight through to the backing file
- read through a hard link whose own name does not match, or through the handle that created it
- created under a directory that was itself made through the mount

A file renamed through the mount reads neither way: its reads fail with EIO until the kernel drops the entry and the new name re-decides the mark.
Each masked read reopens the backing file and reads all of it, so the daemon's cost scales with the file rather than the request.
