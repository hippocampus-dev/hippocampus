...
demo    show the masked view, a nested and a created file, the plaintext backing, and the silent fast path
...
EOS
  ...
  # Nested payload: reads here only resolve if the daemon learned the directory too.
  sudo mkdir -p "${BACKING_DIR}/archive"
  sudo tee "${BACKING_DIR}/archive/old-customers.csv" >/dev/null <<'EOS'
id,name,email,phone,card
9,Saburo Kato,saburo@example.net,080-2222-3333,4000-0000-0000-0002
EOS
...
demo() {
  ...
  echo
  echo "== masked view in a subdirectory (archive/old-customers.csv) =="
  sudo cat "${MOUNT_DIR}/archive/old-customers.csv"
  echo
  echo "== a file created through the mount is masked too =="
  # Removed first, or a second demo run would take the lookup path and prove nothing.
  sudo rm -f "${MOUNT_DIR}/created.csv"
  sudo cp "${BACKING_DIR}/customers.csv" "${MOUNT_DIR}/created.csv"
  sudo cat "${MOUNT_DIR}/created.csv"
  echo
