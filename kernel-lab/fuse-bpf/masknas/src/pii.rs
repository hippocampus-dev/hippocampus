...
// Alternatives ordered most-specific first (leftmost-first matching).
...
/// Mask every PII span in `data` in place; returns the number of spans masked.
...
mod tests {
    ...
    fn preserves_length() {
        for input in [
            ...
            assert_eq!(masked(input).len(), input.len(), "length changed for {input:?}");
        }
    }
