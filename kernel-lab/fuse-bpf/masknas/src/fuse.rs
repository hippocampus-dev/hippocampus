...
// Must match SENSITIVE_TAG in src/bpf/mask.bpf.c: the bit read_iter_prefilter tests.
pub const SENSITIVE_TAG: u64 = 1 << 63;
...
pub const FORGET_IN_SIZE: usize = std::mem::size_of::<FuseForgetIn>();
pub const BATCH_FORGET_IN_SIZE: usize = std::mem::size_of::<FuseBatchForgetIn>();
pub const FORGET_ONE_SIZE: usize = std::mem::size_of::<FuseForgetOne>();
pub const MKDIR_IN_SIZE: usize = std::mem::size_of::<FuseMkdirIn>();
pub const CREATE_IN_SIZE: usize = std::mem::size_of::<FuseCreateIn>();
pub const OPEN_OUT_SIZE: usize = std::mem::size_of::<FuseOpenOut>();
...
unsafe impl plain::Plain for FuseMkdirIn {}
...
unsafe impl plain::Plain for FuseCreateIn {}
...
unsafe impl plain::Plain for FuseOpenOut {}
...
unsafe impl plain::Plain for FuseForgetIn {}
...
unsafe impl plain::Plain for FuseBatchForgetIn {}
...
unsafe impl plain::Plain for FuseForgetOne {}
...
pub fn struct_bytes<T: plain::Plain>(value: &T) -> &[u8] {
...
pub fn read_backing_file(
    ...
    let mut file = open_beneath(directory_fd, path, libc::O_RDONLY)?;
    let mut contents = Vec::new();
    file.read_to_end(&mut contents)?;
    Ok(contents)
}

// FUSE_CREATE leaves the reply's attr zeroed, so the inode number has to come from the backing
// file itself.
pub fn inode_number(
    directory_fd: std::os::fd::RawFd,
    path: &std::path::Path,
) -> std::io::Result<u64> {
    // Confined the same way read_backing_file is: this answer decides whether a file gets marked,
    // so it has to resolve through the same components the read will.
    let file = open_beneath(directory_fd, path, libc::O_PATH | libc::O_NOFOLLOW)?;
...
// Paths here are rebuilt from names the daemon learned earlier, so a component swapped for a
// symlink in between would otherwise resolve outside the backing directory.
fn open_beneath(
    directory_fd: std::os::fd::RawFd,
    path: &std::path::Path,
    flags: libc::c_int,
...
const _: () = {
    ...
    assert!(FORGET_IN_SIZE == 8);
    assert!(BATCH_FORGET_IN_SIZE == 8);
    assert!(FORGET_ONE_SIZE == 16);
    assert!(MKDIR_IN_SIZE == 8);
    assert!(CREATE_IN_SIZE == 16);
    assert!(OPEN_OUT_SIZE == 16);
