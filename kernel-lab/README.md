...
`lib.sh` holds the shared machinery; each experiment is a subdirectory whose `experiment.sh` sets the kernel tree, commit, patches, and config, then sources `lib.sh`.
`fuse-bpf/` builds the out-of-tree fuse-bpf series on `bpf-next`; `sanitizer/` needs no patches and instead builds `v7.1` under KASAN and lockdep, which cost too much to leave on in a kernel you daily-drive.
