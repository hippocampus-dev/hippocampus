...
kernel_lab_build() {
  ...
  # Append -Wno-error (after "$@" to beat the tree's -Werror): GCC >= 15 is stricter,
  # and a throwaway build should not fail on the extra warnings. Keep the wrapper in
  # the lab, not $TMPDIR, in case /tmp is mounted noexec.
  local wrapper="${work}/.gcc-wrapper"
  mkdir -p "${wrapper}"
  cat > "${wrapper}/gcc" <<'WRAP'
#!/bin/sh
exec /usr/bin/gcc -std=gnu11 "$@" -Wno-error
WRAP
  chmod +x "${wrapper}/gcc"
...
kernel_lab_rootfs() {
  ...
  seed="$(mktemp -d)"
  # Expand ${seed} while it is still in scope: the trap fires after this function
  # has returned, and a stale expansion leaves rm -rf "" to succeed silently.
