...
Connect to a remotty workspace via chisel reverse tunnel.
Additional `REMOTE` arguments are passed directly to chisel; use explicit `127.0.0.1` binds to avoid listening on every interface.
...
`--env` accepts a comma-separated glob allow list evaluated with `path.Match` semantics (e.g. `LC_*`).
Matching variables are exposed via the host bridge and sourced by the workspace shell on startup; run `remotty-env` inside an existing shell to refresh.
Clipboard is relayed to the browser via OSC52 through tmux and ttyd.
