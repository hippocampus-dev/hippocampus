...
func Run(args *Args) error {
	...
	bakeryClient := bakery.NewClient("https://bakery.minikube.127.0.0.1.nip.io/callback", args.AuthorizationListenPort)
...
func indexFromLSPServer(ctx context.Context, language string, address string, vectorStore vector.Store, vectorDimension int, embeddingModel string, bakeryClient *bakery.Client) error {
	...
	log.Printf("Indexed %d symbols from %s (skipped %d unchanged)", totalSymbols, address, skippedSymbols)

	if err := vectorStore.DeleteOldEntries(ctx, address, startTime); err != nil {
