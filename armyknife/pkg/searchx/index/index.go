...
func Run(args *Args) error {
	...
	for language, address := range args.LSPServers {
		l := language
		a := address
		eg.Go(func() error {
			if err := indexFromLSPServer(ctx, l, a, vectorStore, args.Dimension, args.EmbeddingModel, bakeryClient); err != nil {
				return xerrors.Errorf("failed to index from server %s (%s): %w", a, l, err)
			}
			return nil
		})
	}
...
func indexFromLSPServer(ctx context.Context, language string, address string, vectorStore vector.Store, vectorDimension int, embeddingModel string, bakeryClient *bakery.Client) error {
	client, err := lsp.NewClient(address)
	if err != nil {
	...
	if len(supportedExtensions) == 0 {
		return xerrors.Errorf("unsupported language: %s", language)
	}

	extensionSet := set.New(supportedExtensions...)
	...
	for _, file := range files {
		if extensionSet.Contains(filepath.Ext(file)) {
			filteredFiles = append(filteredFiles, file)
		}
	}
	files = filteredFiles

	log.Printf("Found %d %s files to index from %s", len(files), language, address)
	...
	for _, filePath := range files {
		...
		languageID := getLanguageIDFromExtension(filepath.Ext(filePath))
		if languageID == "" {
			continue
		}
		...
		if err := client.DidOpen(fileURI, languageID, string(content)); err != nil {
			continue
		}

		symbols, err := client.TextDocumentDocumentSymbol(fileURI)
		_ = client.DidClose(fileURI)
		if err != nil {
			continue
		}
		...
		for _, symbol := range symbols {
			...
			if err := indexSymbol(ctx, vectorStore, symbolDocument, vectorDimension, embeddingModel, bakeryClient); err != nil {
				if errors.Is(err, ErrAlreadyIndexed) {
					skippedIDs = append(skippedIDs, symbolID)
				} else {
	...
	log.Printf("Indexed %d symbols from %s (skipped %d unchanged)", totalSymbols, address, len(skippedIDs))

	if len(skippedIDs) > 0 {
		if err := vectorStore.Touch(ctx, skippedIDs); err != nil {
			log.Printf("Warning: failed to touch skipped symbols: %v", err)
		}
	}

	if err := vectorStore.DeleteOld(ctx, address, startTime.Unix()); err != nil {
		log.Printf("Warning: failed to delete old entries: %v", err)
	} else {
		log.Printf("Cleaned up old symbols for %s", address)
	}

	if err := client.Shutdown(); err != nil {
		log.Printf("Warning: failed to shutdown LSP client: %v", err)
	}
...
func getLanguageIDFromExtension(extension string) string {
	return extensionToLanguageID[extension]
}

func getExtensionsForLanguage(language string) []string {
	aliases, ok := languageAliases[language]
	if !ok {
		return nil
	}

	aliasSet := set.New(aliases...)

	var extensions []string
	for extension, languageID := range extensionToLanguageID {
		if aliasSet.Contains(languageID) {
