...
func promptByOpenAI(ctx context.Context, bakeryClient *bakery.Client, model string, serviceTier string, prompt string) (*StructuredResult, error) {
...
func Run(a *Args) error {
	...
	for _, record := range records[1:] {
		...
		go func(r map[string]string) {
			...
			llmResponse, err := promptByOpenAI(ctx, bakeryClient, a.Model, a.ServiceTier, prompt)
			if err != nil {
