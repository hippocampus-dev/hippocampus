...
func getOpenAIAPIBase() string {
	...
	return "http://ollama.127.0.0.1.nip.io/v1"
}

func CreateHTTPRequest(ctx context.Context, bakeryClient *bakery.Client, path string, body io.Reader) (*http.Request, error) {
	destination, err := url.JoinPath(getOpenAIAPIBase(), path)
	if err != nil {
	...
	if strings.HasPrefix(u.Host, "127.") || strings.HasPrefix(u.Host, "localhost") || strings.HasSuffix(u.Host, ".127.0.0.1.nip.io") {
