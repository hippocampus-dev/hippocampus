...
func (c *Client) doRequest(ctx context.Context, method, path string, body io.Reader) (*http.Response, error) {
	...
	if response.StatusCode >= 400 {
		...
		return nil, xerrors.Errorf("grafana api error: status=%d, body=%s", response.StatusCode, string(body))
	}
...
func (c *Client) Query(ctx context.Context, queries []Query, from, to time.Time) (*QueryResponse, error) {
	if len(queries) == 0 {
		return nil, xerrors.New("at least one query is required")
	}

	request := BuildQueryRequest(queries, from, to)
...
func (c *Client) GetPrometheusDatasource(ctx context.Context, name string) (*Datasource, error) {
	...
	if name != "" {
		return nil, xerrors.Errorf("prometheus datasource not found: %s", name)
	}
	return nil, xerrors.New("prometheus datasource not found")
}
