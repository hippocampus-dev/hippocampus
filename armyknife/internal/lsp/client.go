...
func (c *Client) TextDocumentDocumentSymbol(documentURI string) ([]SymbolInformation, error) {
	...
	var symbols []SymbolInformation
	if err := json.Unmarshal(response.Result, &symbols); err == nil && len(symbols) > 0 {
		return symbols, nil
	}

	var documentSymbols []DocumentSymbol
	if err := json.Unmarshal(response.Result, &documentSymbols); err != nil {
		return nil, xerrors.Errorf("failed to unmarshal document symbols: %w", err)
	}

	result := make([]SymbolInformation, 0, countDocumentSymbols(documentSymbols))
	flattenDocumentSymbolsInto(&result, documentURI, documentSymbols, "")
	return result, nil
}
...
func flattenDocumentSymbolsInto(result *[]SymbolInformation, documentURI string, symbols []DocumentSymbol, containerName string) {
	for _, symbol := range symbols {
		*result = append(*result, SymbolInformation{
			Name:          symbol.Name,
			Kind:          symbol.Kind,
			Deprecated:    symbol.Deprecated,
			Location:      Location{URI: documentURI, Range: symbol.Range},
			ContainerName: containerName,
		})
		flattenDocumentSymbolsInto(result, documentURI, symbol.Children, symbol.Name)
	}
}
...
func (c *Client) Shutdown() error {
	...
	if err != nil {
		// Some LSP servers close the connection immediately after receiving shutdown request
		// without sending a response, which causes EOF error. This is acceptable behavior.
	...
	if err := c.sendNotification("exit", nil); err != nil {
		// Ignore EOF error on exit notification as well since server may already be closed
		if !errors.Is(err, io.EOF) {
