...
- `deviceauth` - OAuth token acquisition via device authorization grant (RFC 8628)
...
- `mcp` - MCP (Model Context Protocol) servers
  ...
  - `codex` - MCP server for codex
  - `computer` - MCP server for X11 desktop automation (Anthropic computer_20250124 compatible)
  - `gemini` - MCP server for gemini
  - `lsp` - MCP server for LSP operations
  - `notify` - MCP server for desktop notifications
  - `prompts` - MCP server for managing prompts and prompt templates
- `proxy` - TCP/HTTP reverse proxy implementation
- `remotty` - Reverse tunnel client for remotty workspaces via chisel with Bakery cookie authentication. Also runs a local HTTP bridge so that `xdg-open` inside the container opens URLs in the host browser
