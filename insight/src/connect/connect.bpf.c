...
SEC("kprobe/__x64_sys_poll") int BPF_KPROBE(__x64_sys_poll, int _, struct pollfd *ufds) {
    u64 __pid_tgid = bpf_get_current_pid_tgid();
    gid_t tgid = __pid_tgid >> 32;
    pid_t pid = __pid_tgid;
...
SEC("kretprobe/__x64_sys_poll") int BPF_KRETPROBE(__x64_sys_poll_ret, int ret) {
    if (ret == -1) {
        return 0;
    }
...
SEC("kprobe/__x64_sys_ppoll") int BPF_KPROBE(__x64_sys_ppoll, int _, struct pollfd *ufds) {
    u64 __pid_tgid = bpf_get_current_pid_tgid();
    gid_t tgid = __pid_tgid >> 32;
    pid_t pid = __pid_tgid;
...
SEC("kretprobe/__x64_sys_ppoll") int BPF_KRETPROBE(__x64_sys_ppoll_ret, int ret) {
    if (ret == -1) {
        return 0;
    }
