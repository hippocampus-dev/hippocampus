...
pub fn watch(
    ...
    let mut skel_builder = MysqlSkelBuilder::default();
    if args.debug {
    ...
    if let Some(tgid) = args.tgid {
        open_skel.rodata().tool_config.tgid = tgid;
    }

    if let Some(port) = args.mysql_port {
        open_skel.rodata().tool_config.target_port = port;
    }

    let mut skel = open_skel.load()?;
    skel.attach()?;

    let perf_array = libbpf_rs::PerfBufferBuilder::new(skel.maps_mut().events())
        .sample_cb(move |_cpu: i32, data: &[u8]| {
            ...
            event_tx.send(crate::Event::Result(mysql_event)).unwrap();
        })
        .build()?;
    ...
    loop {
        ...
        perf_array.poll(std::time::Duration::from_millis(100))?;
    }
