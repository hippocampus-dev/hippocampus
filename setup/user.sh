...
go install github.com/telepresenceio/telepresence/v2@latest

claudex mcp add -t stdio notify --scope user -- armyknife mcp notify
claudex mcp add -t stdio tmux --scope user -- armyknife mcp tmux
...
claudex mcp add -t stdio chrome --scope user -- chrome-devtools-mcp --browserUrl http://127.0.0.1:59222
