...
// https://heroicons.com/
import {Cog8ToothIcon, ComputerDesktopIcon, LanguageIcon} from "@heroicons/react/24/solid";
...
const Index = ({}) => {
    ...
    const handleKeyDown = async (e: KeyboardEvent) => {
        ...
        if (e.key === "ArrowUp") {
            ...
            if (filteredLines[selectedIndex]?.background) {
                await getCurrentWindow().hide();
            }

            try {
                await filteredLines[selectedIndex]?.handler(abortController);
            } finally {
                setIsLoading(false);
            ...
            e.preventDefault();
            if (filteredLines[selectedIndex]?.abortHandler) {
                    ...
                    h("tbody", {class: "bg-white divide-y divide-orange-600"}, filteredLines.map((line, i) =>
                        h("tr", {
                            onClick: async () => {
                                ...
                                try {
                                    await line.handler(abortController);
                                } finally {
                                    setIsLoading(false);
                                    setAbortController(new AbortController());
                                }
