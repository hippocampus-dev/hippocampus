...
import type { Settings as IPCSettings } from "../ipc/types/settings";
...
const Settings = ({}) => {
  ...
  const [realtimeTranslationLanguage, setRealtimeTranslationLanguage] =
    useState<string | null>(null);
  const [voiceInputShortcut, setVoiceInputShortcut] = useState<string | null>(
    null,
  );
  const [voiceInputModel, setVoiceInputModel] = useState<string | null>(null);
  const [voiceInputLanguage, setVoiceInputLanguage] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const status: boolean = await invoke("get_whisper_model_status", {
          modelName: voiceInputModel,
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      await invoke("handle_realtime_translation_language", {
        value: realtimeTranslationLanguage,
      });
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      await invoke("handle_voice_input_shortcut", {
  ...
  useEffect(() => {
    ...
    (async () => {
      await invoke("handle_voice_input_language", {
  ...
  return h(
    ...
    [
      ...
      h("div", { class: "w-full lg:w-1/2 mt-6" }, [
        h(
          "h2",
          { class: "text-lg font-semibold mb-4 border-b pb-2" },
          "General",
        ),
          ...
          h("div", { class: "mt-2" }, [
            h(Select, {
              ...
              onChange: (e: KeyboardEvent) =>
                setRealtimeTranslationLanguage(
        ...
        h("div", { class: "space-y-4" }, [
          ...
          h("div", { class: "mt-2" }, [
            h(Select, {
              ...
              onChange: (e: KeyboardEvent) => {
                ...
                setVoiceInputModel(value);
                if (value === "kotoba-whisper-v2.0") {
                  setVoiceInputLanguage("ja");
                }
              },
              options: [
                ...
                {
                  value: "kotoba-whisper-v2.0",
                  label: "Kotoba Whisper v2.0 (Japanese, 1.5 GB)",
                },
                {
                  value: "kotoba-whisper-bilingual-v1.0",
                  label: "Kotoba Whisper Bilingual v1.0 (JA/EN, 1.5 GB)",
                },
              ],
            }),
          ]),
          h("div", { class: "mt-2" }, [
            h(Select, {
              ...
              onChange: (e: KeyboardEvent) =>
                setVoiceInputLanguage((e.target as HTMLSelectElement).value),
              options: [
          ...
          h("div", { class: "mt-2" }, [
            h(Select, {
              ...
              onChange: (e: KeyboardEvent) =>
                setVoiceInputDevice((e.target as HTMLSelectElement).value),
              options: [
      ...
      h(
        "button",
        {
          ...
          class:
            "bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-6",
        },
        "Reset to default",
      ),
    ],
