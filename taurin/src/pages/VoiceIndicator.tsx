...
const VoiceIndicator = ({}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      removeEventListeners.push(
        await listen<string>("voice-input-state", (event) => {
  ...
  return h(
    "div",
    {
      class: `flex items-center justify-center gap-2 w-screen h-screen bg-gray-800 px-4 m-0 box-border ${stoppable ? "cursor-pointer" : ""}`,
      onClick: handleClick,
    },
    [
      state === "recording" &&
        h("div", {
          class:
            "w-3 h-3 bg-red-500 rounded-full animate-[recording-pulse_1.5s_infinite]",
        }),
      state === "processing" &&
        h("div", {
          class:
            "w-4 h-4 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin",
        }),
