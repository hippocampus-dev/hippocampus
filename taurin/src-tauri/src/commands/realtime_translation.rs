...
pub async fn start_realtime_translation(
    ...
    let cloned_stop_notify = std::sync::Arc::clone(&state.realtime_translation.stop_notify);
    let cloned_language = state.realtime_translation.language.clone();
    tokio::spawn(async move {
        if let Err(e) =
            internal::openai_realtime(cloned_stop_notify, rx, cloned_language, result_tx).await
        {
...
pub async fn stop_realtime_translation(
    ...
    state.realtime_translation.stop_notify.notify_one();
    Ok(())
}
