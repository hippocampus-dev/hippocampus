...
RUN --mount=type=cache,target=/opt/builder/target --mount=type=cache,target=/usr/local/cargo/registry cargo build --release --bin $bin --target x86_64-unknown-linux-gnu && mv /opt/builder/target/x86_64-unknown-linux-gnu/release/$bin /usr/local/bin/main

ENTRYPOINT ["/usr/local/bin/main"]

# Use distroless as minimal base image to package instead of `target-feature=+crt-static`
