...
RUN --mount=type=cache,target=/usr/local/cargo/registry --mount=type=cache,target=/opt/builder/target cargo build --release --bin $bin --features $features --target x86_64-unknown-linux-gnu && cp /opt/builder/target/x86_64-unknown-linux-gnu/release/$bin /usr/local/bin/main
