...
export default SlackFunction(def, async (
    {inputs, token}: { inputs: { parent_message_link: string, reacted_message_link: string }, token: string },
): Promise<{ outputs: { parent_message: string, reacted_message: string } } | { error: string }> => {
    // e.g. https://deno.slack.com/archives/C01U6P7JZ9W/p1627985207000100
    const {parent_message_link, reacted_message_link} = inputs;

    try {
        const [parent, reacted] = await Promise.all([parent_message_link, reacted_message_link].map(async (link) => {
