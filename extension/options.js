chrome.storage.sync.get(
  [
    ...
    "slackEmojiMap",
    "blockedChannels",
    "autoCloseUrlPatterns",
  ],
  function (items) {
    document.getElementById("enable-styler").checked = items.enableStyler;
    document.getElementById("api-key").value = items.apiKey || "";
    document.getElementById("slack-prefix-key").value =
      items.slackPrefixKey || "ctrl";
    ...
    Object.entries(emojiMap).forEach(([key, emoji]) => {
      ...
      bindingDiv.style.cssText =
        "display: grid; grid-template-columns: 50px 200px 80px; gap: 10px; margin-bottom: 5px;";
    ...
    blockedChannels.forEach((channel) => {
      ...
      channelDiv.style.cssText =
        "display: flex; gap: 10px; margin-bottom: 5px; align-items: center; max-width: 400px;";

      const channelSpan = document.createElement("span");
      channelSpan.textContent = channel;
      channelSpan.style.cssText =
        "padding: 5px; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0;";
      ...
      if (urlPatterns.length === 0) {
        ...
        urlPatterns.forEach((pattern) => {
          ...
          patternSpan.style.cssText =
            "padding: 5px; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; font-family: monospace; background: #f0f0f0; border-radius: 3px;";
    ...
    document
      .getElementById("new-url-pattern")
...
document.getElementById("options-form").addEventListener("submit", (event) => {
  ...
  const urlPatterns = [];
  document
    .querySelectorAll("#url-patterns-list > div")
    .forEach((patternDiv) => {
  ...
  const items = {
    enableStyler,
    slackPrefixKey: prefixKey,
    slackEmojiMap: emojiMap,
    autoCloseUrlPatterns: urlPatterns,
  };
  if (apiKey) {
    items.apiKey = apiKey;
  }
