(() => {
  ...
  const loadSettings = () => {
    chrome.storage.sync.get(["slackEmojiMap", "slackPrefixKey"], (items) => {
  ...
  chrome.storage.onChanged.addListener((changes, namespace) => {
    if (
      namespace === "sync" &&
      (changes.slackEmojiMap || changes.slackPrefixKey)
    ) {
      loadSettings();
    }
  });
  ...
  const addReaction = async (messageElement, emoji) => {
    ...
    const contextMenuItems = document.querySelectorAll(
      '[data-qa="menu_item_button"], [role="menuitem"]',
    );
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const firstEmoji = document.querySelector(
          '[data-qa="emoji_list_item"]:first-child',
        );
        if (firstEmoji) {
          firstEmoji.click();
        }
