(() => {
  const URL_PATTERN = /(\/[^/]+\/[^/]+\/blob\/[^/]+\/).+/;
  const FILE_PATH_PATTERN = /(.+.yaml)/;
  ...
  monitor("[data-code-text]", (nodes) => {
    ...
    [...nodes]
      .filter(
        (node) =>
          FILE_PATH_PATTERN.test(node?.dataset?.codeText) &&
  ...
  monitor("nav[aria-label='Repository']", (nodes) => {
    [...nodes].forEach((node) => {
      const navigations = node.querySelectorAll(
        "nav[aria-label='Repository']>ul>li",
      );
