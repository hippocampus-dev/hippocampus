(() => {
    ...
    monitor("nav[aria-label='Repository']", (nodes) => {
        [...nodes].forEach((node) => {
            const navigations = node.querySelectorAll("nav[aria-label='Repository']>ul>li");
            ...
            const appendNavigation = (identifier, filepath) => {
                ...
                const anchor = newNavigation.querySelector("a[data-turbo-frame='repo-content-turbo-frame']");
