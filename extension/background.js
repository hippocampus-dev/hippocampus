...
fetch("https://kaidotio-hippocampus.pages.dev/extension/manifest.json")
  .then((response) => {
...
const cortexReaderStreamMessageToDialogInTab = async (
  reader,
  tabId,
  abortController,
) => {
...
const cortexReaderStreamMessageToDOMInTab = async (
  reader,
  tabId,
  id,
  abortController,
    ...
    .then(async (tabs) => {
      ...
      switch (message.type) {
        ...
        case "translate": {
          ...
          if (
            abortControllerMap["dialog:message"] !== undefined &&
            !abortControllerMap["dialog:message"].signal.aborted
          ) {
          ...
          const response = await fetch(
            "https://cortex-api.kaidotio.dev/v1/chat/completions",
            {
              method: "POST",
              headers: {
...
- Translate into ${message.language ?? chrome.i18n.getUILanguage()}.`,
          ...
          return cortexReaderStreamMessageToDialogInTab(
            reader,
            tabs[0].id,
            abortController,
          );
...
- Translate into ${message.language ?? chrome.i18n.getUILanguage()}.`,
            ...
            await cortexReaderStreamMessageToDOMInTab(
              reader,
              tabs[0].id,
              node.id,
              abortController,
          ...
          while (queue.length > 0 || inProgress.size > 0) {
            ...
            while (
              queue.length > 0 &&
              inProgress.size < MAX_CONCURRENT_TRANSLATIONS
            ) {
...
chrome.contextMenus.onClicked.addListener(async (message, tab) => {
  ...
  if (message.menuItemId.startsWith("translate-dom:")) {
    ...
    const language = message.menuItemId.replace(
      "translate-dom-with-original:",
      "",
    );
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
...
chrome.tabs.onCreated.addListener((tab) => {
  ...
  chrome.storage.sync.get(["autoCloseUrlPatterns"], (items) => {
    ...
    chrome.tabs.get(tab.openerTabId, (parentTab) => {
      ...
      if (
        shouldAutoClose &&
        parentTab.url !== tab.pendingUrl &&
        parentTab.url !== tab.url
      ) {
        setTimeout(() => {
