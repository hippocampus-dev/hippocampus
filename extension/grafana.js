    ...
    const gitHubLinkify = (left, node, match, filepath, lineno) => {
        ...
        if (filepath.startsWith("/")) { // Absolute Path
            if (filepath.startsWith("/opt")) { // see: https://refspecs.linuxfoundation.org/FHS_3.0/fhs/index.html
                ...
                return `<a target="_blank" style="text-decoration: underline;" href="https://github.com/kaidotio/hippocampus/blob/HEAD/cluster/applications/${shards.slice(3, shards.length).join("/")}#L${lineno}">${match}</a>`;
            }
        } else { // Relative Path
