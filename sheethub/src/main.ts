...
export const customOnOpen = () => {
    ...
    sheets.forEach((sheet) => {
        ...
        switch (filename.split(".").pop()) {
            case "csv":
                render(sheet, parseRFC4180(content));
                break;
            case "yml":
            case "yaml":
                render(sheet, parseYAML(content, anchor));
                break;
        }
    });
}

const saveHistory = (source: GoogleAppsScript.Spreadsheet.Spreadsheet, history: History) => {
    const sheet = source.getActiveSheet();

    let i = source.getActiveRange().getRowIndex();
...
export const sync = () => {
    ...
    sheets.forEach((sheet) => {
        ...
        switch (filename.split(".").pop()) {
            case "csv":
                gitClient.add(name, stringifyRFC4180(rows));
                break;
            case "yml":
            case "yaml":
                gitClient.add(filename, stringifyYAML(rows, anchor));
                break;
        }
    });
