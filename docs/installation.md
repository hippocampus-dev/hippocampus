...
$ vim /etc/lvm/lvm.conf # add `--auto-repair` to `thin_check_options`, uncomment `thin_pool_autoextend_threshold = 70`, `thin_pool_autoextend_percent = 20`, uncomment `issue_discards = 1`
...
options cryptdevice=/dev/mapper/vg0-root:root:allow-discards cryptkey=rootfs:/etc/luks_keyfile resume=/dev/vg0/swap root=/dev/mapper/root rw lsm=capability,landlock,lockdown,yama,bpf,apparmor

$ git clone --recursive https://github.com/hippocampus-dev/hippocampus /opt/hippocampus
$ exit
$ reboot
...
6. Check `Tools | AI Assistant | Features | Languages | All Other`
7. Check `Tools | AI Assistant | Features | Preview instantly`
8. Check `Tools | AI Assistant | Features | Chain suggestions`
9. Change `Tools | AI Assistant | Features | Natural Language | Receive AI Assistant chat responses in a custom language` to `Japanese`
10. Check `Tools | AI Assistant | Features | Allow attaching database schemas to AI Assistant chat`
11. Setup `Tools | AI Assistant | Providers & API Keys | Third-party AI providers` to `https://ollama.127.0.0.1.nip.io`
12. Download `Editor | General | Inline Completion | Enable local Full Line completion suggestions`
13. Check `Editor | General | Inline Completion | Enable cloud completion suggestions | XXML`
14. Check `Editor | General | Inline Completion | Enable cloud completion suggestions | All others`
15. Change `Editor | General | Inline Completion | Enable cloud completion suggestions | Completion policy` to `Creative`
16. Check `Editor | General | Inline Completion | Synchronize inline and popup completions`
17. Change `Editor | General | On Save | Remove trailing spaces on` to `All lines`
18. Uncheck `Editor | General | On Save | Keep trailing spaces on caret line`
19. Check `Editor | General | On Save | Remove trailing blank lines at the end of saved files`
20. Check `Editor | General | On Save | Ensure every saved file ends with a line break`
21. Change `Editor | General | Soft Wraps | Soft-wraps these files` to `*`
22. Check `Editor | General | Appearance | Show whitespaces`
23. Change `Editor | Font | Size` to `14.0`
24. Change `Editor | Code Style | Go | Imports | Sorting type` to `goimports`
25. Change `Editor | Code Style | Python | Tabs and Indents | Continuation Indent` to `4`
26. Uncheck `Editor | Code Style | Python | Other | Use continuation indent for | Method declaration parameters`
27. Setup `Tools | Actions on Save | Reformat code` on `Go`, `Rust`
28. Setup `Tools | Actions on Save | Optimize imports` on `Go`, `Rust`
29. Setup `Languages & Framework | Go | GOROOT`
30. Change `Languages & Framework | Rust | External Linters | External linter` to `Clippy`
31. Add `/opt/hippocampus/.idea/crd/crd.yaml` to `Languages & Framework | Kubernetes`
32. Setup `Project Structure | Platform Settings | SDKs | Add Python SDK from disk | Virtualenv Environment | Existing environment` on `.venv/bin/python`
33. Add `target`, `.venv`, `.gradle`, `proto` to `Editor | File Types | Ignore files and folders`
34. Change `Help | Change Memory Settings | Max Heap Size` to `4096`
