...
8. Setup `Tools | AI Assistant | Models | Third-party AI providers` to `https://ollama.127.0.0.1.nip.io`
9. Download `Editor | General | Inline Completion | Enable local Full Line completion suggestions`
10. Check `Editor | General | Inline Completion | Enable cloud completion suggestions | SQL`
11. Check `Editor | General | Inline Completion | Enable cloud completion suggestions | Universal completion`
12. Change `Editor | General | Inline Completion | Enable cloud completion suggestions | Completion policy` to `Creative`
13. Check `Editor | General | Inline Completion | Synchronize inline and popup completions`
14. Change `Editor | General | On Save | Remove trailing spaces on` to `All lines`
15. Uncheck `Editor | General | On Save | Keep trailing spaces on caret line`
16. Check `Editor | General | On Save | Remove trailing blank lines at the end of saved files`
17. Check `Editor | General | On Save | Ensure every saved file ends with a line break`
18. Change `Editor | General | Soft Wraps | Soft-wraps these files` to `*`
19. Check `Editor | General | Appearance | Show whitespaces`
20. Change `Editor | Font | Size` to `16.0`
21. Change `Editor | Code Style | Go | Imports | Sorting type` to `goimports`
22. Change `Editor | Code Style | Python | Tabs and Indents | Continuation Indent` to `4`
23. Uncheck `Editor | Code Style | Python | Other | Use continuation indent for | Method declaration parameters`
24. Change `Editor | Natural Language | Grazie Pro | Text Completion | Completion` to `Always`
25. Setup `Tools | Actions on Save | Reformat code` on `Go`, `Python`, `JavaScript`, `TypeScript`, `Rust`
26. Setup `Tools | Actions on Save | Optimize imports` on `Go`, `Python`, `JavaScript`, `TypeScript`, `Rust`
27. Setup `Languages & Framework | Go | GOROOT`
28. Check `Languages & Framework | Go | Go Modules | Enable Go modules integration`
29. Change `Languages & Framework | Rust | External Linters | External linter` to `Clippy`
30. Check `Languages & Framework | Rust | External Linters | Run external linter on the fly`
