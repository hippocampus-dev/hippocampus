...
### Structure
- `main.tf` - Google Cloud Storage bucket definitions
- `versions.tf` - Terraform and provider version constraints
- `providers.tf` - Provider configurations (Google, Cloudflare)
- `variables.tf` - Input variable definitions
- `terraform.tfvars.example` - Example variable values
- `r2.tf` - Cloudflare R2 storage configurations
...
- **Rust**: Use full module paths (e.g., `std::env::var` instead of `use std::env`)
- **Go**: Implement tests with TableDrivenTest pattern for multiple test cases
- **Python**: Use full module paths (e.g., `collections.abc.Mapping` instead of `from collections.abc import Mapping`)
- **JavaScript**: Use Preact with `h` function (no JSX/TSX syntax)
