...
- **Docker Config Isolation**: Build workflows (`00_*.yaml`) conditionally set `DOCKER_CONFIG` to `${{ runner.temp }}/.docker` on self-hosted local runners (via `contains(needs.runner.outputs.runner, 'local')`), placed before `uses: ./.github/actions/setup-docker`. This prevents `docker login` from overwriting `~/.docker/config.json` with short-lived `ghs_` GitHub App installation tokens
