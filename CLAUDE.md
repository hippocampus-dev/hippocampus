...
- **`/cluster/applications/`** - Kubernetes microservices:
  - AI/ML services (embedding-gateway, embedding-retrieval, lancer, memory-bank, whisper-worker, translator)
  - Infrastructure (various pod hooks, controllers, scheduler plugins, proxies)
...
- **Dynamic Runner Selection**: The project uses a reusable workflow (`reusable_dynamic-runner.yaml`) that intelligently selects between self-hosted and GitHub-hosted runners. It uses `reusable_issue-github-token.yaml` with the `administration-reader` profile to obtain a GitHub App token via the public endpoint (`hippocampus-dev-github-token-server-public.kaidotio.dev`) for querying the self-hosted runners API. When the token server is unavailable, the workflow partially fails but still resolves a fallback runner. Downstream jobs must use `!cancelled()` in their `if:` conditions to run despite this partial failure
- **Git Config Isolation**: Build, mirror, and scan workflows (`00_*.yaml`, `99_mirror-*.yaml`, `reusable_scan-image.yaml`) conditionally set `GIT_CONFIG_GLOBAL` to `${{ runner.temp }}/gitconfig` on self-hosted local runners (via `contains(needs.runner.outputs.runner, 'local')` or `contains(inputs.runner, 'local')` in reusable workflows), followed by `git config --global submodule.recurse false`, both placed before `actions/checkout`. This prevents `actions/checkout`'s internal `git config --global --add safe.directory` from polluting `~/.gitconfig` with temporary directory entries on self-hosted runners
- **Docker Config Isolation**: Build workflows (`00_*.yaml`), mirror workflows (`99_mirror-*.yaml`), and reusable workflows conditionally set `DOCKER_CONFIG` to `${{ runner.temp }}/.docker` on self-hosted local runners (via `contains(needs.runner.outputs.runner, 'local')` or `contains(inputs.runner, 'local')` / `contains(inputs.runs-on, 'local')` in reusable workflows), placed before `docker login` or `uses: ./.github/actions/setup-docker`. This prevents `docker login` from overwriting `~/.docker/config.json` with short-lived `ghs_` GitHub App installation tokens
- **Security Scanning**: Docker image builds include vulnerability scanning via Trivy (`reusable_scan-image.yaml`). Both application builds (`00_*.yaml`) and mirrored image workflows (`99_mirror-*.yaml`) add scan jobs individually calling `reusable_scan-image.yaml` (CRITICAL severity, with automatic per-CVE issue creation). Created issues require triage verification of actual exploitability including version verification against source files, attack surface verification confirming the CVE's specific attack prerequisites are met in the deployment, and CVSS Attack Vector assessment
- **Claude AI Integration**: GitHub Actions support Claude AI interactions via `@claude` mentions in issues and PRs
- **Dependabot PR Triage**: Dependabot PRs are automatically triaged by posting an `@claude` comment that instructs Claude to review release notes, verify usage compatibility, and approve or comment (`10_triage-dependabot.yaml`)
- **Auto-Retry on Failure**: Transient CI failures are handled centrally by `claude-auto-fix-ci.yaml`, which triggers on any workflow failure via `workflow_run`. On the first attempt (`run_attempt == 1`), it retries the failed run using `gh run rerun --failed`. If the rerun is not accepted, it falls back to creating a Claude-powered fix PR. Individual workflows do not include their own `rerun` jobs
...
- `docker-compose up --profile=<profile>` - Run specific AI/ML stacks:
  ...
  - `pianify` - Pianify OMR pipeline (sheet music to MusicXML)
  - `yue` - Yue server
  - `open-webui` - Open WebUI for LLMs
- Docker Compose includes extensive observability stack (Prometheus, Grafana, Tempo, Loki, Pyroscope)
...
- **Biome**: TypeScript projects use Biome for formatting and linting (`make fmt` / `make lint`). Root `biome.jsonc` provides formatter defaults for all other JS/TS projects
...
### Observability
- Built-in support for OpenTelemetry, Tempo tracing, Loki logging, Prometheus metrics
- Grafana dashboards pre-configured for system and application monitoring
- Pyroscope for continuous profiling

### MCP (Model Context Protocol) Servers
- Multiple MCP servers configured: GitHub, Playwright, Spotify, filesystem, inspector
