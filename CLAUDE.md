...
- **Auto-Retry on Failure**: All non-reusable workflows include a `rerun` job that automatically retries failed runs on the first attempt (`github.run_attempt == 1`) using `gh run rerun --failed`. This handles transient failures (e.g., API 529 overloaded errors) before falling back to `claude-auto-fix-ci`. Requires `actions: write` permission
