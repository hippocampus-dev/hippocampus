...
EOS
  ...
  for branch in "${branches[@]}"; do
    curl -fsSL -XPOST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer ${GITHUB_TOKEN}" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/${REPOSITORY}/environments/deployment/deployment-branch-policies -d "{\"name\":\"${branch}\",\"type\":\"branch\"}" || echo "Failed to add branch policy for ${branch}"
  done
  curl -fsSL -XPOST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer ${GITHUB_TOKEN}" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/${REPOSITORY}/environments/deployment/deployment-branch-policies -d "{\"name\":\"v*\",\"type\":\"tag\"}" || echo "Failed to add tag policy"
