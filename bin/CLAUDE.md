...
### Version Management
- `./bump.sh <version>` - Update version numbers across all Cargo.toml, pyproject.toml, version.go, tauri.conf.json, and gradle.properties files. Example: `./bump.sh 1.2.3`
...
- `./sync-agent-files.sh` - Convert Claude Code configurations for Codex and Gemini CLI. Adds auto-generated markers to output files (HTML comments for Markdown, `#` comments for TOML, `_generated` field inside `hooks`/`mcpServers` objects for JSON)
...
3. **Error Handling**: All scripts use `set -eo pipefail` with process substitution (`< <(command)`) instead of pipes to ensure proper error propagation
...
- GitHub API operations are hardcoded for the `hippocampus-dev/hippocampus` repository
