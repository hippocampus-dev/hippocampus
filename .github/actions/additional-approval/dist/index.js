...
const main = async () => {
  ...
  const organizationToken = core__namespace.getInput("organization-token", {
    required: true,
  });
  const repositoryToken = core__namespace.getInput("repository-token", {
    required: true,
  });
  const ownersFiles = core__namespace.getInput("owners-files", {
    required: true,
  });
  ...
  if (
    ...
    const existingUsers = new Set(
      reviewRequests.flatMap((page) => page.users || []).map((u) => u.login),
    );
    const existingTeams = new Set(
      reviewRequests.flatMap((page) => page.teams || []).map((t) => t.slug),
    );
    const users = [];
    const teams = [];
  ...
  const getFileApprovalStatus = async (file, owners) => {
    ...
    for (const owner of owners) {
      ...
      if (isTeam) {
        ...
        ).filter((m) => m !== context.payload.pull_request.user.login);
        teamMembers.forEach((member) => ownerUsernames.add(member));
        const approver = teamMembers.find((member) =>
