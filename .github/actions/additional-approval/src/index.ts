...
const main = async (): Promise<void> => {
  ...
  const getTeamMembers = async (
    ...
    if (!teamMembersCache.has(cacheKey)) {
      const members = await organizationOctokit.paginate(
        organizationOctokit.rest.teams.listMembersInOrg,
        {
          org: organization,
          team_slug: teamSlug,
        },
      );
      teamMembersCache.set(
        cacheKey,
        members.map((m: TeamMember) => m.login),
      );
    }
  ...
  const listedFiles = await repositoryOctokit.paginate(
    repositoryOctokit.rest.pulls.listFiles,
    {
  ...
  if (
    ...
    ["opened", "synchronize"].includes(context.payload.action)
  ) {
    const reviewRequests = await repositoryOctokit.paginate(
      repositoryOctokit.rest.pulls.listRequestedReviewers,
      {
        owner: context.repo.owner,
        repo: context.repo.repo,
        pull_number: context.payload.pull_request.number,
    ...
    const existingUsers = new Set(
      reviewRequests
        .flatMap((page: ReviewRequestPage) => page.users || [])
        .map((u: GitHubUser) => u.login),
    );
    const existingTeams = new Set(
      reviewRequests
        .flatMap((page: ReviewRequestPage) => page.teams || [])
        .map((t: GitHubTeam) => t.slug),
    );
    ...
    for (const owner of allOwners) {
      const clean = (owner as string).startsWith("@")
        ? (owner as string).substring(1)
        : (owner as string);
      ...
      if (isTeam) {
        ...
        teamMembers.forEach((member: string) => {
          if (
            !existingUsers.has(member) &&
            member !== context.payload.pull_request.user.login
          ) {
            users.push(member);
          }
        });
        ...
        if (
          !existingUsers.has(clean) &&
          clean !== context.payload.pull_request.user.login
        ) {
          users.push(clean);
        }
      }
  ...
  const allReviews = await repositoryOctokit.paginate(
    repositoryOctokit.rest.pulls.listReviews,
    {
      owner: context.repo.owner,
      repo: context.repo.repo,
      pull_number: context.payload.pull_request.number,
  ...
  for (const review of allReviews) {
    ...
    if (
      !latestReviews.has(user) ||
      new Date(review.submitted_at) >
        new Date(latestReviews.get(user).submitted_at)
    ) {
  ...
  const getFileLastModifiedTime = async (file: string): Promise<Date> => {
    const commits = await repositoryOctokit.paginate(
      repositoryOctokit.rest.repos.listCommits,
      {
        owner: context.repo.owner,
        repo: context.repo.repo,
        path: file,
  ...
  const isApprovalValidForFile = async (
    approval: GitHubReview,
    file: string,
  ): Promise<boolean> => {
  ...
  const getValidApprovedUsersForFile = async (
    file: string,
  ): Promise<string[]> => {
  ...
  const getFileApprovalStatus = async (
    file: string,
    owners: string[],
  ): Promise<{
    ...
    for (const owner of owners) {
      ...
      if (isTeam) {
        const [organization, teamSlug] = clean.split("/");
        const teamMembers = (
          await getTeamMembers(organization, teamSlug)
        ).filter((m: string) => m !== context.payload.pull_request.user.login);
        teamMembers.forEach((member: string) => ownerUsernames.add(member));

        const approver = teamMembers.find((member: string) =>
          validApprovedUsers.includes(member),
        );
        if (approver) {
    ...
    if (approvedOwners.length === 0) {
      for (const review of latestReviews.values()) {
        if (
          ...
          ownerUsernames.has(review.user.login)
        ) {
  ...
  if (standardCodeowners.size === 0) {
    const approvers = Array.from(latestReviews.values()).filter(
      (review: GitHubReview) =>
        review.state === "APPROVED" &&
    ...
    if (approvers.length === 0) {
      const message =
        "No CODEOWNERS file found. At least one approval is required.";
  ...
  commentBody +=
    "This PR requires additional approvals based on the following rules:\n\n";
  commentBody +=
    "**Important:** Approvals are tracked per-file. If a file is modified after approval, the approval for that specific file becomes invalid and needs to be renewed.\n\n";
  ...
  for (const [ownersFile, grouping] of filesByOwnerFile.entries()) {
    ...
    commentBody +=
      "| File | Status | Required Reviewers | Approved | Pending |\n";
    commentBody +=
      "|------|--------|-------------------|----------|----------|\n";

    for (const { file, owners, approvalStatus } of grouping) {
      const statusIcon = approvalStatus.hasRequiredApproval
        ? ":white_check_mark:"
        : ":hourglass_flowing_sand:";
      const statusText = approvalStatus.hasRequiredApproval
        ? "Approved"
        : "Pending";
      ...
      const approvedText =
        approvalStatus.approvedOwners.length > 0
  ...
  commentBody +=
    "*Review requests have been automatically sent to all required reviewers.*\n";

  const comments = await repositoryOctokit.paginate(
    repositoryOctokit.rest.issues.listComments,
    {
      owner: context.repo.owner,
      repo: context.repo.repo,
  ...
  if (unapprovedFiles.length > 0) {
    const message = unapprovedFiles
      .map(({ file, missingApprovals }) => {
        const approvalDetails = missingApprovals
          .map(
            ({ ownersFile, owners }) =>
