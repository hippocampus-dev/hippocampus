...
function cortex() {
  id_token=$(curl -fsSL --retry 5 --retry-all-errors -H "Authorization: Bearer $ACTIONS_ID_TOKEN_REQUEST_TOKEN" "${ACTIONS_ID_TOKEN_REQUEST_URL}&audience=$GITHUB_ACTION_PATH" | jq -re '.value')
  echo "::add-mask::${id_token}" >&2
  http_code=$(curl -sSL --retry 5 --retry-all-errors -w "%{http_code}" -o "${RUNNER_TEMP}/response.json" -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $id_token" https://cortex-api-public.kaidotio.dev/v1/chat/completions -d "$(echo "$@" | jq -sc --arg model "$MODEL" '. as $messages | {model: $model, messages: $messages}')")
...
if [ "$COMMENT_BODY" == "/rebase" ] && [ "$PULL_REQUEST_EVENT" != "" ]; then
  pr=$(curl -fsSL --retry 5 --retry-all-errors -H "Authorization: Bearer $GITHUB_TOKEN" "https://api.github.com/repos/${GITHUB_REPOSITORY}/pulls/${ISSUE_NUMBER}")
  ...
  if git rebase "origin/${base}"; then
    ...
    curl -fsSL --retry 5 --retry-all-errors -X POST -H "Authorization: Bearer $GITHUB_TOKEN" "https://api.github.com/repos/${GITHUB_REPOSITORY}/issues/${ISSUE_NUMBER}/comments" -d "$(jq -n --arg body "$body" '{body: $body}')"
  fi
fi
...
if [ "$COMMENT_BODY" == "/cortex translate" ]; then
  issue=$(curl -fsSL --retry 5 --retry-all-errors -H "Authorization: Bearer $GITHUB_TOKEN" "https://api.github.com/repos/${GITHUB_REPOSITORY}/issues/${ISSUE_NUMBER}")
...
if [ "$COMMENT_BODY" == "/cortex summary" ]; then
  ...
  page=1
  while true; do
    result=$(curl -fsSL --retry 5 --retry-all-errors -H "Authorization: Bearer $GITHUB_TOKEN" "https://api.github.com/repos/${GITHUB_REPOSITORY}/issues/${ISSUE_NUMBER}/comments?per_page=100&page=${page}") || break
