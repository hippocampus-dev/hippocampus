...
function cortex() {
  ...
  http_code=$(curl -sSL -w "%{http_code}" -o "${RUNNER_TEMP}/response.json" -X POST -H "Content-Type: application/json" -H "Authorization: Bearer ${id_token}" http://cortex-api.cortex-api.svc.cluster.local:8080/v1/chat/completions -d "$(echo ${@} | jq -sc --arg model ${MODEL} '. as $messages | {model: $model, messages: $messages}')")
