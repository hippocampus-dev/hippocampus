...
function cortex() {
  ...
  http_code=$(curl -sSL -w "%{http_code}" -o "${RUNNER_TEMP}/response.json" -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $id_token" https://cortex-api-public.kaidotio.dev/v1/chat/completions -d "$(echo "$@" | jq -sc --arg model "$MODEL" '. as $messages | {model: $model, messages: $messages}')")
...
if [ "$COMMENT_BODY" == "/cortex summary" ]; then
  ...
  page=1
  while true; do
    result=$(curl -fsSL -H "Authorization: Bearer $GITHUB_TOKEN" "https://api.github.com/repos/${GITHUB_REPOSITORY}/issues/${ISSUE_NUMBER}/comments?per_page=100&page=${page}") || break
