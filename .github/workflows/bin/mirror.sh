...
set +e
DIGEST=$(curl -fsSLo /dev/null --retry 3 --retry-delay 5 -H "Authorization: Bearer $(echo "$GITHUB_TOKEN" | base64 -w0)" "https://ghcr.io/v2/${IMAGE_PATH}/manifests/${TAG}" -D - | awk '/docker-content-digest/' | awk '/sha256:[a-f0-9]{64}/ { match($0, /sha256:[a-f0-9]{64}/); print substr($0, RSTART, RLENGTH) }')
set -e

if [ -z "$DIGEST" ]; then
  docker pull "${IMAGE}:${TAG}"
  docker tag "${IMAGE}:${TAG}" "${GHCR_IMAGE}:${TAG}"
  DIGEST=$(docker push "${GHCR_IMAGE}:${TAG}" | awk '/sha256:[a-f0-9]{64}/ { match($0, /sha256:[a-f0-9]{64}/); last=substr($0, RSTART, RLENGTH) } END { print last }')
