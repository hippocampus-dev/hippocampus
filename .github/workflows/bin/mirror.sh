...
set +e
DIGEST=$(curl -fsSLo /dev/null --retry 5 -H "Authorization: Bearer $(echo "$GITHUB_TOKEN" | base64 -w0)" "https://ghcr.io/v2/${IMAGE_PATH}/manifests/${TAG}" -D - | awk '/docker-content-digest/' | awk '/sha256:[a-f0-9]{64}/ { match($0, /sha256:[a-f0-9]{64}/); print substr($0, RSTART, RLENGTH) }')
set -e
