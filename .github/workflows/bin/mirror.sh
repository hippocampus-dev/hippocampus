...
set +e
DIGEST=$(curl -fsSL -H "Authorization: Bearer $(echo ${GITHUB_TOKEN} | base64 -w0)" https://ghcr.io/v2/${IMAGE_PATH}/manifests/${TAG} -o /dev/null -D - | grep docker-content-digest | grep -Eo 'sha256:[a-f0-9]{64}')
set -e
