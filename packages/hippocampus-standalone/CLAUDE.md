...
1. **Storage Configuration** (`config.rs`)
   - Supports File and GCS (Google Cloud Storage) backends
...
- **File/File**: Both documents and tokens stored locally
- **File/GCS**: Documents stored locally, tokens in Google Cloud Storage (requires service account key via `--key-file`)
...
The `hippocampus.toml` file configures storage backends:
