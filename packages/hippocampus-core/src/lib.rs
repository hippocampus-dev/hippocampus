...
pub struct PostingsList {
    ...
    skip_pointers: skip_list::SkipPointers,
}
...
impl PostingsList {
    fn new() -> Self {
        Self {
            postings: Vec::new(),
            skip_pointers: skip_list::SkipPointers::new(),
        }
    }

    fn from_postings(mut postings: Vec<Posting>) -> Self {
        postings.sort();
        let skip_pointers = skip_list::SkipPointers::build(&postings, |p| p.document_uuid);
        Self {
            postings,
            skip_pointers,
        }
    ...
    fn intersection(&self, other: Self) -> Self {
        ...
        if use_skip_pointers {
            ...
            for small_posting in smaller.iter() {
                ...
                j = skip_list::galloping_search(&larger.postings, target, j, |p| p.document_uuid);
    ...
    fn from_bytes(bytes: Vec<u8>) -> Self {
        ...
        let skip_pointers =
            skip_list::SkipPointers::from_bytes(&bytes[postings_end..bytes_len - size_of_usize]);
...
mod test {
    ...
    fn intersection_with_skip_pointers() {
        ...
        let small_list = PostingsList::from_postings(small);
        let large_list = PostingsList::from_postings(large);
        let result = small_list.intersection(large_list);

        assert_eq!(result.len(), 10);
        for (i, posting) in result.postings.iter().enumerate() {
            assert_eq!(posting.document_uuid, (i as u64) * 100);
        }
    }
