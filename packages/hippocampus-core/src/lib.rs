...
pub struct PostingsList {
    ...
    skip_pointers: skip_pointer::SkipPointers,
}
...
impl PostingsList {
    fn new() -> Self {
        Self {
            postings: Vec::new(),
            skip_pointers: skip_pointer::SkipPointers::new(),
        }
    }

    fn from_postings(mut postings: Vec<Posting>) -> Self {
        postings.sort();
        let skip_pointers = skip_pointer::SkipPointers::build(&postings, |p| p.document_uuid);
        Self {
            postings,
            skip_pointers,
        }
    ...
    fn intersection(&self, other: Self) -> Self {
        ...
        if use_skip_pointers {
            ...
            for small_posting in smaller.iter() {
                ...
                j = skip_pointer::galloping_search(&larger.postings, target, j, |p| {
                    p.document_uuid
                });
    ...
    fn from_bytes(bytes: Vec<u8>) -> Self {
        ...
        let skip_pointers =
            skip_pointer::SkipPointers::from_bytes(&bytes[postings_end..bytes_len - size_of_usize]);
