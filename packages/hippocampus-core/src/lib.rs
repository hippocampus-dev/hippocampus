...
impl PostingsList {
    ...
    fn from_postings(mut postings: Vec<Posting>) -> Self {
        postings.sort();
        Self::from_sorted_postings(postings)
    }

    fn from_sorted_postings(postings: Vec<Posting>) -> Self {
        let skip_pointers =
            types::skip_pointer::SkipPointers::build(&postings, |p| p.document_uuid);
        Self {
            postings,
            skip_pointers,
        }
    ...
    fn intersection(&self, other: Self) -> Self {
        ...
        Self::from_sorted_postings(postings_list)
    }
    ...
    fn from_bytes(bytes: Vec<u8>) -> Self {
        ...
        let skip_pointers = types::skip_pointer::SkipPointers::from_bytes(
            &bytes[postings_end..bytes_len - size_of_usize],
        );
