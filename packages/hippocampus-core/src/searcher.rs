    ...
    > DocumentSearcher<DS, TS, T, S>
{
    ...
    fn intersect_postings_list(
        ...
        while first_cursor < first_token_postings_list_size {
            ...
            'first: while i < token_associated_postings_list_list.len() {
                ...
                // postings_list loop
                'next: while next_cursor < next_token_postings_list_size {
                    let next_posting =
                        &token_associated_postings_list_list[i].postings_list[next_cursor];
        ...
        crate::PostingsList(postings_list)
    }
    ...
    ) -> futures::future::BoxFuture<Result<crate::PostingsList, error::Error>> {
        Box::pin(async move {
            match query {
                hippocampusql::Query::Term(term) => {
                ...
                hippocampusql::Query::Phrase(phrase) => {
                    ...
                    Ok(self.intersect_postings_list(&token_associated_postings_list_list, true))
                }
                        ...
                        crate::Value::String(body) => {
                            ...
                            let f = posting
                                ...
                                .map(|position| {
                                    ...
                                    let begin = if position > self.fragments_offset {
...
impl TokenAssociatedPostingsList {
    fn new(token_position: u64, documents_count: u64, postings_list: crate::PostingsList) -> Self {
        Self {
            token_position,
            documents_count,
            postings_list,
...
mod tests {
    ...
    async fn search_with_multiple_fragments() -> Result<(), error::Error> {
        ...
        mock_token_storage
            ...
            .returning(|_| {
                Box::pin(async {
                    Ok(crate::PostingsList(vec![crate::Posting::new(
                        1,
                        std::collections::BTreeSet::from_iter(vec![2, 18]),
                    )]))
                })
            });
    ...
    #[bench]
    fn bench_search(b: &mut test::Bencher) -> Result<(), error::Error> {
        let mut mock_token_storage = crate::storage::MockTokenStorage::new();
        let mut mock_document_storage = crate::storage::MockDocumentStorage::new();
        mock_token_storage
            .expect_get_postings_list()
        ...
        let document1 = crate::Document(bm1);
        let cloned_document1 = document1;
        mock_document_storage
            .expect_find()
            .with(mockall::predicate::eq(1))
            .returning(move |_| {
        ...
        let document2 = crate::Document(bm2);
        let cloned_document2 = document2;
        mock_document_storage
            .expect_find()
            .with(mockall::predicate::eq(2))
            .returning(move |_| {
    ...
    #[bench]
    fn bench_phrased_search(b: &mut test::Bencher) -> Result<(), error::Error> {
        let mut mock_token_storage = crate::storage::MockTokenStorage::new();
        let mut mock_document_storage = crate::storage::MockDocumentStorage::new();
        mock_token_storage
            .expect_get_postings_list()
            .with(mockall::predicate::eq("this"))
            .returning(|_| {
                Box::pin(async {
                    Ok(crate::PostingsList(vec![crate::Posting::new(
                        1,
        ...
        mock_token_storage
            ...
            .returning(|_| {
                Box::pin(async {
                    Ok(crate::PostingsList(vec![
                        crate::Posting::new(1, std::collections::BTreeSet::from_iter(vec![1, 3])),
                        crate::Posting::new(2, std::collections::BTreeSet::from_iter(vec![1, 3])),
                    ]))
                })
            });
        mock_token_storage
        ...
        b.iter(|| {
            rt.block_on(searcher.search(
                &hippocampusql::Query::Phrase(hippocampusql::Phrase(hippocampusql::Term(
                    "this is dummy".to_string(),
                ))),
                SearchOption::default(),
            ))
            .unwrap();
    ...
    async fn eval_single_token() -> Result<(), error::Error> {
        ...
        assert_eq!(
            searcher
                ...
                .await?,
            crate::PostingsList(vec![crate::Posting::new(
                1,
                std::collections::BTreeSet::from_iter(vec![0])
            )]),
        );
    ...
    async fn eval_or() -> Result<(), error::Error> {
        ...
        assert_eq!(
            crate::PostingsList(result),
            crate::PostingsList(vec![
