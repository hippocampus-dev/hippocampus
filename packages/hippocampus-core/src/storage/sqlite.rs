    ...
    T: std::hash::Hasher + Clone + Send + Sync,
{
    ...
    async fn save(&self, document: &crate::Document) -> Result<u64, error::Error> {
        let mut locked_connection = self.connection.lock().map_err(|e| error::error!("{e}"))?;
    ...
    async fn find(&self, uuid: u64) -> Result<crate::Document, error::Error> {
        let locked_connection = self.connection.lock().map_err(|e| error::error!("{e}"))?;
        let mut statement =
    ...
    T: std::hash::Hasher + Clone + Send + Sync,
{
    ...
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self, indices)))]
    #[cfg_attr(debug_assertions, elapsed)]
