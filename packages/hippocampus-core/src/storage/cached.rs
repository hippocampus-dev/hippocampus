use crate::lru::LockFreeLruCache;
use std::sync::Arc;
...
mod tests {
    ...
    async fn test_cache_hit() {
        ...
        mock.expect_get_postings_list()
            ...
            .returning(|_| {
                Box::pin(async {
                    Ok(PostingsList(vec![Posting::new(
                        1,
    ...
    async fn test_cache_eviction() {
        ...
        let cached_storage = CachedTokenStorage::new(mock, 2);

        cached_storage.get_postings_list("token0").await.unwrap();
        cached_storage.get_postings_list("token1").await.unwrap();
        cached_storage.get_postings_list("token2").await.unwrap();

        assert_eq!(cached_storage.cache_stats().await.size, 2);
    }
