...
impl WasmTokenizer {
        ...
        P: AsRef<std::path::Path>,
    {
        ...
        let engine = wasmtime::Engine::new(&configuration).map_err(|e| {
            error::Error::from_message(format!("Failed to create WASM engine: {e}"))
        })?;

        let component_bytes = std::fs::read(path.as_ref())
            .map_err(|e| error::Error::from_message(format!("Failed to read WASM file: {e}")))?;

        let component = wasmtime::component::Component::from_binary(&engine, &component_bytes)
            .map_err(|e| error::Error::from_message(format!("Failed to compile component: {e}")))?;

        let mut linker = wasmtime::component::Linker::new(&engine);
        wasmtime_wasi::add_to_linker_async(&mut linker).map_err(|e| {
            error::Error::from_message(format!("Failed to add WASI to linker: {e}"))
        })?;
...
impl crate::tokenizer::Tokenizer for WasmTokenizer {
        ...
        S: AsRef<str> + std::fmt::Debug,
    {
        ...
        handle.block_on(async {
            ...
            store
                .set_fuel(10_000_000)
                .map_err(|e| error::Error::from_message(format!("Failed to set fuel: {e}")))?;

            let instance = Tokenizer::instantiate_async(&mut store, &self.component, &self.linker)
                .await
                .map_err(|e| error::Error::from_message(format!("Failed to instantiate: {e}")))?;

            let tokens = instance
                .call_tokenize(&mut store, &content_string)
                .await
                .map_err(|e| error::Error::from_message(format!("Tokenize call failed: {e}")))?;
