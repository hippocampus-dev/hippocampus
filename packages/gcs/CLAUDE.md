...
3. **Authentication** - JWT-based Google service account authentication with automatic token refresh via SWR (stale-while-revalidate) caching
...
- `swr` - Stale-while-revalidate cache for token management
