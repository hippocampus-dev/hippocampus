//! A tokio-based [Google Cloud Storage] client with authentication, retries, and hedged requests.
//!
//! [Google Cloud Storage]: https://cloud.google.com/storage/docs/json_api/v1
//!
//! # Examples
//!
//! ```rust,ignore
...
//!     .set_connect_timeout(std::time::Duration::from_millis(100))
//!     .build(service_account_key)?;
//!
//! // Upload
//! client.upload("bucket", "object/path", body).await?;
//!
//! // Download
//! let data = client.download("bucket", "object/path").await?;
//! ```
...
pub struct Client {
    ...
    token_cache: swr::Cache<Scope, Token, error::Error>,
    host: hyper::http::Uri,
...
impl Client {
    ...
    async fn auth(&self, scope: Scope) -> Result<Token, error::Error> {
        ...
        self.token_cache
            .get(&scope, move || async move {
                client.fetch_token(scope_clone).await
            })
            .await
    ...
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self)))]
    async fn fetch_token(&self, scope: Scope) -> Result<swr::FetchResult<Token>, error::Error> {
        ...
        Ok(swr::FetchResult {
            value: Token::from(token_response),
            stale_after: std::time::Duration::from_secs(expires_in.saturating_sub(60)),
            expire_after: std::time::Duration::from_secs(expires_in.saturating_sub(30)),
        })
    }

    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self)))]
    #[cfg_attr(debug_assertions, elapsed)]
    pub async fn download<S>(&self, bucket: S, object: S) -> Result<(Vec<u8>, i64), error::Error>
    where
        S: AsRef<str> + std::fmt::Debug + Sync,
    {
        let result: (hyper::body::Bytes, i64) =
            retry::spawn(self.retry_strategy.clone(), || async {
                hedged::spawn(std::time::Duration::from_millis(50), 1, || async {
                    ...
                    Ok((response_body_bytes, generation))
                })
                .await
            })
            .await?;
        Ok((result.0.to_vec(), result.1))
    }
            ...
            retry::spawn(self.retry_strategy.clone(), || async {
                hedged::spawn(std::time::Duration::from_millis(50), 1, || async {
                    ...
                    if let Some(g) = generation {
                        uri.push_str(&format!("&ifGenerationMatch={g}"));
                    }
...
fn handle_error<T, E>(error: E) -> Result<T, retry::RetryError>
where
...
fn handle_hyper_status_code<T>(
    ...
    match status_code {
        ...
        hyper::http::StatusCode::PRECONDITION_FAILED => Err(
            retry::RetryError::from_unexpected_error(Box::new(Error::PreconditionFailed)),
        ),
